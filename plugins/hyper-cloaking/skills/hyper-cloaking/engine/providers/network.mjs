...
export class NetworkReadError extends Error {
  constructor(code, message, { phase = 'network-read', dispatched = false, fallbackEligible = false, cause } = {}) {
    super(message, { cause });
...
export function canUseDomFallback(error) {
  return error instanceof NetworkReadError && error.dispatched === false && error.fallbackEligible === true;
}

export function createReadPromotionDefaults(actions) {
  if (!Array.isArray(actions) || actions.length === 0) throw new TypeError('read promotion actions must be a non-empty array');
  const names = actions.map((action) => requireText(action, 'read promotion action'));
  if (new Set(names).size !== names.length) throw new TypeError('read promotion actions must be unique');
  return Object.freeze(Object.fromEntries(names.map((action) => [
    action,
    Object.freeze({
...
export async function executeNormalizedReadStrategy({
  ...
  for (const strategy of HANDLER_STRATEGIES) {
    if (typeof handlers[strategy] === 'function') {
      normalizedHandlers[strategy] = async () => normalize(await handlers[strategy](), { strategy });
    }
  }
...
export async function executeReadStrategy({ requested = 'auto', promotion, handlers = {}, observer } = {}) {
  const available = Object.fromEntries(Object.entries(handlers).map(([name, handler]) => [name, typeof handler === 'function']));
  const selected = chooseReadStrategy({ requested, promotion, available });
  const order = requested === 'auto' && readPromotionQualified(promotion)
    ? ['official', 'direct', 'private-replay', 'capture', 'dom'].filter((name) => typeof handlers[name] === 'function')
    : [selected];
...
function exactCookieForUrl(cookie, target) {
  ...
  let cookiePath = typeof cookie.path === 'string' && cookie.path.startsWith('/') ? cookie.path : '/';
  if (cookie.url) {
    let source;
    try {
    ...
    if (String(cookie.domain).replace(/^\./, '').toLowerCase() !== target.hostname.toLowerCase()) return null;
  } else {
    return null;
  }
  const pathMatches = target.pathname === cookiePath
    || (
...
function normalizeHeaders({ staticHeaders = {}, copiedHeaders = {}, allowedCopiedHeaders = [] } = {}) {
...
function inspectStructure(value, { maxRecords, maxDepth }) {
  ...
  while (stack.length > 0) {
    ...
    if (current.depth > maxDepth) fail('response-depth-exceeded', 'JSON response exceeded structural depth', { dispatched: true });
    if (typeof current.value !== 'object' || current.value === null) continue;
    if (seen.has(current.value)) fail('response-cycle', 'JSON response contains a cycle', { dispatched: true });
    seen.add(current.value);
    records += Array.isArray(current.value) ? current.value.length : 1;
    if (records > maxRecords) fail('response-record-cap', 'JSON response exceeded record cap', { dispatched: true });
...
async function disposeWithPrimary(primary, disposers) {
  ...
  if (primary && errors.length > 0) throw new AggregateError([primary, ...errors], 'network request and cleanup failed');
...
export async function isolatedJsonGet({
  ...
  if (target.protocol !== 'https:') fail('origin-not-https', 'Isolated GET requires HTTPS', { phase: 'pre-dispatch' });
  ...
  try {
    ...
    if (!context || typeof context.get !== 'function') throw new TypeError('requestFactory must return an APIRequestContext-like object');
    ...
    if (new URL(finalUrl).origin !== target.origin) fail('redirect-disallowed', 'Isolated GET changed origin', { phase, dispatched: true });
    ...
    if (!Number.isInteger(status) || status < 200 || status > 299 || status === 204 || status === 205) {
      fail(status === 401 || status === 403 ? 'network-auth' : status === 429 ? 'network-rate-limit' : 'network-http-status', `Isolated GET failed with status ${status}`, { phase, dispatched: true });
    }
    ...
    if (advisedLength != null && advisedLength > maxBytes) fail('response-size-advisory', 'Response Content-Length exceeded cap', { phase, dispatched: true });
    if (typeof response.body !== 'function') fail('response-body-unavailable', 'Response body is unavailable', { phase, dispatched: true });
    const body = await response.body();
    const bytes = Buffer.isBuffer(body) ? body : Buffer.from(body);
    if (bytes.length > maxBytes) fail('response-size-cap', 'Buffered response exceeded cap', { phase, dispatched: true });
    let text;
    try {
      text = new TextDecoder('utf-8', { fatal: true }).decode(bytes);
    } catch (error) {
      fail('response-utf8-invalid', 'Response was not strict UTF-8', { phase, dispatched: true, cause: error });
    }
    let parsed;
    try {
      parsed = JSON.parse(text);
    } catch (error) {
      fail('response-json-invalid', 'Response was not valid JSON', { phase, dispatched: true, cause: error });
    }
    inspectStructure(parsed, { maxRecords, maxDepth });
    output = await normalize(parsed);
  } catch (error) {
    primary = error instanceof NetworkReadError
      ? error
      : new NetworkReadError('network-request-failed', error?.message || 'Network request failed', {
        phase,
...
function normalizeSchemaMap(value, label, { header = false } = {}) {
  ...
  for (const [rawKey, rule] of Object.entries(value)) {
    ...
    if (Object.hasOwn(output, key)) throw new TypeError(`${label} contains duplicate normalized key: ${key}`);
    ...
    if (header && HOP_HEADERS.has(key)) throw new TypeError(`capability header is forbidden: ${key}`);
    const maxLength = requirePositiveInteger(rule.maxLength ?? 2048, `${label}.${key}.maxLength`, 8192);
    let fixed;
    if (mode === 'fixed') {
    ...
    if (output[key].cardinality !== 'one') throw new TypeError(`${label}.${key}.cardinality must be one`);
  }
...
export function validateObservedPrivateCapability(input) {
  ...
  if (!path.startsWith('/') || path.includes('?') || path.includes('#')) throw new TypeError('capability.path must be an exact pathname');
  const limits = {
    ttlMs: requirePositiveInteger(input.limits?.ttlMs ?? MAX_PRIVATE_LEASE_MS, 'capability.limits.ttlMs', MAX_PRIVATE_LEASE_MS),
    maxUses: requirePositiveInteger(input.limits?.maxUses ?? MAX_PRIVATE_LEASE_USES, 'capability.limits.maxUses', MAX_PRIVATE_LEASE_USES),
    maxBytes: requirePositiveInteger(input.limits?.maxBytes ?? DEFAULT_NETWORK_MAX_BYTES, 'capability.limits.maxBytes'),
    maxRecords: requirePositiveInteger(input.limits?.maxRecords ?? DEFAULT_NETWORK_MAX_RECORDS, 'capability.limits.maxRecords')
  };
  if (input.cursorExtractor != null && typeof input.cursorExtractor !== 'function') throw new TypeError('capability.cursorExtractor must be a function');
  ...
  const cursorKeys = Object.entries(query).filter(([, rule]) => rule.mode === 'cursor').map(([key]) => key);
  if (cursorKeys.length > 1) throw new TypeError('capability.query may declare at most one cursor');
  if (input.cursorExtractor && cursorKeys.length !== 1) throw new TypeError('cursorExtractor requires exactly one cursor query rule');
...
function qualifyQuery(url, schema, supplied = {}) {
  ...
  for (const key of url.searchParams.keys()) {
    if (!declared.has(key)) fail('private-query-undeclared', `Observed query key is undeclared: ${key}`, { phase: 'qualification' });
  }
  const output = new URLSearchParams();
  for (const [key, rule] of Object.entries(schema)) {
    const observed = valuesFor(url, key);
    if (observed.length > 1) fail('private-query-cardinality', `Observed query key is repeated: ${key}`, { phase: 'qualification' });
    const suppliedPresent = Object.hasOwn(supplied, key);
    if (suppliedPresent && rule.mode === 'fixed') {
      fail('private-query-fixed-supplied', `Fixed query key cannot be synthesized: ${key}`, { phase: 'pre-dispatch' });
    }
    const value = suppliedPresent ? supplied[key] : observed[0];
    if (rule.required && (observed[0] == null || observed[0] === '')) {
      fail('private-query-required', `Required observed query key is missing: ${key}`, { phase: 'qualification' });
    }
    if (rule.mode === 'fixed' && value != null && String(value) !== rule.fixed) {
      fail('private-query-fixed-mismatch', `Fixed query mismatch: ${key}`, { phase: 'qualification' });
    }
    if (value == null || value === '') continue;
    const text = String(value);
    if (text.length > rule.maxLength) fail('private-query-length', `Query value exceeded cap: ${key}`, { phase: 'qualification' });
    output.set(key, text);
  }
  for (const key of Object.keys(supplied)) {
    if (!declared.has(key)) fail('private-query-undeclared', `Supplied query key is undeclared: ${key}`, { phase: 'pre-dispatch' });
  }
  return output;
}

function qualifyHeaders(observed, schema) {
  const source = Object.fromEntries(Object.entries(observed || {}).map(([key, value]) => [key.toLowerCase(), value]));
  const output = Object.create(null);
  for (const [key, rule] of Object.entries(schema)) {
    const value = source[key];
    if (rule.required && (value == null || value === '')) fail('private-header-required', `Required observed header is missing: ${key}`, { phase: 'qualification' });
    if (value == null || value === '') continue;
    const text = String(value);
    if (text.length > rule.maxLength) fail('private-header-length', `Observed header exceeded cap: ${key}`, { phase: 'qualification' });
    if (rule.mode === 'fixed' && text !== rule.fixed) {
      fail('private-header-fixed-mismatch', `Fixed header mismatch: ${key}`, { phase: 'qualification' });
    }
...
export function qualifyObservedPrivateRequest({ capability, method, url, headers, pageOne = false }) {
  const validated = validateObservedPrivateCapability(capability);
  if (String(method).toUpperCase() !== 'GET') fail('private-method-disallowed', 'Observed private capability accepts GET only', { phase: 'qualification' });
  ...
  return Object.freeze({ capability: validated, url: `${validated.origin}${validated.path}?${query}`.replace(/\?$/, ''), copiedHeaders });
}

function bindingMatches(expected, actual) {
  return expected.session === actual?.session
...
function validateBinding(binding, capability) {
  if (!isObject(binding) || !binding.session || !binding.page || !binding.context) throw new TypeError('replay binding requires session, page, and context objects');
  ...
  if (normalized.origin !== capability.origin || normalized.provider !== capability.provider || normalized.action !== capability.action) {
...
export async function withObservedPrivateReplay({
  ...
  if (typeof fn !== 'function') throw new TypeError('withObservedPrivateReplay callback is required');
  const validated = validateObservedPrivateCapability(capability);
  ...
  const client = Object.freeze({
    ...
    async replay({ binding: requestBinding = binding, query = {} } = {}) {
      if (!lease.active || !PRIVATE_LEASES.get(bound.page)?.has(lease)) fail('private-lease-inactive', 'Observed-private lease is inactive', { phase: 'pre-dispatch' });
      if (!bindingMatches(lease.binding, requestBinding)) fail('private-lease-binding', 'Observed-private lease binding mismatch', { phase: 'pre-dispatch' });
      const current = now();
      if (
      ...
      if (lease.uses >= validated.limits.maxUses) fail('private-lease-exhausted', 'Observed-private lease use cap reached', { phase: 'pre-dispatch' });
...
export async function captureObservedPrivateResponse({
  ...
  const validated = validateObservedPrivateCapability(capability);
  if (!page || typeof page.on !== 'function' || typeof page.off !== 'function') throw new TypeError('page event API is required');
  if (!session || typeof session.navigateGuardedForRead !== 'function') throw new TypeError('strict provider session is required');
  ...
  const captureFailure = (error) => {
    ...
    return new NetworkReadError('private-capture-qualification', error?.message || 'Private request qualification failed', {
      phase: 'capture',
      dispatched: true,
      fallbackEligible: false,
      cause: error
  ...
  const onRequest = (request) => {
    ...
    try {
      ...
      if (String(method).toUpperCase() === 'GET' && parsed.origin === validated.origin && parsed.pathname === validated.path) {
        matchingRequests += 1;
      }
    } catch {}
  ...
  const onResponse = (response) => {
    void (async () => {
      ...
      const status = typeof response.status === 'function' ? response.status() : null;
      if (!Number.isInteger(status) || status < 200 || status > 299 || status === 204 || status === 205) {
        fail('private-capture-status', `Captured private response failed with status ${status}`, { phase: 'capture', dispatched: true });
      }
      const body = typeof response.body === 'function' ? await response.body() : null;
      if (!body) fail('private-capture-body', 'Captured private response body is unavailable', { phase: 'capture', dispatched: true });
      const bytes = Buffer.isBuffer(body) ? body : Buffer.from(body);
      if (bytes.length > validated.limits.maxBytes) fail('private-capture-size', 'Captured private response exceeded size cap', { phase: 'capture', dispatched: true });
      let data;
      try {
        data = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes));
      } catch (error) {
        fail('private-capture-json', 'Captured private response was not strict JSON', { phase: 'capture', dispatched: true, cause: error });
      }
      inspectStructure(data, { maxRecords: validated.limits.maxRecords, maxDepth: DEFAULT_NETWORK_MAX_DEPTH });
  ...
  try {
    ...
    while (!terminalError && matchingRequests <= 1 && qualifiedRequests <= 1 && completed.length <= 1) {
      const now = Date.now();
    ...
    if (matchingRequests > 1 || qualifiedRequests > 1 || completed.length > 1) {
      fail('private-capture-ambiguous', 'More than one matching private request or response was observed', {
        phase: 'capture',
        dispatched: true
      });
    }
...
export function extractBoundedCursor(capability, payload) {
  ...
  if (text.length > cursorRule.maxLength) fail('private-cursor-length', 'Extracted cursor exceeded cap', { phase: 'normalization', dispatched: true });
  return text;
}
