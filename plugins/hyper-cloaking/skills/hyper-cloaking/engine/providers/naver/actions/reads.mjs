...
function normalizeSearchResults(value, { kind, query, limit, refNormalizer }) {
  if (!value || typeof value !== 'object' || !Array.isArray(value.results) || value.results.length > MAX_RAW) {
  ...
  for (const entry of value.results) {
    ...
    if (refNormalizer) {
      ...
      if (!ref) throw new TypeError(`Naver ${kind} search entry contains an invalid owned-origin reference`);
      url = ref.url;
    } else {
      try {
  ...
  if (results.length === 0 && value.emptyState !== true) {
    throw new TypeError(`Naver empty ${kind} search content requires explicit empty-state evidence`);
  }
...
async function extractSearchLinks(session, selector) {
  return session.page.$$eval(selector, (nodes) => nodes.slice(0, 400).map((node) => ({
    ...
    snippet: node.closest('li, div')?.querySelector('.dsc, .api_txt_lines:not(.total_tit)')?.textContent?.trim() || null
  })));
}
...
async function search(session, { kind, query, url, selectors, limit, refNormalizer }, opts) {
  ...
  const { value } = await readOptions(opts, `search${kind[0].toUpperCase()}${kind.slice(1)}`, dom, (content) => normalizeSearchResults(content, { kind, query, limit, refNormalizer }));
...
export async function searchWeb(session, query, opts = {}) {
  ...
  return search(session, {
    kind: 'web', query: searchQuery, url, selectors: naverSelectors.search.web, limit: limitFor(opts.limit)
  }, opts);
}
...
export async function searchBlog(session, query, opts = {}) {
  ...
  return search(session, {
    kind: 'blog', query: searchQuery, url, selectors: naverSelectors.search.blog, limit: limitFor(opts.limit),
...
export async function searchCafe(session, query, opts = {}) {
  ...
  return search(session, {
    kind: 'cafe', query: searchQuery, url, selectors: naverSelectors.search.cafe, limit: limitFor(opts.limit),
    refNormalizer: normalizeCafePostRef
  }, opts);
}

function normalizePostContent(value, { post, kind, includeComments, limit }) {
  if (!value || typeof value !== 'object' || !Array.isArray(value.comments) || value.comments.length > MAX_RAW) {
  ...
  const comments = includeComments
    ? value.comments.slice(0, limit).map((comment) => boundedText(comment, `${kind} comment`, 10_000)).filter(Boolean)
    : [];
  return {
    ...post,
...
export async function getBlogPost(session, blogPostRef, opts = {}) {
  ...
  const dom = async () => {
    ...
    if (includeComments) {
      comments = await session.page.$$eval(naverSelectors.blog.commentText, (nodes) => nodes.slice(0, 400).map((node) => node.textContent?.trim()).filter(Boolean));
      commentsEmptyState = comments.length === 0
        && await session.page.locator(naverSelectors.blog.commentEmptyState).count() > 0;
      if (comments.length === 0 && !commentsEmptyState) throw new Error('Naver blog comment state could not be proven');
    }
    return { title, body, author, timestamp, commentCount, comments, commentsEmptyState, present: true };
  };
  const { value } = await readOptions(opts, 'getBlogPost', dom, (content) => normalizePostContent(content, { post, kind: 'blog', includeComments, limit }));
...
export async function getCafePost(session, cafePostRef, opts = {}) {
  ...
  const dom = async () => {
    ...
    let commentsEmptyState = !includeComments;
    if (includeComments) {
      comments = await session.page.$$eval(naverSelectors.cafe.commentText, (nodes) => nodes.slice(0, 400).map((node) => node.textContent?.trim()).filter(Boolean));
      commentsEmptyState = comments.length === 0
        && await session.page.locator(naverSelectors.cafe.commentEmptyState).count() > 0;
      if (comments.length === 0 && !commentsEmptyState) throw new Error('Naver cafe comment state could not be proven');
    }
    return { title, body, author, timestamp, commentCount: null, comments, commentsEmptyState, present: true };
  };
  const { value } = await readOptions(opts, 'getCafePost', dom, (content) => normalizePostContent(content, { post, kind: 'cafe', includeComments, limit }));
...
function normalizePostList(value, { owner, ownerKey, refNormalizer, limit }) {
  if (!value || typeof value !== 'object' || !Array.isArray(value.posts) || value.posts.length > MAX_RAW) {
    throw new TypeError(`Naver ${ownerKey} list content must contain at most ${MAX_RAW} posts`);
  }
  const seen = new Set();
  const posts = [];
  for (const entry of value.posts) {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)) throw new TypeError(`Naver ${ownerKey} list entries must be objects`);
    const ref = refNormalizer(entry);
    if (!ref || ref[ownerKey] !== owner[ownerKey]) throw new TypeError(`Naver ${ownerKey} list entry does not belong to the requested owner`);
    const key = ref.url;
  ...
  if (posts.length === 0 && value.emptyState !== true) {
    throw new TypeError(`Naver empty ${ownerKey} list content requires explicit empty-state evidence`);
  }
...
export async function getBlogList(session, blogRef, opts = {}) {
  ...
  const dom = async () => {
    await session.navigateGuardedForRead(blog.url, { waitUntil: 'domcontentloaded', timeout: 30_000 });
    const posts = await extractListLinks(session, naverSelectors.blog.listItem);
    if (posts.length === 0 && await session.page.locator(naverSelectors.blog.listEmptyState).count() === 0) {
      throw new Error('Naver blog list state could not be proven');
    }
    return { posts, emptyState: posts.length === 0 };
  };
  const { value } = await readOptions(opts, 'getBlogList', dom, (content) => normalizePostList(content, { owner: blog, ownerKey: 'blogId', refNormalizer: normalizeBlogPostRef, limit }));
...
export async function getCafeList(session, cafeRef, opts = {}) {
  ...
  const limit = limitFor(opts.limit);
  const dom = async () => {
    await session.navigateGuardedForRead(cafe.url, { waitUntil: 'domcontentloaded', timeout: 30_000 });
    const posts = await extractListLinks(session, naverSelectors.cafe.listItem);
    if (posts.length === 0 && await session.page.locator(naverSelectors.cafe.listEmptyState).count() === 0) {
      throw new Error('Naver cafe list state could not be proven');
    }
    return { posts, emptyState: posts.length === 0 };
  };
  const { value } = await readOptions(opts, 'getCafeList', dom, (content) => normalizePostList(content, { owner: cafe, ownerKey: 'cafeId', refNormalizer: normalizeCafePostRef, limit }));
