...
import { getVideo } from './video.mjs';
import {
  youtubeSelectors,
...
function pageFor({
  ...
  return {
    ...
    locator(selector) {
      return { async count() { return selectorCounts[selector] ?? 1; } };
    },
    async evaluate() {
      evaluateCount += 1;
    ...
    async $$eval(_selector, callback, arg) {
      ...
      return callback(links.map((link) => ({
...
test('getChannel navigates to videos and emits analyzer-compatible records', async () => {
  const page = pageFor({
    channelNodes: [
      channelLinkNode({ href: `/watch?v=${VIDEO_A}`, title: 'First', publishedAt: '2026-01-01T00:00:00Z', viewCount: '1.2K views', likeCount: '34 likes', tags: ['Build'] }),
      channelLinkNode({ href: `/watch?v=${VIDEO_B}`, title: 'Second', publishedAt: '2026-01-08T00:00:00Z', viewCount: '900 views', likeCount: '20 likes', tags: ['Build', 'Testing'] })
    ]
  });
...
test('getVideo parses visible counts and returns analyzer fields', async () => {
  const page = pageFor({
    ...
    metadata: { publishedAt: '2026-01-02T00:00:00Z', timestamp: Date.parse('2026-01-02T00:00:00Z'), tags: ['Fixture'] }
  });
...
test('resolveYouTubeSelector distinguishes primary, fallback, and exhaustion', async () => {
  const countsFor = (counts) => ({
    locator(selector) {
      return { async count() { return counts[selector] ?? 0; } };
    }
  });
...
test('searchVideos rejects selector exhaustion instead of returning an empty result', async () => {
  await assert.rejects(searchVideos(sessionFor(pageFor({
    selectorCounts: {
  ...
  })), 'fixture'), /YouTube selector extraction failed/);
});
...
test('getChannel accepts an empty video list only with an explicit empty-state anchor', async () => {
  const read = await getChannel(sessionFor(pageFor({
    selectorCounts: {
...
test('searchVideos preserves its extraction callback argument', async () => {
  let extraction = null;
  await searchVideos(sessionFor(pageFor({
    onExtraction(value) { extraction = value; },
...
test('resolveYouTubeExtractionTier prefers primary and rejects exhaustion', async () => {
  ...
  assert.equal(
    await resolveYouTubeExtractionTier(countsFor({
