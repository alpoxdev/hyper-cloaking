...
function live(overrides = {}) {
  return {
    ...
    publicNavigation: { finalUrl: 'https://example.com/', documentUrls: ['about:blank', 'https://example.com/'], violations: [] },
...
test('cleanup failure is a hard blocker', async () => {
  const result = await runBrowserTask(input, { runLiveVerification: async () => live({ cleanup: { ok: false, closed: false, timedOut: true, blocker: 'close timeout' } }) });
...
test('only explicit verified telemetry can produce success', async () => {
  const result = await runBrowserTask(input, { runLiveVerification: async () => live({ ok: true, humanization: { ok: true, configured: true, evidence: 'runtime telemetry id 1', blocker: null }, blockers: [] }) });
...
test('rejects actions, duplicate origins, invalid redirects and non-http targets', async () => {
  for (const candidate of [
    ...
    const result = await runBrowserTask(candidate, { runLiveVerification: async () => { throw new Error('must not run'); } });
    assert.equal(result.status, 'blocked');
...
test('maps staging evidence to relative refs', async () => {
  const result = await runBrowserTask(input, { runLiveVerification: async () => live({ evidenceRefs: [{ path: '/tmp/hyper-stage/live.png', kind: 'live-browser-screenshot' }] }) });
  assert.deepEqual(result.result.evidenceRefs, [{ type: 'screenshot', relPath: 'live.png', description: 'live-browser-screenshot' }]);
});
