...
function hasNamedEntry(collection, expected) {
  ...
  if (collection && typeof collection === 'object') {
    return names.every((name) =>
      Boolean(
        collection[name] ||
        Object.values(collection).some((entry) => {
          if (typeof entry === 'string') return entry === name;
          return entry?.path === name || entry?.name === name || entry?.id === name;
        })
      )
...
function selectorIsVisible(selectors, selector) {
  ...
  if (record && typeof record === 'object')
    return Boolean(record.visible ?? record.exists ?? record.found);
  if (Array.isArray(selectors)) {
    return selectors.some(
      (entry) =>
        entry === selector ||
        (entry?.selector === selector && (entry.visible ?? entry.exists ?? entry.found))
    );
  }
  return false;
}
...
function recordCount(observation, criterion) {
  ...
  if (key && observation.records && Array.isArray(observation.records[key]))
    return observation.records[key].length;
...
function evaluateSingle(observation, criterion) {
  ...
  switch (type) {
    case 'urlLoaded':
      return Boolean(
        observation.urlLoaded ?? observation.loaded ?? observation.pageLoaded ?? observation.url
      );
    case 'urlIncludes':
    ...
    case 'selectorVisible':
      return selectorIsVisible(
        observation.selectors ?? observation.selectorVisibility,
        String(expected ?? '')
      );
    case 'fileExists':
    ...
    case 'recordCountAtLeast':
      return (
        recordCount(observation, criterion) >=
        Number(criterion.count ?? criterion.atLeast ?? expected ?? 0)
      );
    case 'evidenceCaptured':
      return (
        Boolean(observation.evidenceCaptured) ||
        asArray(observation.evidenceRefs).length > 0 ||
        hasNamedEntry(observation.artifacts, expected)
      );
    case 'negativeAssertion': {
      const inner =
        ...
        criterion.criterion ||
        Object.fromEntries(
          Object.entries(criterion).filter(
            ([key]) => !['type', 'id', 'name', 'expected', 'value'].includes(key)
          )
        );
      const normalized = inner.type
        ? inner
        : Object.entries(inner).find(([key]) => CRITERION_TYPES.has(key))
          ? (() => {
              const [innerType, innerExpected] = Object.entries(inner).find(([key]) =>
                CRITERION_TYPES.has(key)
              );
...
function explicitPageLoadCriterion(criterion) {
  return (
    criterion.requiresPageOpen === true ||
...
export function evaluateOutcome(observation = {}, criteria = []) {
  ...
  const pageLoadOnlySuccess =
    rawPassed &&
    criteriaResults.length > 0 &&
    criteriaResults.every((result) => result.type === 'urlLoaded');
  const pageLoadOnlyJustified =
    !pageLoadOnlySuccess || criteriaResults.some((result) => result.requiresPageOpen);
  ...
  return {
    ...
    failedCriteria:
      pageLoadOnlySuccess && !pageLoadOnlyJustified
        ? [
            ...failedCriteria,
            {
              id: 'page-load-only',
              type: 'pageLoadOnlyJustification',
              passed: false,
              expected: 'explicit page-open-only justification'
            }
          ]
        : failedCriteria,
...
export function makeOutcomeReport({
  targetSafety,
  outcome,
  failure,
  contentBoundary,
  ...
  const normalizedOutcome =
    outcome && typeof outcome === 'object' ? outcome : { passed: Boolean(outcome), criteria: [] };
  ...
  const passedCriteria = asArray(
    normalizedOutcome.passedCriteria ?? normalizedOutcome.satisfiedCriteria
  );
  const ok = Boolean(normalizedOutcome.ok ?? normalizedOutcome.passed ?? normalizedOutcome.success);
  const pageLoadOnlySuccess =
    Boolean(normalizedOutcome.pageLoadOnlySuccess) ||
    (ok === true &&
      criteria.length > 0 &&
      criteria.every((criterion) => criterion.type === 'urlLoaded'));
  const pageLoadOnlyJustified =
    !pageLoadOnlySuccess ||
    criteria.some(
      (criterion) =>
  ...
  return {
    targetSafety: targetSafety ?? {
      disposition: 'blocker',
      reason: 'target-safety-not-provided',
      risks: []
    },
    outcome: {
      ...normalizedOutcome,
      ...
      passed:
        passedCriteria.length > 0
          ? passedCriteria
          : criteria.filter((criterion) => criterion.passed === true),
      failed:
        failed.length > 0 ? failed : criteria.filter((criterion) => criterion.passed === false),
      passedBoolean: ok,
    ...
    learning: learning ?? {
      ...
      applied: false,
      written: false,
      reason: 'self-learning-disabled-by-default'
    }
  };
}
