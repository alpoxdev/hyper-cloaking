...
export function resolveWriteGate(opts = {}) {
  ...
  if (dryRun) {
    return {
      dryRun: true,
      allowed: false,
      reason: 'dry-run: no write performed (pass dryRun:false to act)'
    };
  }
...
export function resolveConfirmationGate(opts = {}) {
  ...
  if (!interactive) {
    return {
      allowed: false,
      blocked: true,
      reason: 'blocked: bulk write confirmation cannot be satisfied non-interactively'
    };
  }
  if (!confirmed) {
...
function validateRateStore(store) {
  ...
  for (const [actionType, events] of Object.entries(store)) {
    if (!Array.isArray(events) || events.some((timestamp) => !Number.isFinite(timestamp))) {
      throw new TypeError(
        `invalid action-rate state for "${actionType}": expected finite timestamps`
      );
    }
  }
    ...
    try {
      ...
      if (cleanupError?.code !== 'ENOENT') {
        throw new AggregateError(
          [error, cleanupError],
          'stale action lock verification and cleanup failed'
        );
      }
    }
    throw error;
    ...
    try {
      ...
      try {
        ...
        if (cleanupError?.code !== 'ENOENT') {
          throw new AggregateError(
            [error, cleanupError],
            'action lock candidate initialization and cleanup failed'
          );
        }
      }
      throw error;
    ...
    try {
      ...
      try {
        ...
        if (cleanupError?.code !== 'ENOENT') {
          throw new AggregateError(
            [error, cleanupError],
            'action lock contention and candidate cleanup failed'
          );
        }
      }
  ...
  try {
    ...
    if (operationError) {
      throw new AggregateError(
        ...
        'action-rate operation and lock cleanup failed'
      );
    }
    throw releaseError;
  }
...
function validateGuardedStore(store) {
  if (
    ...
    typeof store !== 'object' ||
    Array.isArray(store) ||
    store.version !== GUARDED_STORE_VERSION
  ) {
 ...
 */
export async function reserveGuardedAction(
  stateDir,
  {
    actionType,
...
export async function finalizeGuardedAction(
  stateDir,
  { idempotencyHash, state, evidenceIdHash = null, now = Date.now() } = {}
) {
  requireHash(idempotencyHash, 'idempotencyHash');
 ...
 */
export async function reconcileGuardedAction(
  stateDir,
  {
    idempotencyHash,
    resolution,
  ...
  if (!interactive || !confirmed)
    throw new Error('guarded-action reconciliation requires interactive confirmation');
  if (!['verified', 'clear'].includes(resolution))
    throw new TypeError('resolution must be verified or clear');
