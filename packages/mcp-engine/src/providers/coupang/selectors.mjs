...
export const coupangSelectors = {
  search: {
    ...
    emptyState:
      'main [role="status"]:has-text("검색 결과가 없습니다"), main [role="status"]:has-text("No results")'
  },
  product: {
    ...
    reviewEmptyState:
      'main [role="status"]:has-text("상품평이 없습니다"), main [role="status"]:has-text("No reviews")',
    ...
    unsave:
      'main button[aria-pressed="true"][aria-label*="찜"], main button[aria-pressed="true"][aria-label*="Save"]'
  },
  cart: {
    ...
    emptyState:
      'main [role="status"]:has-text("장바구니가 비어"), main [role="status"]:has-text("cart is empty")'
  },
  review: {
