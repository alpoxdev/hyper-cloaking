...
export function normalizeProductRef(ref) {
  const suppliedId =
    ref && typeof ref === 'object' && ref.productId != null ? String(ref.productId) : null;
  const rawUrl =
  ...
  if (!productId || !PRODUCT_ID_RE.test(productId) || (suppliedId && urlId && suppliedId !== urlId))
    return null;
...
export function normalizeOrderItemRef(ref) {
  ...
  const url = ownedUrl(ref.url);
  if (
    !OPAQUE_ID_RE.test(orderItemId) ||
    !product ||
    !url ||
    !/^\/my-orders(?:\/|$)/.test(url.pathname)
  )
    return null;
  return {
    orderItemId,
