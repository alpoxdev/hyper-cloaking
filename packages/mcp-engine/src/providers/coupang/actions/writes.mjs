...
function validateLiveWrite(session, action, opts, enableKey) {
  ...
  if (typeof opts.runId !== 'string' || !SAFE_RUN_ID_RE.test(opts.runId)) {
    return policyBlock(
      action,
      'a safe explicit runId is required for real writes',
      'run-id-required'
    );
  }
  return null;
...
async function cartLineState(session, productId) {
  await session.navigateGuardedForWrite(coupangSelectors.cart.url, {
  ...
  if ((await quantityLocator.count()) !== 1)
    throw new Error('Coupang cart quantity identity is ambiguous');
  const quantityText = await quantityLocator.first().inputValue();
  const quantity = Number(quantityText);
  if (!cartLineId || !Number.isInteger(quantity) || quantity <= 0)
    throw new Error('Coupang cart line state is malformed');
...
export async function addToCart(session, productRef, quantity = 1, opts = {}) {
  ...
  let product;
  try {
    product = assertProductRef(productRef);
  } catch (error) {
    return invalidRef(action, error, 'product-ref-validation');
  }
  if (!Number.isInteger(quantity) || quantity < 1 || quantity > 99)
    return policyBlock(action, 'cart quantity must be an integer from 1 to 99', 'input-validation');
  ...
  if (existing?.quantity === quantity) {
    return successfulResult(
      session,
      action,
      'cart already has desired product quantity',
      coupangSelectors.cart.url,
      null,
  ...
  if (existing)
    return policyBlock(
      action,
      'cart line exists with a different quantity; use setCartQuantity',
      'quantity-routing'
    );
  if (quantity !== 1)
    return policyBlock(
      action,
      'non-atomic add quantity is unsupported; add one then use setCartQuantity',
      'quantity-routing'
    );
  await session.navigateGuardedForWrite(product.url, {
    waitUntil: 'domcontentloaded',
    timeout: 30_000
  });
  const addControl = session.page.locator(coupangSelectors.product.addToCart);
  if ((await addControl.count()) !== 1)
    return policyBlock(action, 'owned add-to-cart control is not unique', 'selector-ownership');
  return guardedDispatch(session, {
    action,
    rateKey: 'coupang-cart',
    maxPerWindow: 20,
    target: product.url,
    content: { productId: product.productId, quantity },
    runId: opts.runId,
    dispatch: () => session.humanClick(addControl.first()),
    verify: () =>
      waitForState(
        session.page,
        async () => (await cartLineState(session, product.productId))?.quantity === 1
      ),
...
export async function setCartQuantity(session, cartLineRef, quantity, opts = {}) {
  ...
  let line;
  try {
    line = assertCartLineRef(cartLineRef);
  } catch (error) {
    return invalidRef(action, error, 'cart-line-ref-validation');
  }
  if (!Number.isInteger(quantity) || quantity < 1 || quantity > 99)
    return policyBlock(action, 'cart quantity must be an integer from 1 to 99', 'input-validation');
  ...
  if (!existing || existing.cartLineId !== line.cartLineId)
    return policyBlock(action, 'owned cart line could not be proven', 'cart-line-state');
  if (existing.quantity === quantity)
    return successfulResult(session, action, 'cart quantity already satisfied', line.url, null, {
      alreadySatisfied: true
    });
  ...
  if (steps !== 1)
    return policyBlock(
      action,
      'multi-step quantity mutation is non-atomic; request one-unit transitions',
      'non-atomic-quantity'
    );
  ...
  if ((await quantityControl.count()) !== 1)
    return policyBlock(action, 'owned cart quantity control is not unique', 'selector-ownership');
  return guardedDispatch(session, {
    action,
    rateKey: 'coupang-cart',
    maxPerWindow: 20,
    target: line.url,
    content: { cartLineId: line.cartLineId, quantity },
    runId: opts.runId,
    dispatch: () => session.humanClick(quantityControl.first()),
    verify: () =>
      waitForState(
        session.page,
        async () => (await cartLineState(session, line.productId))?.quantity === quantity
      ),
...
export async function removeCartItem(session, cartLineRef, opts = {}) {
  ...
  const existing = await cartLineState(session, line.productId);
  if (!existing)
    return successfulResult(session, action, 'cart item already absent', line.url, null, {
      alreadySatisfied: true
    });
  if (existing.cartLineId !== line.cartLineId)
    return policyBlock(action, 'owned cart line identity mismatch', 'cart-line-state');
  const root = session.page.locator(`main [data-cart-item-id="${line.cartLineId}"]`).first();
  const removeControl = root.locator(coupangSelectors.cart.remove);
  if ((await removeControl.count()) !== 1)
    return policyBlock(action, 'owned cart remove control is not unique', 'selector-ownership');
  return guardedDispatch(session, {
    action,
    rateKey: 'coupang-cart',
    maxPerWindow: 20,
    target: line.url,
    content: { cartLineId: line.cartLineId, absent: true },
    runId: opts.runId,
    dispatch: () => session.humanClick(removeControl.first()),
    verify: () =>
      waitForState(
        session.page,
        async () => (await cartLineState(session, line.productId)) === null
      ),
...
export async function setSavedState(session, productRef, saved, opts = {}) {
  ...
  if (typeof saved !== 'boolean')
    return policyBlock(action, 'saved must be boolean', 'input-validation');
  ...
  const isSaved = async () =>
    count(session.page, coupangSelectors.product.unsave).then((value) => value === 1);
  if ((await isSaved()) === saved)
    return successfulResult(session, action, 'saved state already satisfied', product.url, null, {
      alreadySatisfied: true
    });
  const savedControl = session.page.locator(
    saved ? coupangSelectors.product.save : coupangSelectors.product.unsave
  );
  if ((await savedControl.count()) !== 1)
    return policyBlock(action, 'owned saved-state control is not unique', 'selector-ownership');
  return guardedDispatch(session, {
    action,
    rateKey: 'coupang-save',
    maxPerWindow: 20,
    target: product.url,
    content: { productId: product.productId, saved },
    runId: opts.runId,
...
export async function submitOwnOrderReview(session, orderItemRef, review, opts = {}) {
  ...
  let item;
  try {
    item = assertOrderItemRef(orderItemRef);
  } catch (error) {
    return invalidRef(action, error, 'order-item-ref-validation');
  }
  ...
  if (!text || text.length > 5_000 || !Number.isInteger(rating) || rating < 1 || rating > 5) {
    return policyBlock(
      action,
      'review requires text up to 5000 characters and an integer rating from 1 to 5',
      'input-validation'
    );
  }
  const blocked = validateLiveWrite(session, action, opts, 'enableSubmitOwnOrderReview');
  if (blocked) return blocked;
  const confirm = resolveConfirmationGate({
    interactive: session.interactive,
    confirmed: opts.confirmed
  });
  if (!confirm.allowed) return policyBlock(action, confirm.reason, 'confirmation-gate');
  await session.navigateGuardedForWrite(item.url, {
    waitUntil: 'domcontentloaded',
    timeout: 30_000
  });
  const eligible = session.page.locator(
    `${coupangSelectors.review.eligible}[data-order-item-id="${item.orderItemId}"]`
  );
  if ((await eligible.count()) !== 1)
    return policyBlock(
      action,
      'eligible own-order review target could not be proven',
      'review-eligibility'
    );
  const reviewSelector = `[data-review-order-item-id="${item.orderItemId}"][data-review-id]`;
  const beforeReviewIds = new Set(
    await session.page
      .locator(reviewSelector)
      .evaluateAll((nodes) =>
        nodes.map((node) => node.getAttribute('data-review-id')).filter(Boolean)
      )
  );
  const ratingControl = session.page.locator(
    `${coupangSelectors.review.rating}[data-rating="${rating}"]`
  );
  ...
  if (
    (await ratingControl.count()) !== 1 ||
    (await textControl.count()) !== 1 ||
  ...
  return guardedDispatch(session, {
    action,
    rateKey: 'coupang-review',
    maxPerWindow: 2,
    target: item.url,
    content: { orderItemId: item.orderItemId, rating, text },
    runId: opts.runId,
    dispatch: async () => {
    ...
    verify: () =>
      waitForState(session.page, async () => {
        const reviewIds = await session.page
          .locator(reviewSelector)
          .evaluateAll((nodes) =>
            nodes.map((node) => node.getAttribute('data-review-id')).filter(Boolean)
          );
...
export function blockedCoupangAction(actionName) {
  ...
  if (!FORBIDDEN_ACTIONS.has(name))
    throw new TypeError(`unsupported Coupang structural blocker: ${name}`);
  return policyBlock(
    `coupang:${name}`,
    `${name} is structurally blocked for Coupang automation`,
    'structural-blocker'
  );
}
