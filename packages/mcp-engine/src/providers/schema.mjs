...
export function validateProviderSchema(provider) {
  ...
  if (!isPlainObject(provider)) {
    return {
      ok: false,
      errors: [{ code: 'provider-schema-invalid', message: 'provider must be a plain object' }]
    };
  }
  ...
  if (!isNonEmptyString(provider.label))
    fail('provider-schema-invalid', 'label must be a non-empty string');

  if (!isPlainObject(provider.metadata)) {
    fail('provider-schema-invalid', 'metadata must be an object');
  } else {
    if (!isNonEmptyString(provider.metadata.category))
      fail('provider-schema-invalid', 'metadata.category must be a non-empty string');
    if (!isNonEmptyString(provider.metadata.notes))
      fail('provider-schema-invalid', 'metadata.notes must be a non-empty string');
  }

  if (!isPlainObject(provider.domains)) {
    fail('provider-schema-invalid', 'domains must be an object');
  } else {
    const { primary, aliases, navigationOnlyAliases, allowedOrigins, disallowedOrigins } =
      provider.domains;

    if (!isValidHostname(primary))
      fail('provider-schema-invalid', `domains.primary "${primary}" must be a hostname, not a URL`);
    ...
    if (!isStringArray(aliases)) {
      ...
      for (const alias of aliases) {
        if (!isValidHostname(alias))
          fail('provider-schema-invalid', `domains.aliases contains invalid hostname "${alias}"`);
      }
    }
    ...
    if (!isOptionalStringArray(navigationOnlyAliases)) {
      fail(
        ...
        'domains.navigationOnlyAliases must be an array of strings when present'
      );
      ...
      for (const alias of navigationOnlyAliases) {
        if (!isValidHostname(alias))
          fail(
            'provider-schema-invalid',
            `domains.navigationOnlyAliases contains invalid hostname "${alias}"`
          );
      }
    }
    ...
    if (!isStringArray(allowedOrigins)) {
      ...
      for (const origin of allowedOrigins) {
        if (!isValidOrigin(origin))
          fail(
            'provider-schema-invalid',
            `domains.allowedOrigins contains invalid origin "${origin}"`
          );
      }
    }
    ...
    if (!isStringArray(disallowedOrigins)) {
      ...
      for (const origin of disallowedOrigins) {
        if (!isValidOrigin(origin))
          fail(
            'provider-schema-invalid',
            `domains.disallowedOrigins contains invalid origin "${origin}"`
          );
      }
    }
    ...
    const allDomainStrings = [
      primary,
      ...(isStringArray(aliases) ? aliases : []),
      ...(isStringArray(navigationOnlyAliases) ? navigationOnlyAliases : [])
    ...
    for (const value of normalizedAll) {
      if (seenAll.has(value))
        fail(
          'provider-schema-invalid',
          `domains contains duplicate hostname "${value}" across primary/aliases/navigationOnlyAliases`
        );
      seenAll.add(value);
    }
  }
  ...
  if (!isPlainObject(provider.cookie)) {
    ...
    if (!isNonEmptyString(provider.cookie.siteKey))
      fail('provider-schema-invalid', 'cookie.siteKey must be a non-empty string');
    ...
    if (!isBoolean(provider.cookie.required))
      fail('provider-schema-invalid', 'cookie.required must be a boolean');
  }

  if (!isPlainObject(provider.profile)) {
    fail('provider-schema-invalid', 'profile must be an object');
  } else {
    if (!isNonEmptyString(provider.profile.label))
      fail('provider-schema-invalid', 'profile.label must be a non-empty string');
    if (!isBoolean(provider.profile.persistentRecommended))
      fail('provider-schema-invalid', 'profile.persistentRecommended must be a boolean');
  }

  if (!isPlainObject(provider.preflight)) {
    fail('provider-schema-invalid', 'preflight must be an object');
  } else {
    if (!isBoolean(provider.preflight.headlessDefault))
      fail('provider-schema-invalid', 'preflight.headlessDefault must be a boolean');
    if (!isNonEmptyString(provider.preflight.cookieModeDefault))
      fail('provider-schema-invalid', 'preflight.cookieModeDefault must be a non-empty string');
    if (!isNonEmptyString(provider.preflight.credentialSensitivity))
      fail('provider-schema-invalid', 'preflight.credentialSensitivity must be a non-empty string');
    if (!isBoolean(provider.preflight.allowedOriginsPrompt))
      fail('provider-schema-invalid', 'preflight.allowedOriginsPrompt must be a boolean');
  }

  if (!isStringArray(provider.outcomeHints))
    fail('provider-schema-invalid', 'outcomeHints must be an array of strings');
  if (!isStringArray(provider.safeFlowNotes))
    fail('provider-schema-invalid', 'safeFlowNotes must be an array of strings');

  for (const text of collectWordingTexts(provider)) {
    if (containsBypassInstruction(text)) {
      fail(
        'provider-schema-invalid',
        `wording must be diagnostic-only (stop/report), not a bypass instruction: "${text}"`
      );
    }
  }
