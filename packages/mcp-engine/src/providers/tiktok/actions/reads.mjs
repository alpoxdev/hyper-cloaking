...
function normalizeVideoEntry(entry) {
  if (!entry || typeof entry !== 'object' || Array.isArray(entry))
    throw new TypeError('TikTok video entries must be objects');
  const ref = normalizeVideoRef(entry);
  if (!ref)
    throw new TypeError('TikTok video entry contains an invalid owned-origin video reference');
  const hashtags = Array.isArray(entry.hashtags) ? entry.hashtags : [];
  if (hashtags.length > 100)
    throw new TypeError('TikTok video hashtags must contain at most 100 entries');
  return {
    ...ref,
...
function normalizeVideoList(value, { owner = null, query = null, limit }) {
  ...
  for (const entry of value.videos) {
    ...
    if (owner && video.handle !== owner.handle)
      throw new TypeError('TikTok user video does not belong to the requested user');
...
async function extractVideoLinks(session, selector) {
  return session.page.$$eval(selector, (nodes) =>
    nodes.slice(0, 400).map((node) => ({
      ...
      viewCount:
        node.closest('[data-e2e="user-post-item"]')?.getAttribute('data-view-count') || null
    }))
  );
}
...
export async function getUser(session, userRef, opts = {}) {
  ...
  const { value } = await readOptions(opts, 'getUser', dom, (content) => {
    if (!content || typeof content !== 'object' || Array.isArray(content))
      throw new TypeError('TikTok user content must be an object');
    if (content.present !== true)
      throw new TypeError('TikTok user content requires explicit presence evidence');
    return {
      ...user,
...
export async function getUserVideos(session, userRef, opts = {}) {
  ...
  const dom = async () => {
    ...
    if (
      videos.length === 0 &&
      (await session.page.locator(tiktokSelectors.user.emptyState).count()) === 0
    ) {
      throw new Error('TikTok user video state could not be proven');
    }
    return { videos, emptyState: videos.length === 0 };
  };
  const { value } = await readOptions(opts, 'getUserVideos', dom, (content) =>
    normalizeVideoList(content, { owner: user, limit })
  );
...
export async function getVideo(session, videoRef, opts = {}) {
  ...
  const dom = async () => {
    await session.navigateGuardedForRead(video.url, {
      waitUntil: 'domcontentloaded',
      timeout: 30_000
    });
    ...
    const base = await article.first().evaluate((node) => ({
      description:
        node
          .querySelector('[data-e2e="browse-video-desc"], [data-testid="video-description"]')
    ...
    const comments = includeComments
      ? await session.page.$$eval(tiktokSelectors.video.commentText, (nodes) =>
          nodes
            .slice(0, 400)
  ...
  const { value } = await readOptions(opts, 'getVideo', dom, (content) => {
    if (
      ...
      content.comments.length > MAX_RAW
    ) {
      throw new TypeError(`TikTok video content must contain at most ${MAX_RAW} comments`);
    }
    if (content.present !== true)
      throw new TypeError('TikTok video content requires explicit presence evidence');
    ...
    return {
      ...
      comments: includeComments
        ? content.comments
            .slice(0, limit)
            .map((comment) => boundedText(comment, 'comment', 10_000))
            .filter(Boolean)
        : []
    };
  });
...
export async function searchVideos(session, query, opts = {}) {
  ...
  if (!searchQuery || searchQuery.length > 500)
    throw new TypeError('TikTok search query must contain 1-500 characters');
  const limit = limitFor(opts.limit);
  ...
  const dom = async () => {
    ...
    if (
      videos.length === 0 &&
      (await session.page.locator(tiktokSelectors.search.emptyState).count()) === 0
    ) {
      throw new Error('TikTok search state could not be proven');
    }
    return { videos, emptyState: videos.length === 0 };
  };
  const { value } = await readOptions(opts, 'searchVideos', dom, (content) =>
    normalizeVideoList(content, { query: searchQuery, limit })
  );
...
export async function listDMThreads(session, opts = {}) {
  ...
  const limit = limitFor(opts.limit);
  const dom = async () => {
    await session.navigateGuardedForRead(tiktokSelectors.dm.inboxUrl, {
      waitUntil: 'domcontentloaded',
      timeout: 30_000
    });
    const hrefs = await session.page.$$eval(tiktokSelectors.dm.threadLink, (nodes) =>
      nodes
        .slice(0, 400)
    ...
    if (
      hrefs.length === 0 &&
      (await session.page.locator(tiktokSelectors.dm.emptyInbox).count()) === 0
    ) {
  ...
  const { value } = await readOptions(opts, 'listDMThreads', dom, (content) => {
    ...
    if (threads.length === 0 && content.emptyState !== true)
      throw new TypeError('TikTok empty DM list requires explicit empty-state evidence');
    return { accountId, threads };
  });
  return wrapReadPayload({
    url: tiktokSelectors.dm.inboxUrl,
    kind: 'tiktok-dm-threads',
    content: value
  });
}
...
export async function readDMThread(session, threadRef, opts = {}) {
  ...
  const dom = async () => {
    ...
    const messages = await session.page.$$eval(tiktokSelectors.dm.message, (nodes) =>
      nodes
        .slice(-100)
        .map((node) => ({
    ...
    if (
      messages.length === 0 &&
      (await session.page.locator(tiktokSelectors.dm.emptyThread).count()) === 0
    ) {
  ...
  const { value } = await readOptions(opts, 'readDMThread', dom, (content) => {
    ...
    if (content.messages.length === 0 && content.emptyState !== true)
      throw new TypeError('TikTok empty DM thread requires explicit empty-state evidence');
