...
import {
  ...
  makeBlockedResult,
  wrapReadPayload
} from '../../../action-runtime/action-result.mjs';
...
export class InvalidThreadRefError extends Error {
  constructor(ref) {
    super(
      `Invalid thread reference: replies are only allowed to existing threads (/direct/t/<id>), got ${JSON.stringify(ref)}`
    );
...
export function normalizeThreadRef(ref) {
  if (ref && typeof ref === 'object') {
    ...
    return id === null ? null : { threadId: id, url: `https://www.instagram.com/direct/t/${id}/` };
  }
...
function normalizeThreadList(value, limit) {
  ...
  for (const entry of value.threads) {
    ...
    if (!normalized || seen.has(normalized.threadId)) {
      if (!normalized)
        throw new TypeError('Instagram DM thread content contains an invalid thread');
      continue;
    }
...
function normalizeThreadContent(value, { thread, limit }) {
  ...
  if (value.messages.length === 0 && value.emptyState !== true) {
    throw new TypeError(
      'Instagram empty DM message content requires explicit empty-state evidence'
    );
  }
  const messages = value.messages
    .map((entry) => {
      ...
      if (!text || text.length > 10_000)
        throw new TypeError('Instagram DM message text is invalid');
...
export async function listDMThreads(session, opts = {}) {
  ...
  const dom = async () => {
    ...
    const hrefs = await session.page.$$eval(instagramSelectors.dm.threadLink, (nodes) =>
      nodes
        .slice(0, 100)
...
async function replyToDMPrepared(session, thread, message, { beforeDispatch } = {}) {
  ...
  if (!(await threadHasInbound(session))) {
    return makeBlockedResult(
      action,
      'thread has no inbound message; refusing to send (no cold outreach)',
      {
  ...
  return makeActionResult({
    ...
    dryRun: false,
    observation: {
      text: verified ? 'dm reply sent and verified' : 'dm reply not verified',
...
export async function replyToMany(session, items, opts = {}) {
  ...
  if (!bulk.allowed)
    return makeBlockedResult(action, bulk.reason, {
      stage: 'bulk-cap',
      requiresUserDecision: true
    });
  ...
  for (const item of list) {
    ...
    if (!thread)
      return makeBlockedResult(
        action,
        `invalid thread ref in batch: ${JSON.stringify(item?.threadRef)}`,
        { stage: 'thread-ref-validation', requiresUserDecision: true }
      );
    ...
    if (typeof item.message !== 'string' || !item.message.trim()) {
      return makeBlockedResult(action, `empty message for thread ${thread.threadId}`, {
  ...
  const gate = resolveWriteGate(opts);
  if (!gate.allowed) {
    return makeBlockedResult(action, `${gate.reason} (${normalized.length} recipients validated)`, {
      dryRun: true,
      stage: 'dry-run'
    });
  }
  ...
  if (!confirm.allowed) {
    return makeBlockedResult(action, confirm.reason, {
  ...
  for (const { thread, message } of normalized) {
    ...
    if (ledger.done.has(uncertainKey)) {
      return makeBlockedResult(
        action,
        `thread ${thread.threadId} has an unresolved prior dispatch`,
        {
