...
function normalizeVideoContent(value, { videoId, url, includeComments }) {
  if (!value || typeof value !== 'object')
    throw new TypeError('YouTube video content must be an object');
  ...
  return {
    ...
    comments: includeComments
      ? value.comments
          .map((comment) => boundedRequiredText(comment, 'video comment', 10_000))
          .filter(Boolean)
      : []
  };
}
...
async function metadataFor(session) {
  ...
  return session.page.evaluate(
    (extraction) => {
      const publishedAt =
        document.querySelector(extraction.publishedAt)?.getAttribute('content') || null;
      const timestamp = publishedAt === null ? null : Date.parse(publishedAt);
      return {
        publishedAt,
        timestamp: Number.isNaN(timestamp) ? null : timestamp,
        tags:
          extraction.tags === null
            ? []
            : [
                ...new Set(
                  [...document.querySelectorAll(extraction.tags)]
...
export async function getVideo(session, videoRef, opts = {}) {
  ...
  const dom = async () => {
    ...
    const [title, channelHref, description, viewCountText, likeCountText, metadata] =
      await Promise.all([
    ...
    if (includeComments) {
      const selector = await resolveYouTubeSelector(
        session.page,
        youtubeSelectors.video.commentThreads
      );
      if ((await session.page.locator(selector).count()) > 0) {
        comments = await session.page.$$eval(selector, (nodes) =>
          nodes
            .slice(0, 100)
