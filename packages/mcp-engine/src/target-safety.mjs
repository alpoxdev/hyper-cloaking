...
const UNSAFE_PROTOCOLS = new Set(['file:', 'data:', 'javascript:', 'chrome:', 'devtools:']);
...
function classifyIpv4(host) {
  ...
  if (a === 169 && b === 254 && c === 169 && d === 254)
    return { scope: 'blocked', reason: 'cloud-metadata-address' };
  ...
  if (a === 0 || host === '255.255.255.255')
    return { scope: 'blocked', reason: 'unspecified-address' };
...
function classifyIpv6(host) {
  ...
  if (normalized === '::1' || normalized === '0:0:0:0:0:0:0:1')
    return { scope: 'loopback', reason: 'loopback-address' };
  ...
  if ((first & 0xff00) === 0xff00)
    return { scope: 'blocked', reason: 'multicast-or-reserved-address' };
  if (normalized.startsWith('2001:db8:') || normalized === '2001:db8::')
    return { scope: 'blocked', reason: 'reserved-address' };
  return { scope: 'public', reason: 'public-ip-literal' };
}
