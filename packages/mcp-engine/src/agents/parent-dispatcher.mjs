...
export async function dispatchParent(request, dependencies = {}) {
  ...
  if (request.executionMode === 'subagent') {
    if (!dependencies.nativeAdapter?.spawn)
      return failureResponse(
        'native_unavailable',
        'native_unavailable',
        'No verified native adapter is available.'
      );
    try {
    ...
    try {
      envelope = await runners[request.trigger](
        request.input,
        dependencies.roleDependencies?.[request.trigger] || {}
      );
    } catch (error) {
  ...
  if (!verification.ok)
    return failureResponse(
      'contract_failure',
      verification.verifierCode || 'contract_failure',
      verification.failure?.observedSignal || 'role output failed verification'
    );
...
function validateRequest(request) {
  ...
  if (!request.evidence || typeof request.evidence.enabled !== 'boolean')
    return 'evidence config is required';
  const evidenceKeys = new Set(['enabled', 'homeDir', 'publication']);
  if (Object.keys(request.evidence).some((key) => !evidenceKeys.has(key)))
    return 'evidence config contains unknown fields';
  if (
    request.evidence.enabled &&
    (!path.isAbsolute(request.evidence.homeDir || '') ||
      !request.evidence.publication ||
      typeof request.evidence.publication !== 'object')
  )
    return 'enabled evidence requires parent-authorized homeDir and publication roots';
  return null;
}

/** Browser cleanup must be positively closed, successful, and non-timeout before publication. */
function cleanupVerified(value) {
  return value?.ok === true && value?.closed === true && value?.timedOut === false;
}
function isRoleInputFailure(failure) {
  return ['setup-input-invalid', 'browser-input-invalid', 'diagnostics-input-invalid'].includes(
    failure?.code
  );
}
...
export async function runParentDispatcherCli(argv = process.argv.slice(2), io = {}) {
  ...
  if (argv.length !== 2 || argv[0] !== '--input-stdin' || argv[1] !== '--json') {
    stderr.write('Usage: parent-dispatcher --input-stdin --json\n');
    return 2;
  }
  try {
...
export { runParentDispatcherCli as runParentDispatcher };
