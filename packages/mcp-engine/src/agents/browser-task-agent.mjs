...
export async function runBrowserTask(
  input,
  { runLiveVerification = defaultRunLiveVerification } = {}
) {
  const error = validateInput(input);
  if (error)
    return blockedEnvelope(input, 'browser-input-invalid', 'browser-input', error, emptyCleanup());
  ...
  const code = !cleanupVerified
    ? 'browser-cleanup-unverified'
    : !humanization.telemetryAvailable
      ? 'humanization-proof-unavailable'
  ...
  const result = {
    ...
    outcome: liveSucceeded
      ? 'verified'
      : live.publicNavigation?.finalUrl
        ? 'not_observed'
        : 'blocked',
    ...
    learning: {
      summary: liveSucceeded
        ? 'Live verification completed with observed evidence.'
        : 'Live verification stopped at a verified safety or evidence boundary.',
      limitations: [
        ...(live.blockers || []),
        ...(humanization.blocker ? [humanization.blocker] : []),
        ...(!cleanupVerified && cleanup.blocker ? [cleanup.blocker] : [])
      ]
    }
  };
  return {
    ...
    failure: liveSucceeded
      ? null
      : { code, phase: 'browser-live', retryable: false, observedSignal },
    result
  };
}
...
function validateInput(input) {
  ...
  if (input.schemaVersion !== 1 || input.taskMode !== 'verification-only')
    return 'invalid protocol or task mode';
  try {
    const url = new URL(input.targetUrl);
    if (!['http:', 'https:'].includes(url.protocol)) return 'targetUrl must use http or https';
  } catch {
    return 'targetUrl must be an absolute URL';
  }
  try {
  ...
  if (!path.isAbsolute(input.workspace || '') || !path.isAbsolute(input.agentStagingRoot || ''))
    return 'workspace and agentStagingRoot must be absolute parent-authorized paths';
  if (!Number.isInteger(input.maxRedirects) || input.maxRedirects < 0 || input.maxRedirects > 5)
    return 'maxRedirects must be 0..5';
  for (const key of ['provider', 'cookieSite', 'account'])
...
function emptyCleanup() {
  return { ok: false, closed: false, timedOut: false, blocker: 'browser lifecycle did not start' };
}
function normalizeHumanization(value) {
  const telemetryAvailable = value?.ok === true && value?.evidence;
  return {
    enabled: value?.configured === true,
    telemetryAvailable: Boolean(telemetryAvailable),
    source: value?.evidence || null,
    blocker:
      value?.blocker || (telemetryAvailable ? null : 'runtime humanization telemetry unavailable')
  };
}
function mapTargetSafety(value) {
  if (value?.disposition === 'ok') return 'allowed';
  if (value?.disposition === 'approvalRequired' || value?.disposition === 'needs_clarification')
...
function normalizeEvidenceRefs(refs, root) {
  ...
  return refs.map((ref) => {
    ...
    if (!relPath || relPath.startsWith('..') || path.isAbsolute(relPath))
      throw new Error('live evidence escaped agent staging root');
    ...
    return {
      type:
        extension === '.png' || extension === '.jpg' || extension === '.jpeg'
          ...
          : 'log',
      relPath,
      description: ref.kind || ref.description || 'live verification evidence'
    };
  });
}
  ...
  return {
    ...
    result: {
      ...
      contentBoundary: {
        ...
        observedOrigin: null,
        redirects: [],
        violations: [String(observedSignal)]
      },
      humanizationProof: {
        enabled: false,
        telemetryAvailable: false,
        source: null,
        blocker: 'verification did not complete'
      },
      cleanup,
      evidenceRefs: [],
      learning: {
        summary: 'Verification was blocked before completion.',
        limitations: [String(observedSignal)]
      }
    }
  };
}
...
export async function runBrowserTaskCli(argv = process.argv.slice(2), io = {}) {
  ...
  if (argv.length !== 2 || argv[0] !== '--input-stdin' || argv[1] !== '--json') {
    stderr.write('Usage: browser-task-agent --input-stdin --json\n');
    return 2;
  }
  try {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url))
  process.exitCode = await runBrowserTaskCli();
