...
import { generateAllMcpConfigs, generateMcpConfig, validateExecutablePath } from './mcp-config.mjs';
import { assertNavigationAllowed, classifyRedirect, classifyTargetUrl } from './target-safety.mjs';
...
import { markUntrustedBrowserContent, summarizeEvidenceRef } from './evidence-boundary.mjs';
import {
...
import { appendRunShape, clearRunShapes, sanitizeRunShape } from './run-shapes.mjs';
import { launchCloakBrowser } from './browser-utils.mjs';
import {
  getProvider,
...
function wantsHeadless(options) {
  ...
  if (options.headless === false || options.headless === 'false' || options.headless === '0')
    return false;
  return true;
}
  ...
  const normalizedLearning =
    learning === true
      ...
      : {
          enabled: false,
          persisted: false,
          reason: 'self-learning disabled unless explicitly enabled'
        };
...
async function validateCommand() {
  ...
  const outcome = safeOutcome({ text: 'helper-contract no-network no-launch' }, [
    { type: 'textIncludes', expected: 'helper-contract' },
    { type: 'textIncludes', expected: 'no-network' },
    { type: 'textIncludes', expected: 'no-launch' }
  ]);
  ...
  const checks = [
    {
      name: 'skill-id',
      ok: SKILL_ID === 'hyper-cloaking',
      expected: 'hyper-cloaking',
      actual: SKILL_ID
    },
    { name: 'version', ok: VERSION === '0.0.1', expected: '0.0.1', actual: VERSION },
    {
      name: 'schema-version',
      ok: SCHEMA_VERSION === '0.0.1',
      expected: '0.0.1',
      actual: SCHEMA_VERSION
    },
    {
      name: 'default-home',
      ok: DEFAULT_HOME === '~/.hyper-cloaking',
      expected: '~/.hyper-cloaking',
      actual: DEFAULT_HOME
    },
    {
      name: 'mcp-server-id',
      ok: MCP_SERVER_ID === 'hyper-cloaking',
      expected: 'hyper-cloaking',
      actual: MCP_SERVER_ID
    },
    {
      name: 'cloakbrowser-package',
      ok: CLOAKBROWSER_PACKAGE === 'cloakbrowser',
      expected: 'cloakbrowser',
      actual: CLOAKBROWSER_PACKAGE
    },
    {
      name: 'playwright-core-package',
      ok: PLAYWRIGHT_CORE_PACKAGE === 'playwright-core',
      expected: 'playwright-core',
      actual: PLAYWRIGHT_CORE_PACKAGE
    },
    {
      name: 'playwright-mcp-package',
      ok: PLAYWRIGHT_MCP_PACKAGE === '@playwright/mcp',
      expected: '@playwright/mcp',
      actual: PLAYWRIGHT_MCP_PACKAGE
    },
    {
      name: 'provider-registry',
      ok: providerRegistry.ok,
      expected: true,
      actual: providerRegistry.ok
    },
    ...helperChecks
  ];
...
async function smokeCommand(options) {
  ...
  const configs = generateAllMcpConfigs({
  ...
  const diagnostic = blockedFailure(
    'smoke-live-launch',
    ['smoke command does not launch browser dependencies'],
    ['sandbox directory check', 'fake executable validation', 'MCP config rendering']
  );
  const contentBoundary = sampleContentBoundary('about:blank');
  ...
  const runShape = await appendRunShape(
    homePath(home, 'state'),
    {
      command: 'smoke',
      learning: false,
  ...
  return makeResult('smoke', {
    ...
    outcomeReportSample: makeOutcomeReport({
      targetSafety,
      outcome,
      contentBoundary,
      learning: { enabled: false, written: false }
    }),
...
async function mcpConfigCommand(options) {
  ...
  const explicitExecutable = options.executable
    ? path.resolve(expandHome(String(options.executable)))
    : null;
  const executablePath =
    explicitExecutable || (await executableCandidates(homePath(home, 'cache', 'cloakbrowser')))[0];
  const targetSafety = targetSafetyFor('about:blank');
  const contentBoundary = sampleContentBoundary('about:blank');
  ...
  if (!executablePath) {
    return makeResult('mcp-config', {
      ...
      failure: blockedFailure(
        'mcp-config-executable-discovery',
        [`No executable found under ${homePath(home, 'cache', 'cloakbrowser')}`],
        ['contained cache executable discovery']
      ),
      ...completionFields({
        targetSafety,
        outcome: safeOutcome({ text: 'blocked missing executable-path' }, [
          { type: 'textIncludes', expected: 'valid executable' }
        ]),
        failure: blockedFailure(
          'mcp-config-executable-discovery',
          [`No executable found under ${homePath(home, 'cache', 'cloakbrowser')}`],
      ...
      installHint:
        'Run local live/setup under a contained HYPER_CLOAKING_HOME so CloakBrowser cache artifacts stay under ~/.hyper-cloaking.',
      network: 'not-used',
      liveLaunch: 'not-attempted'
    });
  }
  ...
  if (!executable.ok) {
    return makeResult('mcp-config', {
      ...
      failure: blockedFailure(
        'mcp-config-executable-validation',
        [executable.reason],
        ['executable path validation']
      ),
      ...completionFields({
        targetSafety,
        outcome: safeOutcome({ text: 'blocked invalid executable-path' }, [
          { type: 'textIncludes', expected: 'valid executable' }
        ]),
        failure: blockedFailure(
          'mcp-config-executable-validation',
  ...
  return makeResult('mcp-config', {
    ...
    ...completionFields({
      targetSafety,
      outcome: safeOutcome({ text: 'valid-executable-path mcp-config' }, [
        { type: 'textIncludes', expected: 'valid-executable-path' },
        { type: 'textIncludes', expected: 'mcp-config' }
      ]),
      contentBoundary,
      learning: false
    }),
...
export async function installNavigationSafety(page, options = {}) {
  if (!page || typeof page.route !== 'function') {
    return {
      ok: false,
      blocker:
        'Playwright route interception is unavailable; unsafe redirects cannot be blocked before request dispatch'
    };
  }
...
function normalizedAllowedOrigins(values) {
  ...
  if (normalized.some((value) => value === null))
    throw new Error('allowed origin is invalid or opaque');
  if (new Set(normalized).size !== normalized.length)
    throw new Error('allowed origins contain duplicates');
  return normalized;
}
...
function normalizeMaxRedirects(value) {
  ...
  if (!Number.isInteger(parsed) || parsed < 0 || parsed > 5)
    throw new Error('maxRedirects must be an integer from 0 to 5');
  return parsed;
}
  ...
  if (typeof page.locator === 'function') {
    try {
      ...
      if (body && typeof body.innerText === 'function')
        return await body.innerText({ timeout: 1_000 });
    } catch (error) {
...
export async function gotoAndClassify(page, fromUrl, toUrl, options = {}) {
  ...
  if (!navigationState || typeof navigationState !== 'object') {
    const error = new Error(
      'Navigation safety route interception must be installed before page.goto'
    );
  ...
  return {
    finalUrl,
    redirectSafety,
    documentUrls: [...navigationState.documentUrls],
    violations: [...navigationState.violations]
  };
}
...
function buildCookieSelection(options, providerInfo, matchedViaNavigationOnlyAlias) {
  ...
  const providerSiteKeyHint =
    providerInfo?.ok && !matchedViaNavigationOnlyAlias ? providerInfo.cookieSiteKeyHint : null;
...
export async function runLiveVerification(options, deps = {}) {
  ...
  const cookieSelection = buildCookieSelection(
    options,
    providerResolution.provider,
    providerResolution.matchedViaNavigationOnlyAlias
  );

  if (providerResolution.providerError) {
    const providerBlockers = [
      `Provider resolution failed: ${providerResolution.providerError.code}: ${providerResolution.providerError.message}`
    ];
    const providerFailure = blockedFailure('live-provider-resolution', providerBlockers, [
  ...
  if (!runtimeOk)
    blockers.push(`Node ${process.version} is below CloakBrowser JavaScript requirement >=20`);
  if (targetSafety.disposition && targetSafety.disposition !== 'ok')
    blockers.push(
      `Target safety ${targetSafety.disposition}: ${targetSafety.reason || 'navigation is not allowed'}`
    );
  if (navigationTargetSafety.disposition && navigationTargetSafety.disposition !== 'ok')
    blockers.push(
      `Navigation target safety ${navigationTargetSafety.disposition}: ${navigationTargetSafety.reason || 'navigation is not allowed'}`
    );
  ...
  if (!mcpExecutablePath)
    blockers.push(`No executable found under ${homePath(home, 'cache', 'cloakbrowser')}`);
  ...
  function blockedResult(
    ...
    return makeResult('live', {
      ...
      ...baseLiveFields,
      aboutBlank: {
        requested: target === 'about:blank',
        ok: false,
        blocker: 'browser launch not attempted before live preflight passes'
      },
      publicNavigation: {
        requested: true,
      ...
      mcpFeasibility: {
        ok: false,
        blocker:
          'Playwright MCP launch is not attempted before live package availability and containment pass'
      },
      cleanup: {
        ok: true,
        closed: true,
      ...
      ...completionFields({
        ...
        outcome: safeOutcome(
          {
            text: 'blocked before actual-browser-launch',
            target,
            targetSafety,
            blockers: blockedBy,
            packageChecks,
  ...
  try {
    const launched = await runWithTemporaryEnv(childEnv, () =>
      launchBrowser({ workspace: home, headless: wantsHeadless(options) })
    );
    ...
    let navigation;
    if (
      providerResolution.resolvedProvider?.id &&
      providerResolution.resolvedProvider.id !== 'generic'
    ) {
      ...
      const strictNavigation = await strictSession.navigateGuardedForRead(navigationTarget, {
      ...
      navigation = {
        ...
        documentUrls: Array.isArray(routeState?.documentUrls)
          ? [...routeState.documentUrls]
          : [strictNavigation.url],
    ...
    if (challenge.blocker) {
      const error = new Error(
        `Challenge or access blocker observed: ${challenge.labels.join(', ')}`
      );
    ...
    const outcome = safeOutcome(
      ...
      [
        ...
        {
          type: 'urlIncludes',
          id: 'live-target-origin',
          expected: new URL(navigationTarget).hostname
        },
        ...(evidenceRefs.length > 0
          ? [{ type: 'evidenceCaptured', id: 'live-evidence-captured' }]
          : [])
      ]
    );
    const humanization = {
      ...
      method: 'cloakbrowser-js-api',
      evidence:
        'launchCloakBrowser configuration requests humanize:true; runtime telemetry is unavailable',
      blocker: 'runtime humanization telemetry unavailable'
    };
    const cleanupVerified =
      cleanup.ok === true && cleanup.closed === true && cleanup.timedOut === false;
    ...
    return makeResult('live', {
      ...
      ...completionFields({
        ...
        failure: livePassed
          ? null
          : blockedFailure(
              cleanupVerified ? 'live-outcome' : 'browser-cleanup-unverified',
              liveBlockers,
              attempted
            ),
        contentBoundary,
    ...
    return makeResult('live', {
      ...
      ...completionFields({
        ...
        outcome: safeOutcome({ text: reason, target, navigationTarget, liveLaunch: 'attempted' }, [
...
async function readCredentialStdin() {
  if (process.stdin.isTTY)
    throw new Error('credential import requires piped stdin, --source, or --env-prefix');
  let input = '';
  let bytes = 0;
  process.stdin.setEncoding('utf8');
  for await (const chunk of process.stdin) {
    bytes += Buffer.byteLength(chunk);
    if (bytes > MAX_CREDENTIAL_STDIN_BYTES)
      throw new Error('credential import stdin exceeds the size limit');
    input += chunk;
  }
...
function credentialCommandResult(operation, fields = {}) {
  const targetSafety = targetSafetyFor('about:blank');
  const outcome = safeOutcome({ text: `credential-${operation}-complete no-network`, operation }, [
...
async function credentialsCommand(options) {
  ...
  const home = resolveHome(options.home);
  const forbidden = Object.keys(options).filter(
    (key) => key !== '_' && /(?:secret|token|password|api-key|access-key|credential)/i.test(key)
  );
  if (forbidden.length > 0) {
    throw new Error(
      `credential values must not be supplied as command arguments: ${forbidden.join(', ')}`
    );
  }
  ...
  if (options._.length > 2)
    throw new Error('credentials command does not accept extra positional arguments');
  const unknown = Object.keys(options).filter(
    (key) => key !== '_' && key !== 'home' && key !== 'json' && !allowed.includes(key)
  );
  ...
  if (operation === 'import') {
    ...
    if (hasSource && hasEnvironment) {
      throw new Error(
        'credential import requires exactly one source: stdin, --source, or --env-prefix'
      );
    }
    let profile;
    if (hasSource) {
      if (typeof options.source !== 'string')
        throw new Error('credential import --source requires a file path');
      profile = await profileFromSecureSource({ file: options.source });
    } else if (hasEnvironment) {
      if (typeof options['env-prefix'] !== 'string')
        throw new Error('credential import --env-prefix requires a value');
  ...
  if (operation === 'set-default') {
    return credentialCommandResult(
      operation,
      await setDefaultCredentialProfile({
        home,
        provider: options.provider,
        profileId: options['profile-id']
      })
  ...
  if (operation === 'reconcile') {
    return credentialCommandResult(
      operation,
      await reconcileCredentialOperation({
        home,
