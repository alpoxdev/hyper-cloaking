...
export async function humanScroll(page, options = {}) {
  ...
  const pixelsPerSecond = Math.max(
    1,
    Number(options.pixelsPerSecond ?? DEFAULT_HUMAN_SCROLL_PIXELS_PER_SECOND)
  );
  const pauseMs =
    options.pauseMs ?? Math.round(((Math.abs(distance) / pixelsPerSecond) * 1000) / steps);
  const pauseJitter =
    options.pauseMs == null ? (options.pauseJitter ?? DEFAULT_HUMAN_SCROLL_PAUSE_JITTER) : 0;
