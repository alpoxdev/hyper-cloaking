...
export function inferSiteForUrl(config, targetUrl) {
  ...
  if (matches.length === 0)
    return Object.hasOwn(validatedConfig.sites, 'default') ? 'default' : null;
  if (
    matches.length > 1 &&
...
export function selectCookieRecords(config, targetUrl, options = {}) {
  ...
  if (targetUrl != null && targetUrl !== '')
    validateCookieUrl(targetUrl, 'cookie selection target URL');

  const requestedSite = options.site;
  const explicitSite =
    requestedSite && Object.hasOwn(validatedConfig.sites, requestedSite) ? requestedSite : null;
...
export async function cookiesForUrl(targetUrl, options = {}) {
  ...
  if (selection.needsAccount) {
    throw new Error(
      `Multiple cookie accounts are available for site "${selection.site}". Choose one with --account. Available: ${selection.availableAccounts.join(', ')}`
    );
  }
...
export function cookiesFromJsonPayload(payload) {
  ...
  throw new TypeError(
    'Unsupported cookie JSON payload: expected an array or an object with a cookies array'
  );
}
    ...
    for (const [accountName, account] of Object.entries(accounts)) {
      ...
      for (const cookie of cookies) {
        const normalizedCookie =
          normalizeCookie(cookie, { domain: site.domain, url: site.url }) || cookie;
        const entries = Object.entries(normalizedCookie).filter(
...
export async function importJsonCookies(payload, options) {
  ...
  const domain = validateCookieDomain(
    options.domain ||
      (targetUrl ? `.${new URL(targetUrl).hostname.replace(/^www\./, '')}` : undefined),
    'import domain'
  );
  const url = validateCookieUrl(options.url, 'import URL');
  const normalized = rawCookies.map((cookie, index) =>
    normalizeImportedCookie(cookie, { domain, url }, index)
  );
  const config = await loadCookieConfig(cookieFile);

  if (!Object.hasOwn(config.sites, siteKey))
    config.sites[siteKey] = { accounts: Object.create(null) };
    ...
    try {
      ...
      if (cleanupError?.code !== 'ENOENT') {
        throw new AggregateError(
          [error, cleanupError],
          'cookie import and temporary-file cleanup failed'
        );
      }
    }
    throw error;
 ...
 */
export async function runCookieCli(argv = process.argv.slice(2)) {
  const args = argv;
...
Purpose:
  ...
  if (command === 'inspect') {
    ...
    console.log(
      json ? JSON.stringify(output, null, 2) : `${cookies.length} cookies from ${paths.cookieFile}`
    );
    return 0;
  }
  ...
  if (command === 'import-json') {
    ...
    console.log(
      json
        ...
        : `Imported ${result.count} cookies into ${result.cookieFile}`
    );
    return 0;
  }
