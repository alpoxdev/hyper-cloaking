...
export function normalizeBlogRef(ref) {
  const suppliedId =
    ref && typeof ref === 'object' && ref.blogId != null
      ? String(ref.blogId)
  ...
  if (!blogId || !BLOG_ID_RE.test(blogId) || (suppliedId && match?.[1] && suppliedId !== match[1]))
    return null;
...
export function normalizeBlogPostRef(ref) {
  const suppliedBlogId =
    ref && typeof ref === 'object' && ref.blogId != null ? String(ref.blogId) : null;
  const suppliedLogNo =
    ref && typeof ref === 'object' && ref.logNo != null ? String(ref.logNo) : null;
  const rawUrl =
    typeof ref === 'string' ? ref : ref && typeof ref === 'object' ? ref.url || ref.href : null;
  const url = parseOwnedUrl(rawUrl, BLOG_HOSTS);
...
export function normalizeCafeRef(ref) {
  const suppliedId =
    ref && typeof ref === 'object' && ref.cafeId != null
      ? String(ref.cafeId)
      : typeof ref === 'string' && !ref.includes('://') && !ref.includes('/')
        ? ref
        : null;
  ...
  if (!cafeId || !CAFE_ID_RE.test(cafeId) || (suppliedId && match?.[1] && suppliedId !== match[1]))
    return null;
  if (rawUrl != null && !match) return null;
...
export function normalizeCafePostRef(ref) {
  const suppliedCafeId =
    ref && typeof ref === 'object' && ref.cafeId != null ? String(ref.cafeId) : null;
  const suppliedArticleId =
    ref && typeof ref === 'object' && ref.articleId != null ? String(ref.articleId) : null;
  const rawUrl =
    typeof ref === 'string' ? ref : ref && typeof ref === 'object' ? ref.url || ref.href : null;
  const url = parseOwnedUrl(rawUrl, CAFE_HOSTS);
  const match = url?.pathname.match(/^\/([A-Za-z0-9_-]{1,64})\/(\d{1,32})\/?$/);
  ...
  if (!cafeId || !CAFE_ID_RE.test(cafeId) || !articleId || !ARTICLE_ID_RE.test(articleId))
    return null;
