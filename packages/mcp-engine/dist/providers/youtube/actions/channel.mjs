...
function tagsFromText(value) {
  return [
    ...new Set((String(value ?? '').match(/#[\p{L}\p{N}_-]+/gu) || []).map((tag) => tag.slice(1)))
  ];
}
...
function boundedText(value, field, maxLength) {
  ...
  if (text.length > maxLength)
    throw new TypeError(`YouTube ${field} exceeds ${maxLength} characters`);
  return text;
}
...
function videoRecord(link) {
  ...
  const tags = (Array.isArray(link.tags) ? link.tags : tagsFromText(link.metadataText)).map(
    (tag) => {
      const text = String(tag);
      if (text.length > 100)
        throw new TypeError('YouTube channel video tag exceeds 100 characters');
      return text;
    }
  );
  return {
...
function normalizeChannelContent(value, { channelId, url, limit }) {
  ...
  for (const entry of value.videos) {
    if (!entry || typeof entry !== 'object')
      throw new TypeError('YouTube channel video entries must be objects');
  ...
  if (videos.length === 0 && value.emptyState !== true) {
    throw new TypeError(
      'YouTube normalized empty channel content requires explicit empty-state evidence'
    );
  }
  return {
    channelId,
...
export async function getChannel(session, channelRef, opts = {}) {
  ...
  const dom = async () => {
    ...
    const [name, handle, subscriberCount, description, videoSelector] = await Promise.all([
      ...
      resolveYouTubeSelector(session.page, youtubeSelectors.channel.videoLinks, {
    ...
    const links =
      videoSelector === null
        ...
        : await session.page.$$eval(
            videoSelector,
            (nodes, selectedExtraction) =>
              nodes.slice(0, 400).map((node) => {
                const card = node.closest(selectedExtraction.videoCard);
                const metadataText =
                  card?.querySelector(selectedExtraction.metadataLine)?.textContent?.trim() || '';
                const publishedAt =
                  card?.querySelector(selectedExtraction.publishedAt)?.getAttribute('datetime') ||
                  null;
                return {
                  ...
                  tags: [
                    ...new Set(
                      (card?.textContent?.match(/#[\p{L}\p{N}_-]+/gu) || []).map((tag) =>
                        tag.slice(1)
                      )
                    )
                  ]
  ...
  const { value } = await executeYouTubeRead({
    ...
    dom,
    normalize: (content) =>
      normalizeChannelContent(content, {
        channelId,
        url,
        limit
      })
