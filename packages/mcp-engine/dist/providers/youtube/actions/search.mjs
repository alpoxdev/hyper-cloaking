...
function normalizeSearchContent(value, { query, limit }) {
  ...
  for (const entry of value.videos) {
    if (!entry || typeof entry !== 'object')
      throw new TypeError('YouTube search entries must be objects');
  ...
  if (videos.length === 0 && value.emptyState !== true) {
    throw new TypeError(
      'YouTube normalized empty search content requires explicit empty-state evidence'
    );
  }
...
export async function searchVideos(session, query, opts = {}) {
        ...
        : await session.page.$$eval(
            ...
            (nodes, selectedExtraction) =>
              nodes.slice(0, 400).map((node) => ({
                ...
                channel:
                  node
                    .closest(selectedExtraction.resultCard)
                    ?.querySelector(selectedExtraction.channelLink)
  ...
  const { value } = await executeYouTubeRead({
    ...
    dom,
    normalize: (content) =>
      normalizeSearchContent(content, {
        query: searchQuery,
        limit
      })
  });
