...
async function desiredToggle(
  ...
  if ((await session.page.locator(selector).count()) !== 1)
    return block(action, 'owned TikTok transition control is not unique', 'selector-ownership');
...
export async function setLiked(session, videoRef, liked, opts = {}) {
  ...
  let video;
  try {
    video = assertVideoRef(videoRef);
  } catch (error) {
    return invalid(action, error, 'video-ref-validation');
  }
  return desiredToggle(session, {
    ...
    rateKey: 'tiktok-like',
    maxPerWindow: 10,
    offSelector: tiktokSelectors.video.like,
    onSelector: tiktokSelectors.video.unlike,
...
export async function setSaved(session, videoRef, saved, opts = {}) {
  ...
  return desiredToggle(session, {
    ...
    desired: saved,
    opts,
    enableKey: 'enableSave',
    rateKey: 'tiktok-save',
    maxPerWindow: 10,
    offSelector: tiktokSelectors.video.save,
    onSelector: tiktokSelectors.video.unsave,
    content: { videoId: video.videoId }
  });
}
...
export async function setFollowing(session, userRef, following, opts = {}) {
  ...
  return desiredToggle(session, {
    ...
    rateKey: 'tiktok-follow',
    maxPerWindow: 5,
    offSelector: tiktokSelectors.video.follow,
    onSelector: tiktokSelectors.video.following,
...
export async function setReposted(session, videoRef, reposted, opts = {}) {
  ...
  return desiredToggle(session, {
    ...
    rateKey: 'tiktok-repost',
    maxPerWindow: 5,
    offSelector: tiktokSelectors.video.repost,
    onSelector: tiktokSelectors.video.unrepost,
    content: { videoId: video.videoId }
  });
}
...
async function createComment(session, { action, target, parentId, expected, opts, enableKey }) {
  ...
  if (parentId) {
    ...
    if ((await root.count()) !== 1)
      return block(action, 'TikTok parent comment is not unique', 'selector-ownership');
    replyControl = root.first().locator(tiktokSelectors.video.reply);
    if ((await replyControl.count()) !== 1)
      return block(action, 'owned TikTok reply control is not unique', 'selector-ownership');
  }

  const before = await exactTextCount(session.page, tiktokSelectors.video.commentText, expected);
  return guarded(session, {
    action,
    rateKey: 'tiktok-comment',
    maxPerWindow: 5,
    target,
    ...
    verify: () =>
      waitFor(
        session.page,
        async () =>
          (await exactTextCount(session.page, tiktokSelectors.video.commentText, expected)) > before
      ),
...
export async function commentVideo(session, videoRef, text, opts = {}) {
  ...
  let expected;
  try {
    expected = normalizeText(text, 'TikTok comment', 2_200);
  } catch (error) {
...
export async function replyToComment(session, commentRef, text, opts = {}) {
  ...
  let comment;
  try {
    comment = assertCommentRef(commentRef);
  } catch (error) {
    return invalid(action, error, 'comment-ref-validation');
  }
  let expected;
  try {
    expected = normalizeText(text, 'TikTok comment', 2_200);
  ...
  return createComment(session, {
    ...
    expected,
    opts,
    enableKey: 'enableReply'
  });
}

async function messageRows(page) {
  return page.$$eval(tiktokSelectors.dm.message, (nodes) =>
    nodes
      .slice(-100)
      .map((node) => ({
...
export async function replyToDM(session, threadRef, text, opts = {}) {
  ...
  let expected;
  try {
    expected = normalizeText(text, 'TikTok DM', 10_000);
  } catch (error) {
    return block(action, error.message, 'input-validation');
  }
...
export async function createUploadDraft(session, file, opts = {}) {
  ...
  let upload;
  try {
    upload = await validateUploadFile(file);
  } catch (error) {
    return block(action, error.message, 'upload-validation');
  }
  await session.navigateGuardedForWrite(UPLOAD_URL, {
    waitUntil: 'domcontentloaded',
    timeout: 30_000
  });
  const before = new Set(
    await session.page
      .locator(tiktokSelectors.upload.draft)
      .evaluateAll((nodes) =>
  ...
  return guarded(session, {
    action,
    rateKey: 'tiktok-upload',
    maxPerWindow: 2,
    target: UPLOAD_URL,
    content: { fileHash: upload.fileHash },
    runId: opts.runId,
    dispatch: () =>
      session.page.locator(tiktokSelectors.upload.input).setInputFiles({
        name: upload.name,
...
export async function publishDraft(session, draftRef, opts = {}) {
  ...
  const draftNode = session.page.locator(
    `${tiktokSelectors.upload.draft}[data-draft-id="${draft.draftId}"]`
  );
  if ((await draftNode.count()) !== 1)
    return block(action, 'existing owned draft could not be proven', 'draft-state');
  ...
  const beforeVideoIds = new Set(
    await session.page
      ...
      .evaluateAll((nodes) =>
        nodes.map((node) => node.getAttribute('data-video-id')).filter(Boolean)
      )
  );
  return guarded(session, {
    action,
    rateKey: 'tiktok-publish',
    maxPerWindow: 1,
    target: UPLOAD_URL,
    content: { draftId: draft.draftId, publish: true },
    runId: opts.runId,
    dispatch: async () => {
      const publishControl = draftNode
        .first()
        .locator('button[data-e2e="post_video_button"], button:has-text("Post")');
      if ((await publishControl.count()) !== 1)
        throw new Error('owned draft publish control is not unique');
      await session.humanClick(publishControl.first());
    },
    verify: () =>
      waitFor(session.page, async () => {
        const videoIds = await session.page
          .locator(publishedSelector)
          .evaluateAll((nodes) =>
            nodes
...
export function blockedTikTokAction(actionName) {
  ...
  if (!FORBIDDEN_ACTIONS.has(name))
    throw new TypeError(`unsupported TikTok structural blocker: ${name}`);
  return block(
    `tiktok:${name}`,
    `${name} is structurally blocked for TikTok automation`,
    'structural-blocker'
  );
}
