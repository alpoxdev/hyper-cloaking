...
export const MANAGED_EXCLUDED_SEGMENTS = Object.freeze([
  '.git',
  '.gjc',
  '.impeccable',
  '.omc',
 ...
 */
export async function captureMirrorBaseline({
  canonicalDir,
  mirrorDirs,
  ownedManifest = OWNED_CANONICAL_MANIFEST,
  excludedSegments = MANAGED_EXCLUDED_SEGMENTS,
  invocationToken = crypto.randomUUID(),
  dirtyLedgerSha256 = null
}) {
  ...
  if (
    mirrorRealpaths.includes(canonicalRealpath) ||
    new Set(mirrorRealpaths).size !== mirrorRealpaths.length
  ) {
  ...
  for (const mirror of mirrorRealpaths)
    mirrorInventories.push(await inventory(mirror, excludedSegments));
  for (let index = 0; index < mirrorInventories.length; index += 1) {
    if (stableStringify(mirrorInventories[index]) !== stableStringify(canonicalInventory))
      throw new Error(`initial managed parity mismatch: ${mirrorRealpaths[index]}`);
  }
  const ownedBaseline = {};
...
export async function prepareMirrorSync({ baseline }) {
  ...
  for (const mirror of baseline.mirrorRealpaths)
    mirrors.push(await inventory(mirror, baseline.excludedSegments));
  for (let index = 0; index < mirrors.length; index += 1) {
    if (
      stableStringify(mirrors[index]) !==
      stableStringify(baseline.managedInventories.mirrors[index])
    )
      throw new Error(`mirror drift detected: ${baseline.mirrorRealpaths[index]}`);
  }
  const owned = new Set(baseline.ownedManifest);
  const allPaths = new Set([
    ...Object.keys(canonical),
    ...Object.keys(baseline.managedInventories.canonical)
  ]);
  for (const relativePath of allPaths) {
    const before = baseline.managedInventories.canonical[relativePath] || absent();
    const after = canonical[relativePath] || absent();
    if (!sameState(before, after) && !owned.has(relativePath))
      throw new Error(`canonical change outside owned manifest: ${relativePath}`);
  }
  const operations = [];
  ...
  const prepared = {
    ...
    canonicalFinal: Object.fromEntries(
      baseline.ownedManifest.map((item) => [item, canonical[item] || absent()])
    ),
    operations,
    createdAt: new Date().toISOString()
  };
...
export async function applyMirrorSync({
  ...
  if (!recoveryRoot || !path.isAbsolute(recoveryRoot))
    throw new Error('recoveryRoot must be an absolute path');
...
export async function recoverMirrorSync({ journalPath, invocationToken }) {
  const lines = (await fsp.readFile(journalPath, 'utf8'))
    .trim()
    .split(/\r?\n/)
    .filter(Boolean)
  ...
  const completed = new Set(
    lines.filter((event) => event.event === 'commit-complete').map((event) => event.opId)
  );
  const commitIntents = new Set(
    lines.filter((event) => event.event === 'commit-intent').map((event) => event.opId)
  );
  const backups = new Map(
    lines
      .filter((event) => event.event === 'backup-complete')
      .map((event) => [event.opId, event.backupPath])
  );
...
async function revalidatePrepared(baseline, prepared) {
  ...
  if (
    stableStringify(next.canonicalFinal) !== stableStringify(prepared.canonicalFinal) ||
...
async function verifyFinalParity(baseline) {
  ...
  for (const mirror of baseline.mirrorRealpaths) {
    ...
    if (stableStringify(current) !== stableStringify(canonical))
      throw new Error(`final managed parity mismatch: ${mirror}`);
  }
}
...
async function validateRecoveryHeader(header, invocationToken) {
  if (
    header?.event !== 'header' ||
    header.version !== 1 ||
    header.invocationToken !== invocationToken
  ) {
  ...
  if (
    !Array.isArray(header.excludedSegments) ||
  ...
  if (
    !path.isAbsolute(header.canonicalRealpath) ||
  ...
  for (const mirror of header.mirrorRealpaths) {
    if ((await secureDirectory(mirror)) !== mirror)
      throw new Error(`mirror recovery topology changed: ${mirror}`);
  }
  if (
    !header.managedBaseline ||
  ...
  for (const operation of header.operations) {
    if (
      !operation ||
      !Number.isInteger(operation.mirrorIndex) ||
    ...
    if (
      ...
      !sameState(operation.final, header.canonicalFinal[operation.relativePath] || absent())
    ) {
...
async function inspectRecoveryLock(header, journalPath) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink())
    throw new Error('stale mirror lock is not a regular file');
  const record = JSON.parse(await fsp.readFile(header.lockPath, 'utf8'));
  if (
    record.invocationToken !== header.invocationToken ||
  ...
  if (!Number.isInteger(record.pid) || record.pid <= 0)
    throw new Error('stale mirror lock pid is invalid');
  if (isProcessAlive(record.pid))
    throw new Error(`mirror lock owner is still active: ${record.pid}`);
  return record;
}
...
function validateManifest(manifest) {
  if (!Array.isArray(manifest) || manifest.length === 0)
    throw new Error('owned manifest is required');
  const seen = new Set();
...
function validateBaseline(baseline) {
  if (!baseline || baseline.version !== 1 || !baseline.baselineSha256)
    throw new Error('invalid baseline');
  if (baseline.baselineSha256 !== digestJson(stripDigest(baseline, 'baselineSha256')))
...
function absent() {
  return { type: 'absent', sha256: null, size: null, mode: null };
}
...
function defaultLockPath(canonicalRealpath) {
  return path.join(
    os.tmpdir(),
    'hyper-cloaking-sync',
    `${sha256(Buffer.from(canonicalRealpath))}.lock`
  );
}
