...
export function normalizeAllowedOrigins(allowedOrigins) {
  ...
  if (normalized.some((value) => value === null))
    throw new Error('allowedOrigins contains an invalid or opaque origin');
  if (new Set(normalized).size !== normalized.length)
    throw new Error('allowedOrigins contains duplicates');
...
export function guardAllowedOrigin({
  url,
  allowedOrigins,
  classify = classifyTargetUrl,
  allowAboutBlank = false
}) {
  if (allowAboutBlank && url === 'about:blank')
    return {
      ok: true,
      url,
      origin: 'about:blank',
      classification: classify(url, { allowAboutBlank: true })
    };
  ...
  if (!normalizedAllowed.includes(origin))
    return { ok: false, reason: 'origin-not-in-allowlist', url, origin };
  const classification = classify(url);
  if (classification.disposition !== 'ok')
    return { ok: false, reason: 'target-safety-rejected', url, origin, classification };
