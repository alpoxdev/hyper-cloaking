...
export async function runSetup(input, { runCli = defaultRunCli, makeIo = makeMemoryIo } = {}) {
  ...
  if (inputError)
    return failedEnvelope('setup-input-invalid', 'setup-input', inputError, 'blocked');
  ...
  if (!validation.ok || validation.value?.ok !== true) {
    return setupEnvelope({
      ...
      failure: failure(
        ...
        'setup-validate',
        validation.error || firstBlocker(validation.value) || 'validation failed'
      ),
      blockers: [
        {
          code: 'permission',
          message: validation.error || firstBlocker(validation.value) || 'validation failed',
          recoverable: true
        }
      ]
    });
  ...
  if (!generated.value)
    return failedEnvelope('setup-config-invalid-json', 'setup-config', generated.error, 'failed');
  ...
  if (mismatch)
    return failedEnvelope('setup-config-mismatch', 'setup-config-verify', mismatch, 'failed');
  return setupEnvelope({
    status: 'succeeded',
    setupStatus: 'ready',
    failure: null,
...
function validateInput(input) {
  ...
  if (Object.keys(input).some((key) => !expected.includes(key)))
    return 'input contains unknown fields';
  ...
  if (typeof input.workspace !== 'string' || !path.isAbsolute(input.workspace))
    return 'workspace must be an authorized absolute path';
  return null;
}
...
async function invokeJson(runCli, argv, makeIo) {
  ...
  try {
    return {
      ...
      error: exitCode === 0 ? null : captured.stderr.trim() || null
    };
  } catch {
    return { ok: false, error: 'CLI returned malformed JSON' };
  }
}
...
function verifyGeneratedConfig(value, input) {
  ...
  if (path.resolve(value.home || '') !== path.resolve(input.workspace))
    return 'generated workspace does not match input';
  if (!value.executablePath || !path.isAbsolute(value.executablePath))
    return 'generated executable path is invalid';
  ...
  if (countOption(value.config, '--no-sandbox') > 0)
    return 'generated config contains --no-sandbox';
  const headlessCount = countOption(value.config, '--headless');
  if (headlessCount !== (input.headless ? 1 : 0))
    return 'generated headless mode contradicts input';
  if (!strings.some((item) => item.includes('@playwright/mcp')))
    return 'generated config is missing Playwright MCP package';
  if (
    !strings.includes(value.executablePath) &&
    !strings.some((item) => item.includes(value.executablePath))
  )
    return 'generated config is missing executable path';
  return null;
}
