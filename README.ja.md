...
- **手間いらずのセットアップ** —— インストール済みパッケージが管理された MCP サーバーと型付きツールを提供し、ソースビルドはリポジトリ開発専用です。
...
3. **セットアップゲート** —— Node.js と管理されたローカル MCP サーバーを確認し、欠けているものをインストール・修復します。
...
8. **MCP 設定** —— パッケージ提供の管理サーバーと登録レンダラーを、現在の Node 実行ファイルおよび解決済みサーバーパスとともに使用します。
...
インストール済みの利用者は、`@alpoxdev/hyper-cloaking` パッケージから管理されたローカル MCP サーバーとエンジンユーティリティを利用できます。クライアントをホストするプロジェクトにインストールします。

```bash
npm install @alpoxdev/hyper-cloaking
```

`hyper-cloaking-mcp` は stdio サーバーを起動します。サポートされたレンダラー export から、使用するクライアントの登録を生成します。
...
すべてのエンジンコードは `mcp/engine/` に一元所有され、公開パッケージに同梱されます。インストール済みの利用者は次のコマンドと export を使います。

| サーフェス | インストール済み形式 |
|---|---|
| 管理された stdio MCP | `hyper-cloaking-mcp` |
| エンジン CLI: `validate`、`smoke`、`mcp-config`、`live`、`credentials` | `hyper-cloaking-engine <subcommand>` |
| ブラウザヘルパー: `init`、`cookies` | `hyper-cloaking-browser-utils <subcommand>` |
| クッキーヘルパー: `inspect`、`import-json` | `hyper-cloaking-cookie <subcommand>` |
| 親ディスパッチャー | `hyper-cloaking-parent-dispatcher --input-stdin --json` |
| 登録レンダラー | `@alpoxdev/hyper-cloaking/register` |

エンジンの行は実行コマンドのラベルであり、npm パッケージや import specifier ではありません。プログラムからの利用は、明示的に選定された `@alpoxdev/hyper-cloaking/engine/...` export に限定されます。列挙されていないパスや直接の provider import には依存しないでください。
...
**Provider（メタデータのみ）。** `hyper-cloaking-engine live --provider <id>` は**メタデータのみ**を選択します —— `naver`、`instagram`、`youtube`、`x`、`coupang`、`tiktok`、`generic` に対するドメイン/オリジンおよびクッキー/プロファイルのヒントです。Provider はより広いオリジンを認可したり、安全・偵察・プリフライトのゲートを回避したりせず、未知の provider は安全側に失敗（fail closed）します。
...
以下のコマンドはリポジトリ開発専用です。インストール済みの利用者は、上記のパッケージコマンドと export を使用してください。
...
```text
mcp/engine/                         # 唯一のエンジン所有者、@alpoxdev/hyper-cloaking に同梱
mcp/register.mjs                    # サポートされた登録レンダラー
mcp/src/                             # 管理された MCP サーバーソース
...
以下はインストール済み利用者向けのセットアップではなく、リポジトリのソースビルドコマンドです。

```bash
npm --workspace mcp run build # ソース MCP バンドル
...
node mcp/engine/cli.mjs validate --json   # エンジンの自己チェック（ネットワークなし）
```

`npm test` はルート E2E スイート（`tests/e2e/`）と `tests/unit/engine/` のエンジン単体テストを実行し、これらは唯一の `mcp/engine/` ソースをインポートします。`npm run validate` はミラーされたスキルディレクトリがバイト単位で一致することを検証します。
