...
- **패치된 User-Agent가 아니라 진짜 스텔스 브라우저** — 관리형 로컬 `hyper-cloaking-mcp` 서버가 헤더만 바꾸는 대신 실제 브라우저 지문을 가진 CloakBrowser를 구동합니다.
...
- **번거로움 없는 설정** — 설치된 패키지가 관리형 MCP 서버와 타입이 지정된 도구를 제공하며, 소스 빌드는 저장소 개발에만 사용합니다.
...
3. **설정 게이트** — Node.js와 관리형 로컬 MCP 서버를 확인하고, 없는 것은 설치·복구합니다.
...
8. **MCP 설정** — 패키지로 제공되는 관리형 서버와 등록 렌더러를 현재 Node 실행 파일 및 해석된 서버 경로와 함께 사용합니다.
...
설치 사용자는 `@alpoxdev/hyper-cloaking` 패키지로 관리형 로컬 MCP 서버와 엔진 유틸리티를 받습니다. 클라이언트를 호스팅하는 프로젝트에 설치합니다.

```bash
npm install @alpoxdev/hyper-cloaking
```

`hyper-cloaking-mcp`가 stdio 서버를 시작합니다. 지원되는 렌더러 export에서 사용할 클라이언트의 등록을 생성합니다.
...
생성된 등록을 Codex, Claude Code/Cursor JSON, OpenClaw, Hermes 또는 Gajae-Code 세션과 함께 사용하는 MCP 클라이언트에 적용합니다. 직접 실행:
...
모든 엔진 코드는 `mcp/engine/` 한 곳에서 소유하며 공개 패키지에 포함됩니다. 설치 사용자는 다음 명령 및 export 인터페이스를 사용합니다.

| 인터페이스 | 설치된 형식 |
|---|---|
| 관리형 stdio MCP | `hyper-cloaking-mcp` |
| 엔진 CLI: `validate`, `smoke`, `mcp-config`, `live`, `credentials` | `hyper-cloaking-engine <subcommand>` |
| 브라우저 헬퍼: `init`, `cookies` | `hyper-cloaking-browser-utils <subcommand>` |
| 쿠키 헬퍼: `inspect`, `import-json` | `hyper-cloaking-cookie <subcommand>` |
| 부모 디스패처 | `hyper-cloaking-parent-dispatcher --input-stdin --json` |
| 등록 렌더러 | `@alpoxdev/hyper-cloaking/register` |

엔진 행은 실행 명령 라벨이며 npm 패키지나 import specifier가 아닙니다. 프로그램 방식 사용은 명시적으로 선별된 `@alpoxdev/hyper-cloaking/engine/...` export로 제한됩니다. 나열되지 않은 경로나 직접 provider import에 의존하지 마세요.
...
**프로바이더 (메타데이터 전용).** `hyper-cloaking-engine live --provider <id>`는 **메타데이터만** 선택합니다 — `naver`, `instagram`, `youtube`, `x`, `coupang`, `tiktok`, `generic`에 대한 도메인/출처 및 쿠키/프로필 힌트입니다. 프로바이더는 더 넓은 출처를 승인하거나 안전·정찰·프리플라이트 게이트를 우회하지 않으며, 알 수 없는 프로바이더는 즉시 실패(fail closed)합니다.
...
다음 명령은 저장소 개발용일 뿐입니다. 설치 사용자는 위의 패키지 명령과 export를 사용해야 합니다.
...
무자격 증명 검증 lane은 distribution bundle을 build하고 stdio handshake, 실제 humanized CloakBrowser session launch, status 확인, teardown을 수행합니다. Provider별 실제 사이트 read/write는 credential과 authorization이 필요한 live check이며 CI에서 성공한 것처럼 모의하지 않습니다.
...
```text
mcp/engine/                         # 유일한 엔진 소유자, @alpoxdev/hyper-cloaking에 포함
mcp/register.mjs                    # 지원되는 등록 렌더러
mcp/src/                             # 관리형 MCP 서버 소스
...
다음은 설치 사용자를 위한 설정이 아닌 저장소 소스 빌드 명령입니다.

```bash
npm --workspace mcp run build # 소스 MCP 번들
...
node mcp/engine/cli.mjs validate --json   # 엔진 자체 점검 (네트워크 없음)
```

`npm test`는 루트 E2E 스위트(`tests/e2e/`)와 `tests/unit/engine/`의 엔진 단위 테스트를 실행하며, 이 테스트들은 유일한 `mcp/engine/` 소스를 가져옵니다. `npm run validate`는 미러링된 스킬 디렉터리가 바이트 단위로 동일한지 검증합니다.
