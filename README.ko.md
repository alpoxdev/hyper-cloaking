...
- **번거로움 없는 설정** — 로컬 MCP 번들을 빌드하고 현재 Node 실행 파일과 절대 번들 경로를 사용해 `mcp/src/register.mjs`에서 클라이언트 등록을 생성하며, 타입이 지정된 도구를 사용합니다.
...
3. **설정 게이트** — Node.js와 관리형 로컬 MCP 번들을 확인하고, 없는 것은 설치·복구합니다.
...
8. **MCP 설정** — `mcp/dist/server.mjs`를 빌드한 다음, 현재 Node 실행 파일과 절대 번들 경로를 포함하는 `mcp/src/register.mjs` 생성 등록을 사용합니다.
...
권장하는 실사용 경로는 관리형 `hyper-cloaking-mcp` 서버입니다. 저장소 루트에서 번들을 빌드합니다.

```bash
npm --workspace mcp run build
```

사용할 클라이언트의 등록을 생성합니다. 렌더러가 현재 Node 실행 파일과 절대 경로인 `mcp/dist/server.mjs`를 해석합니다.
...
생성된 등록을 Codex, Claude Code/Cursor JSON, OpenClaw, Hermes 또는 Gajae-Code 세션과 함께 사용하는 MCP 클라이언트에 적용합니다. 직접 실행 확인:
...
| 헬퍼 | 용도 |
|---|---|
| `engine/cli.mjs` | `validate` / `smoke` / `live` 명령; 격리된 라이브 검증 실행. |
...
**프로바이더 (메타데이터 전용).** `engine/cli.mjs live --provider <id>`는 **메타데이터만** 선택합니다 — `naver`, `instagram`, `youtube`, `x`, `coupang`, `tiktok`, `generic`에 대한 도메인/출처 및 쿠키/프로필 힌트입니다. 프로바이더는 더 넓은 출처를 승인하거나 안전·정찰·프리플라이트 게이트를 우회하지 않으며, 알 수 없는 프로바이더는 즉시 실패(fail closed)합니다.
