...
test('bulk cap rejects oversized requests', () => {
  const under = enforceBulkCap(Array.from({ length: 3 }), { cap: 5 });
  assert.equal(under.allowed, true);
  const over = enforceBulkCap(Array.from({ length: 6 }), { cap: 5 });
  assert.equal(over.allowed, false);
  assert.equal(enforceBulkCap(Array.from({ length: DEFAULT_BULK_CAP + 1 })).allowed, false);
});
...
test('rate window persists across calls and blocks at the cap', async () => {
  ...
  for (let i = 0; i < 3; i += 1) {
    const r = await checkAndRecordAction(dir, 'like', {
      maxPerWindow: 3,
      windowMs: 1000,
      now: now + i
    });
  ...
  const blocked = await checkAndRecordAction(dir, 'like', {
    maxPerWindow: 3,
    windowMs: 1000,
    now: now + 4
  });
  ...
  const recovered = await checkAndRecordAction(dir, 'like', {
    maxPerWindow: 3,
    windowMs: 1000,
    now: now + 5000
  });
...
test('rate state rejects corrupt JSON and invalid shapes', async () => {
  ...
  await assert.rejects(checkAndRecordAction(corruptDir, 'like'), SyntaxError);

  const invalidDir = await tmpStateDir();
  await fs.writeFile(
    path.join(invalidDir, 'action-rate.json'),
    JSON.stringify({ like: 'not-an-array' }),
    'utf8'
  );
  await assert.rejects(checkAndRecordAction(invalidDir, 'like'), /invalid action-rate state/);
});
...
test('atomic rate persistence preserves concurrent action-type events', async () => {
  ...
  const actions = Array.from({ length: 12 }, (_, index) =>
    checkAndRecordAction(dir, index % 2 === 0 ? 'like' : 'comment', {
      maxPerWindow: 10,
      windowMs: 1000,
      now: 1_000_000_000_000 + index
    })
...
test('action results enforce exclusive terminal states and sanitized transport evidence', () => {
  ...
  assert.throws(
    () => makeActionResult({ ...base, alreadySatisfied: true, performed: true }),
    /alreadySatisfied/
  );
...
test('atomic guarded reservations enforce both rate and idempotency under concurrency', async () => {
  ...
  const store = JSON.parse(
    await fs.readFile(path.join(sameDir, 'guarded-actions-v1.json'), 'utf8')
  );
  assert.equal(store.rates['fixture-write'].length, 1);
  assert.equal(store.claims[hash('c')].state, 'pending');

  const rateStore = JSON.parse(
    await fs.readFile(path.join(dir, 'guarded-actions-v1.json'), 'utf8')
  );
  ...
  await Promise.all([
    reserveGuardedAction(
      mixedDir,
      reservationInput(1, { actionType: 'like', idempotencyHash: hash('1') })
    ),
    reserveGuardedAction(
      mixedDir,
      reservationInput(2, { actionType: 'comment', idempotencyHash: hash('2') })
    )
  ]);
  const mixedStore = JSON.parse(
    await fs.readFile(path.join(mixedDir, 'guarded-actions-v1.json'), 'utf8')
  );
...
test('guarded claims finalize, block replay, and require interactive reconciliation', async () => {
  ...
  assert.equal(
    (await reserveGuardedAction(dir, { ...input, now: input.now + 2 })).status,
    'claim-ambiguous'
  );
  await assert.rejects(
  ...
  assert.equal((await inspectGuardedAction(dir, input.idempotencyHash)).state, 'ambiguous');
  assert.equal(
    await reconcileGuardedAction(dir, {
      idempotencyHash: input.idempotencyHash,
      resolution: 'clear',
      interactive: true,
      confirmed: true
...
test('guarded-action store corruption fails closed', async () => {
  const dir = await tmpStateDir();
  await fs.writeFile(
    path.join(dir, 'guarded-actions-v1.json'),
    JSON.stringify({
      version: 1,
