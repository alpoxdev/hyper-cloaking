...
test(
  'cookie and CLI surfaces honor the canonical environment home when explicit home is absent',
  { concurrency: false },
  async () => {
...
test('credentials CLI imports only secure sources and returns redacted profiles', async () => {
  ...
  const unknownOption = await runJson([
    'credentials',
    'list',
    '--home',
    home,
...
test('Hermes MCP config renders config.yaml-compatible mcp_servers YAML and preserves command args', () => {
  ...
  assert.match(
    config.config,
    new RegExp(
      `      - ${JSON.stringify(process.execPath).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\n`
    )
  );
...
test('CloakBrowser JS options suppress no-sandbox warning flag', () => {
  ...
  assert.deepEqual(options.launchOptions.ignoreDefaultArgs, [
    '--enable-automation',
...
test('document navigation uses exact origins while subresources bypass the document guard', async () => {
  ...
  await installNavigationSafety(page, {
...
test('final redirected URL is checked even after an allowed request', async () => {
  ...
  await assert.rejects(
    () =>
      gotoAndClassify(page, 'about:blank', 'https://example.com/', {
        allowedOrigins: ['https://example.com'],
        maxRedirects: 2
      }),
    /unauthorized origin/
...
test('browser close rejection and timeout are explicit cleanup blockers', async () => {
  const rejected = await closeBrowserHandle(
    {
      async close() {
  ...
  assert.deepEqual(rejected, {
    ...
    closed: false,
    timedOut: false,
    blocker: 'close failed'
  });
  const timedOut = await closeBrowserHandle(
    {
      close() {
        return new Promise(() => {});
      }
...
test('injectable legacy live verification keeps the existing home evidence destination', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'hyper-live-'));
  const home = path.join(root, 'home');
  const result = await runLiveVerification(liveOptions(home), injectableLiveDependencies());
