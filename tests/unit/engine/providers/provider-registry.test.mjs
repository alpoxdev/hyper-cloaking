...
test('runtime resolution fails closed with provider-ambiguous-host if two providers overlap a host', () => {
  ...
  const resolution = resolveProviderForUrlFromRegistry(
    fixtureRegistry,
    'https://sub.shared-example.test'
  );
  assert.equal(resolution.ok, false);
  ...
  for (const provider of providers) {
    for (const key of Object.keys(provider)) {
      assert.ok(
        ...
        `unexpected top-level key "${key}" on provider "${provider.id}"`
      );
    }
    for (const key of Object.keys(provider.domains)) {
      assert.ok(
        allowedDomainKeys.has(key),
        `unexpected domains key "${key}" on provider "${provider.id}"`
      );
    }
  }
});
