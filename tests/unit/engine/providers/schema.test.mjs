...
test('guarded navigation blocks denied or unsafe targets before goto and verifies safe targets', async () => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error.code === 'HYPER_CLOAKING_TARGET_SAFETY' && error.classification === suppliedNonOk
  );
  assert.equal(gotoCalls, 0);
...
test('guarded navigation ignores challenge words outside targeted UI evidence', async () => {
  ...
  assert.equal(
    await session.navigateGuarded('https://public.test/comments'),
    'https://public.test/comments'
  );
});
  ...
  for (const scenario of [
    {
      ...
      cause: evaluatorCause,
      evaluate: async () => {
        throw evaluatorCause;
      }
    },
    ...
    await assert.rejects(
      ...
      (error) => {
        thrown = error;
        return (
          error.code === 'challenge-evidence-unavailable' &&
...
test('challenge capture ignores login affordances but detects login walls, modals, and alerts', async () => {
  for (const scenario of [
    ...
    if (scenario.blocks) {
      ...
      assert.equal(
        await session.navigateGuarded('https://public.test/path'),
...
test('forbidden field names are rejected anywhere in a provider object', () => {
  for (const field of FORBIDDEN_PROVIDER_FIELDS) {
    ...
    assert.ok(
      topResult.errors.some((error) => error.code === 'provider-forbidden-field'),
      field
    );
    ...
    assert.ok(
      nestedResult.errors.some((error) => error.code === 'provider-forbidden-field'),
      field
    );
  }
});
...
test('safe diagnostic-only wording passes but bypass instructions are rejected', () => {
  const safe = {
    ...
    id: 'safe-wording',
    safeFlowNotes: [
      'If a CAPTCHA or WAF block appears, stop and report it as a diagnostic blocker.'
    ]
  };
  assert.equal(
    validateProviderSchema(safe).ok,
    true,
    JSON.stringify(validateProviderSchema(safe).errors)
  );
