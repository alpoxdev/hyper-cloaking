...
function toggleSession(
  stateDir,
  { active = false, clickError = null, onSelector = naverSelectors.blog.unlike } = {}
) {
  let current = active;
  const locator = (selector) => ({
    async count() {
      return selector === onSelector ? (current ? 1 : 0) : 1;
    },
    first() {
      return this;
    }
...
test('Naver refs canonicalize owned identities and reject off-origin, nid, or incoherent refs', () => {
  ...
  assert.equal(
    normalizeBlogPostRef({
      blogId: 'fixture',
      logNo: '1',
      url: 'https://blog.naver.com/fixture/2'
    }),
    null
  );
  ...
  assert.deepEqual(normalizeBlogRef('fixture'), {
    blogId: 'fixture',
    url: 'https://blog.naver.com/fixture'
  });
  assert.deepEqual(normalizeCafeRef('fixture'), {
    cafeId: 'fixture',
    url: 'https://cafe.naver.com/fixture'
  });
...
test('Naver forced reads preserve bounded canonical envelopes and require explicit empty-state evidence', async () => {
  const web = await searchWeb({}, 'fixture', {
    readStrategy: 'direct',
    readHandlers: {
      direct: async () => ({ results: [{ url: 'https://example.com/a', title: 'A' }] })
    }
  });
  ...
  const blogSearch = await searchBlog({}, 'fixture', {
    ...
    readHandlers: {
      direct: async () => ({
        results: [{ url: 'https://blog.naver.com/fixture/1', title: 'Post' }]
      })
    }
  });
  ...
  await assert.rejects(
    searchCafe({}, 'fixture', {
      readStrategy: 'direct',
      readHandlers: {
        direct: async () => ({ results: [{ url: 'https://evil.example/fixture/1' }] })
      }
    }),
  ...
  await assert.rejects(
    getCafePost(
      {},
      { cafeId: 'fixture', articleId: '1' },
      {
        readStrategy: 'direct',
        readHandlers: {
          direct: async () => ({ present: true, title: 'T', body: 'B', comments: [] })
        }
      }
    ),
    /explicit empty-state/
  ...
  const blogList = await getBlogList({}, 'fixture', {
    readStrategy: 'direct',
    readHandlers: {
      direct: async () => ({ posts: [{ blogId: 'fixture', logNo: '2', title: 'X' }] })
    }
  });
...
test('Naver writes block before navigation unless dry-run, enable, and state gates pass', async () => {
  ...
  const disabled = await setBlogPostLiked(noNavigation, { blogId: 'fixture', logNo: '1' }, true, {
    dryRun: false
  });
  assert.match(disabled.reason, /enableBlogLike/);

  const missingState = await setBlogPostLiked(
    noNavigation,
    { blogId: 'fixture', logNo: '1' },
    true,
    {
  ...
  const missingRunId = await setBlogPostLiked(
    { stateDir: '/tmp/does-not-matter', ...noNavigation },
    { blogId: 'fixture', logNo: '1' },
    true,
    {
      dryRun: false,
...
test('Naver desired writes reserve atomically, no-op on already-satisfied state, and block ambiguous replay', async () => {
  ...
  try {
    ...
    const uncertain = await setBlogPostLiked(
      uncertainSession,
      { blogId: 'fixture', logNo: '2' },
      true,
      {
        dryRun: false,
        enableBlogLike: true,
    ...
    const replay = await setBlogPostLiked(
      uncertainSession,
      { blogId: 'fixture', logNo: '2' },
      true,
      {
...
test('Naver cafe writes require current membership and write permission before dispatch', async () => {
  ...
  try {
    ...
    const blockedNoMembership = await setCafePostLiked(
      notMember,
      { cafeId: 'fixture', articleId: '1' },
      true,
      {
    ...
    const blockedNoPermission = await setCafePostLiked(
      noWritePermission,
      { cafeId: 'fixture', articleId: '1' },
      true,
      {
...
test('Naver post/comment content schemas are closed and reject unsupported fields', async () => {
  ...
  try {
    ...
    const badPost = await createBlogDraft(
      noNavigation,
    ...
    const badVisibility = await createBlogDraft(
      noNavigation,
      { title: 'T', body: 'B', visibility: 'everyone' },
      {
        dryRun: false,
        enableBlogDraft: true,
        runId: 'draft-2'
    ...
    const badComment = await commentBlogPost(
      noNavigation,
      { blogId: 'fixture', logNo: '1' },
      { text: 'hi', extra: 'nope' },
      {
...
test('Naver media schema validates count, size, extension/magic match, and symlink safety', async () => {
  ...
  try {
    ...
    const symlinked = await createBlogDraft(
      noNavigation,
      {
        title: 'T',
        body: 'B',
    ...
    const missing = await createBlogDraft(
      noNavigation,
      {
        title: 'T',
        body: 'B',
        media: [path.join(mediaDir, 'missing.jpg')]
      },
...
test('Naver createCafePost requires bulk confirmation and current membership before dispatch', async () => {
  ...
  try {
    ...
    const blockedConfirm = await createCafePost(
      notConfirmed,
      'fixture',
      { title: 'T', body: 'B' },
      {
...
test('Naver createBlogDraft applies the requested visibility and proves a new draft ID', async () => {
  ...
  try {
    ...
    const result = await createBlogDraft(
      session,
      { title: 'T', body: 'B', visibility: 'private' },
      {
        dryRun: false,
        enableBlogDraft: true,
...
test('Naver createCafePost applies the requested visibility and proves a new article ID', async () => {
  ...
  try {
    ...
    const result = await createCafePost(
      session,
      'fixture',
      { title: 'T', body: 'B', visibility: 'private' },
      {
