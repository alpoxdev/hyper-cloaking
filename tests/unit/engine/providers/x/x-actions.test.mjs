...
function toggleSession(stateDir, { active = false, clickError = null } = {}) {
  ...
  const locator = (selector) => ({
    async count() {
      return selector.includes('unlike') || selector.includes('following') ? (current ? 1 : 0) : 1;
    },
    first() {
      return this;
    },
...
test('X metadata includes the bare x.com origin alongside existing hosts and stays registry-safe', () => {
  ...
  assert.deepEqual(xProvider.domains.aliases, [
    'www.x.com',
    'twitter.com',
    'www.twitter.com',
    'mobile.twitter.com'
...
test('X refs canonicalize owned identities and reject cold, off-origin, or incoherent refs', () => {
  ...
  assert.equal(
    normalizePostRef({ handle: 'fixture', postId: '1', url: 'https://x.com/fixture/status/2' }),
    null
  );
  assert.equal(
    normalizeThreadRef(
      { threadId: '1', accountId: 'acct', url: 'https://x.com/messages/2' },
      { accountId: 'acct' }
    ),
    null
  );
  ...
  const thread = await getThread(
    ...
    { handle: 'fixture', postId: '789' },
    {
      ...
      readHandlers: {
        direct: async () => ({
          posts: [
            { handle: 'fixture', postId: '790' },
            { handle: 'fixture', postId: '791' }
          ]
        })
      }
    }
  ...
  const threads = await listDMThreads(
    { accountId: 'acct' },
    {
      ...
      readHandlers: {
        direct: async () => ({
          threads: [
            { accountId: 'acct', threadId: 'thread-1', url: 'https://x.com/messages/thread-1' }
          ]
        })
      }
    }
...
test('X desired writes are dry-run default, require exact enable/state, reserve atomically, and replay ambiguous claims', async () => {
  ...
  const disabled = await setLiked(noNavigation, { handle: 'fixture', postId: '123' }, true, {
    dryRun: false
  });
...
test('X media schema enforces closed modes, size limits, and pre-dispatch TOCTOU re-checks', async () => {
  ...
  try {
    ...
    try {
      const tampered = await createPost(
        composeSession(stateDir, { postId: '1002' }),
        'toctou-bad',
        {
          dryRun: false,
          enablePost: true,
