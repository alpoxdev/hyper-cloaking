...
function normalizePostEntry(entry) {
  if (!entry || typeof entry !== 'object' || Array.isArray(entry)) throw new TypeError('X post entries must be objects');
...
function normalizePostList(value, { owner = null, query = null, limit, dedupe = true }) {
  ...
  for (const entry of value.posts) {
    ...
    if (owner && post.handle !== owner.handle) throw new TypeError('X user post does not belong to the requested user');
    if (dedupe) {
  ...
  return { ...(owner ? { user: owner } : {}), ...(query !== null ? { query } : {}), count: posts.length, posts };
}
...
async function extractPostLinks(session, selector) {
  return session.page.$$eval(selector, (nodes) => nodes.slice(0, 400).map((node) => ({
    ...
    text: node.closest('article')?.querySelector('[data-testid="tweetText"]')?.textContent?.trim() || null,
...
export async function getUser(session, userRef, opts = {}) {
  ...
  const { value } = await readOptions(opts, 'getUser', dom, (content) => {
    if (!content || typeof content !== 'object' || Array.isArray(content)) throw new TypeError('X user content must be an object');
    if (content.present !== true) throw new TypeError('X user content requires explicit presence evidence');
    return {
      ...user,
...
export async function getUserPosts(session, userRef, opts = {}) {
  ...
  const dom = async () => {
    ...
    if (posts.length === 0 && await session.page.locator(xSelectors.user.emptyState).count() === 0) {
      throw new Error('X user post state could not be proven');
    }
    return { posts, emptyState: posts.length === 0 };
  };
  const { value } = await readOptions(opts, 'getUserPosts', dom, (content) => normalizePostList(content, { owner: user, limit }));
...
export async function getPost(session, postRef, opts = {}) {
  ...
  const dom = async () => {
    ...
    const replies = includeReplies
      ? await session.page.$$eval(xSelectors.post.replyText, (nodes) => nodes.slice(1, 401).map((node) => node.textContent?.trim()).filter(Boolean))
      : [];
    const repliesEmptyState = !includeReplies || (
      replies.length === 0
  ...
  const { value } = await readOptions(opts, 'getPost', dom, (content) => {
    if (!content || typeof content !== 'object' || !Array.isArray(content.replies) || content.replies.length > MAX_RAW) {
      throw new TypeError(`X post content must contain at most ${MAX_RAW} replies`);
    }
    if (content.present !== true) throw new TypeError('X post content requires explicit presence evidence');
    ...
    return {
      ...
      replies: includeReplies
        ? content.replies.slice(0, limit).map((reply) => boundedText(reply, 'reply', 10_000)).filter(Boolean)
        : []
    };
  });
...
export async function searchPosts(session, query, opts = {}) {
  ...
  const dom = async () => {
    ...
    if (posts.length === 0 && await session.page.locator(xSelectors.search.emptyState).count() === 0) {
      throw new Error('X search state could not be proven');
    }
    return { posts, emptyState: posts.length === 0 };
  };
  const { value } = await readOptions(opts, 'searchPosts', dom, (content) => normalizePostList(content, { query: searchQuery, limit }));
...
export async function getThread(session, postRef, opts = {}) {
  ...
  const dom = async () => {
    await session.navigateGuardedForRead(post.url, { waitUntil: 'domcontentloaded', timeout: 30_000 });
    const posts = await extractPostLinks(session, xSelectors.thread.article + ' a[href*="/status/"]');
    if (posts.length === 0 && await session.page.locator(xSelectors.thread.emptyState).count() === 0) {
      throw new Error('X thread state could not be proven');
    }
    return { posts, emptyState: posts.length === 0 };
  };
  const { value } = await readOptions(opts, 'getThread', dom, (content) => normalizePostList(content, { owner: post, limit }));
...
export async function listDMThreads(session, opts = {}) {
  ...
  const limit = limitFor(opts.limit, 20);
  const dom = async () => {
    await session.navigateGuardedForRead(xSelectors.dm.inboxUrl, { waitUntil: 'domcontentloaded', timeout: 30_000 });
    const hrefs = await session.page.$$eval(xSelectors.dm.threadLink, (nodes) => nodes.slice(0, 400).map((node) => node.getAttribute('href')).filter(Boolean));
    if (hrefs.length === 0 && await session.page.locator(xSelectors.dm.emptyInbox).count() === 0) {
  ...
  const { value } = await readOptions(opts, 'listDMThreads', dom, (content) => {
    ...
    if (threads.length === 0 && content.emptyState !== true) throw new TypeError('X empty DM list requires explicit empty-state evidence');
...
export async function readDMThread(session, threadRef, opts = {}) {
  ...
  const dom = async () => {
    ...
    const messages = await session.page.$$eval(xSelectors.dm.message, (nodes) => nodes.slice(-100).map((node) => ({
    ...
    if (messages.length === 0 && await session.page.locator(xSelectors.dm.emptyThread).count() === 0) {
  ...
  const { value } = await readOptions(opts, 'readDMThread', dom, (content) => {
    ...
    if (content.messages.length === 0 && content.emptyState !== true) throw new TypeError('X empty DM thread requires explicit empty-state evidence');
