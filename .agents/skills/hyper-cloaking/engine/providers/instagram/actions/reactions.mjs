...
function finishWrite(action, observation, criteriaExpected, {
  rate = null,
...
export async function commentPost(session, postRef, text, opts = {}) {
  ...
  const matchingCount = () => session.page.$$eval(
  ...
  await session.page.waitForFunction(
    ({ selector, needle, baseline }) => [...document.querySelectorAll(selector)]
      .filter((node) => (node.textContent || '').trim() === needle).length > baseline,
    {
...
export async function sharePost(session, postRef, opts = {}) {
  ...
  const sheetOpen = await session.page.locator(instagramSelectors.posts.shareDialog).count() > dialogBaseline;
  return finishWrite(
    action,
