...
import { applyMirrorSync, captureMirrorBaseline, prepareMirrorSync, recoverMirrorSync } from './sync-mirror.mjs';
...
async function trees() {
  ...
  return { root, canonicalDir, mirrorDirs, recoveryRoot: path.join(root, 'recovery'), manifest: ['owned.txt', 'new.txt'] };
}

test('excluded canonical-local state does not break initial parity', async () => {
  const f = await trees();
  const baseline = await captureMirrorBaseline({ canonicalDir: f.canonicalDir, mirrorDirs: f.mirrorDirs, ownedManifest: f.manifest });
...
test('prepares and applies owned copies with a durable pre-write journal', async () => {
  ...
  const result = await applyMirrorSync({ baseline, prepared, recoveryRoot: f.recoveryRoot, lockPath: path.join(f.root, 'sync.lock') });
...
test('prepared record tampering is rejected before writes', async () => {
  ...
  await fsp.writeFile(path.join(f.canonicalDir, 'owned.txt'), 'after\n');
  const prepared = await prepareMirrorSync({ baseline });
  await assert.rejects(() => applyMirrorSync({ baseline, prepared: { ...prepared, manifestSha256: 'bad' }, recoveryRoot: f.recoveryRoot, lockPath: path.join(f.root, 'sync.lock') }), /does not match/);
});
...
test('recovery refuses token mismatch', async () => {
  ...
  await fsp.writeFile(journalPath, `${JSON.stringify({ event: 'header', invocationToken: 'expected', operations: [], mirrorRealpaths: [] })}\n`);
  await assert.rejects(() => recoverMirrorSync({ journalPath, invocationToken: 'other' }), /token mismatch/);
});

test('manifest rejects globs and duplicates', async () => {
  const f = await trees();
  await assert.rejects(() => captureMirrorBaseline({ canonicalDir: f.canonicalDir, mirrorDirs: f.mirrorDirs, ownedManifest: ['*.mjs'] }), /invalid/);
  await assert.rejects(() => captureMirrorBaseline({ canonicalDir: f.canonicalDir, mirrorDirs: f.mirrorDirs, ownedManifest: ['owned.txt', 'owned.txt'] }), /duplicate/);
});
...
test('explicit recovery handles commit-intent interruption and verified stale lock', async () => {
  ...
  const interrupted = events.filter((event) => !['commit-complete', 'complete'].includes(event.event));
  await fsp.writeFile(applied.journalPath, `${interrupted.map((event) => JSON.stringify(event)).join('\n')}\n`);
...
test('recovery preserves mismatched user bytes and reports manual recovery', async () => {
  ...
  assert.match(
    await fsp.readFile(path.join(f.mirrorDirs[0], 'owned.txt'), 'utf8'),
    /user changed/
  );
