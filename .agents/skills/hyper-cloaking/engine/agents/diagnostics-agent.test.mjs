...
function setupEnvelope(failure = { code: 'missing-binary', phase: 'setup', retryable: true, observedSignal: 'binary missing' }) {
  return { schemaVersion: 1, agent: 'setup', status: 'blocked', executionMode: 'parent', failure, result: { agentType: 'setup', setupStatus: 'needs_install', mcpConfig: null, executablePath: null, blockers: [] } };
}
...
test('classifies setup failures and returns in-memory reports', async () => {
  ...
  const result = await runDiagnostics({ schemaVersion: 1, lastAgentOutput: setupEnvelope(), logPaths: ['run.log'], screenshotPaths: [], stateDir });
...
test('maps target safety to clarification and WAF to manual review', async () => {
  const stateDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'diagnostics-'));
  const target = await runDiagnostics({ schemaVersion: 1, lastAgentOutput: setupEnvelope({ code: 'origin-not-allowed', phase: 'target-safety', retryable: false, observedSignal: 'off origin' }), logPaths: [], screenshotPaths: [], stateDir });
  assert.equal(target.result.nextAuthorizedStep, 'clarify_scope');
  const waf = await runDiagnostics({ schemaVersion: 1, lastAgentOutput: setupEnvelope({ code: 'waf-challenge', phase: 'browser', retryable: false, observedSignal: 'challenge' }), logPaths: [], screenshotPaths: [], stateDir });
...
test('rejects traversal and unknown input without writing', async () => {
  const stateDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'diagnostics-'));
  const result = await runDiagnostics({ schemaVersion: 1, lastAgentOutput: setupEnvelope(), logPaths: ['../secret'], screenshotPaths: [], stateDir });
...
test('reports bounded screenshot metadata without decoding binary as text', async () => {
  const stateDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'diagnostics-'));
  await fsp.writeFile(path.join(stateDir, 'page.png'), Buffer.from([0x89, 0x50, 0x4e, 0x47, 0, 0xff]));
  const result = await runDiagnostics({
    schemaVersion: 1,
    lastAgentOutput: setupEnvelope(),
    logPaths: [],
