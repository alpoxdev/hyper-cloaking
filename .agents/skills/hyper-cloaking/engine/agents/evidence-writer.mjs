...
export async function persistEvidence({
  ...
  if (!homeDir || !path.isAbsolute(homeDir)) throw new Error('homeDir must be an absolute parent-authorized path');
  const homeRoot = await secureRealDirectory(homeDir, 'homeDir');
  if (evidenceRefs.length > 0 && !cleanupSucceeded(cleanup)) throw new Error('browser cleanup is not verified; evidence publication is forbidden');

  const agentRoot = await secureRealDirectory(agentStagingRoot, 'agentStagingRoot');
  const privateRoot = await secureRealDirectory(parentPrivateStagingRoot, 'parentPrivateStagingRoot');
  if (agentRoot === privateRoot || isInside(agentRoot, privateRoot) || isInside(privateRoot, agentRoot)) throw new Error('agent and parent-private staging roots must be separate');
  ...
  for (const ref of evidenceRefs) {
    ...
    if (destinations.has(destination)) throw new Error(`duplicate evidence destination: ${destination}`);
    sources.add(relPath);
  ...
  if (diagnosticReport !== null) publish.push(await generatedFile(privateRoot, 'diagnostics-report.json', diagnosticReport, destinations, redactSecrets));
  if (failure !== null) publish.push(await generatedFile(privateRoot, 'failure-report.json', failure, destinations, redactSecrets));
  ...
  try {
    ...
    for (const item of publish) {
      ...
      if (!isInside(finalDir, destination)) throw new Error('evidence destination escapes final directory');
      ...
      if (!stat.isFile() || stat.isSymbolicLink()) throw new Error('published evidence is not a regular file');
      const digest = await hashFile(destination);
      if (digest !== item.sha256) throw new Error(`published evidence hash mismatch: ${item.relativePath}`);
    }
  ...
  const cleanupErrors = cleanupStaging
    ? await cleanupOwnedStaging([agentRoot, privateRoot])
    : [];
  if (publicationError) {
...
export async function recoverEvidencePublication({ finalDir, invocationToken, removeIncomplete = false }) {
  ...
  for (const relPath of actual) {
    ...
    if (!stat.isFile() || stat.isSymbolicLink()) throw new Error(`publication entry is not a regular file: ${relPath}`);
...
function validatePublicationMarker(marker, finalDir, invocationToken) {
  ...
  if (path.basename(path.resolve(finalDir)) !== marker.evidenceId) throw new Error('publication namespace does not match evidenceId');
  if (!['reserved', 'publishing', 'complete'].includes(marker.state)) throw new Error('invalid publication marker state');
  ...
  for (const file of marker.files) {
    ...
    if (relativePath !== file.relativePath) throw new Error(`publication marker path is not canonical: ${file.relativePath}`);
    if (expected.has(relativePath)) throw new Error(`duplicate publication marker path: ${relativePath}`);
    if (!['agent', 'parent'].includes(file.sourceScope)) throw new Error(`invalid publication source scope: ${relativePath}`);
  ...
  if (marker.manifestSha256 !== manifestSha256) throw new Error('publication marker manifest digest mismatch');
  return expected;
}
...
async function generatedFile(privateRoot, name, value, destinations, redactSecrets) {
  ...
  return { source: file, relativePath: name, sourceScope: 'parent', type: 'json', description: name, sha256: await hashFile(file), size: Buffer.byteLength(serialized) };
}
...
async function secureSource(root, relPath) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) throw new Error('evidence source must be a regular non-symlink file');
  return source;
}
...
async function ensureSecureDirectory(directory) {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) throw new Error('evidence root is not a secure directory');
}
...
function validateRelative(value) {
  if (typeof value !== 'string' || !value || path.isAbsolute(value)) throw new Error('evidence path must be relative');
  const normalized = path.normalize(value);
  const segments = normalized.split(path.sep);
  if (normalized === '..' || normalized.startsWith(`..${path.sep}`) || segments.some((segment) => RESERVED_SEGMENTS.has(segment.toLowerCase()))) throw new Error(`unsafe evidence path: ${value}`);
  return normalized;
}

function validateUuid(value, label) {
  if (typeof value !== 'string' || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value)) throw new Error(`${label} must be a canonical UUID`);
}

function markerValue(state, evidenceId, invocationToken, publish) {
  const files = publish.map((item) => ({ relativePath: item.relativePath, sourceScope: item.sourceScope, sha256: item.sha256, size: item.size ?? fs.statSync(item.source).size }));
  return { version: 1, evidenceId, invocationToken, state, manifestSha256: sha256(Buffer.from(stableStringify(files))), files, updatedAt: new Date().toISOString() };
}
...
function redact(value) {
  ...
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, /(cookie|token|password|credential|authorization)/i.test(key) ? '[REDACTED]' : redact(item)]));
  }
  return value;
}
...
async function hashFile(file) { return sha256(await fsp.readFile(file)); }
