...
const FORBIDDEN_ACTIONS = new Set([
  'account', 'security', 'ads', 'moderation', 'delete',
  'followerScraping', 'protectedBypass', 'bulk', 'coldDM'
]);
const REPLY_AUDIENCE = new Set(['everyone', 'following', 'mentioned', 'verified']);
const TEXT_MAX = 280;
const IMAGE_EXTENSIONS = { '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.png': 'image/png', '.webp': 'image/webp' };
...
async function desiredToggle(session, {
  ...
  if (await session.page.locator(selector).count() !== 1) return block(action, 'owned X transition control is not unique', 'selector-ownership');
...
export async function setLiked(session, postRef, liked, opts = {}) {
  ...
  let post;
  try { post = assertPostRef(postRef); } catch (error) { return invalid(action, error, 'post-ref-validation'); }
  return desiredToggle(session, {
    action, target: post.url, desired: liked, opts, enableKey: 'enableLike',
    rateKey: 'x-like', maxPerWindow: 10,
    offSelector: xSelectors.post.like, onSelector: xSelectors.post.unlike,
...
export async function setBookmarked(session, postRef, bookmarked, opts = {}) {
  ...
  try { post = assertPostRef(postRef); } catch (error) { return invalid(action, error, 'post-ref-validation'); }
  return desiredToggle(session, {
    action, target: post.url, desired: bookmarked, opts, enableKey: 'enableBookmark',
    rateKey: 'x-bookmark', maxPerWindow: 10,
    offSelector: xSelectors.post.bookmark, onSelector: xSelectors.post.unbookmark,
    content: { postId: post.postId }
  });
}
...
export async function setFollowing(session, userRef, following, opts = {}) {
  ...
  return desiredToggle(session, {
    ...
    rateKey: 'x-follow', maxPerWindow: 5,
    offSelector: xSelectors.post.follow, onSelector: xSelectors.post.following,
...
export async function setReposted(session, postRef, reposted, opts = {}) {
  ...
  try { post = assertPostRef(postRef); } catch (error) { return invalid(action, error, 'post-ref-validation'); }
  return desiredToggle(session, {
    action, target: post.url, desired: reposted, opts, enableKey: 'enableRepost',
    rateKey: 'x-repost', maxPerWindow: 5,
    offSelector: xSelectors.post.repost, onSelector: xSelectors.post.unrepost,
    content: { postId: post.postId }
  });
}
...
function classifyExtension(extension) {
  if (Object.hasOwn(IMAGE_EXTENSIONS, extension)) return { kind: 'image', mimeType: IMAGE_EXTENSIONS[extension] };
  if (Object.hasOwn(GIF_EXTENSIONS, extension)) return { kind: 'gif', mimeType: GIF_EXTENSIONS[extension] };
  if (Object.hasOwn(VIDEO_EXTENSIONS, extension)) return { kind: 'video', mimeType: VIDEO_EXTENSIONS[extension] };
  return null;
}
...
async function validateMediaFile(file) {
  ...
  if (!classified) throw new TypeError('X media extension must be jpg, jpeg, png, webp, gif, mp4, or mov');
...
async function validatePostMedia(media) {
  ...
  if (files.length > IMAGE_MAX_COUNT) throw new TypeError(`X posts accept at most ${IMAGE_MAX_COUNT} media items`);
  ...
  if (mode === 'image') {
    for (const item of items) {
      if (item.size > IMAGE_MAX_BYTES) throw new TypeError(`X image "${item.name}" exceeds ${IMAGE_MAX_BYTES} bytes`);
    }
    const total = items.reduce((sum, item) => sum + item.size, 0);
    if (total > IMAGE_TOTAL_MAX_BYTES) throw new TypeError(`X image set exceeds ${IMAGE_TOTAL_MAX_BYTES} bytes total`);
  } else if (mode === 'gif') {
    ...
    if (items[0].size > VIDEO_MAX_BYTES) throw new TypeError(`X video exceeds ${VIDEO_MAX_BYTES} bytes`);
  }
...
async function revalidateMedia(items) {
  for (const item of items) {
    ...
    if (fresh.dev !== item.dev || fresh.ino !== item.ino || fresh.fileHash !== item.fileHash) {
      throw new TypeError(`X media "${item.name}" changed identity between validation and dispatch`);
    }
  }
}
...
async function setComposerFiles(session, items) {
  ...
  await session.page.locator(xSelectors.compose.mediaInput).setInputFiles(items.map((item) => ({
    name: item.name,
...
async function newConfirmedPostId(session, before) {
  const ids = await session.page.locator('main [data-testid="toast"] [data-post-id]').evaluateAll((nodes) => nodes
...
export async function createPost(session, text, opts = {}) {
  ...
  let expected;
  try { expected = normalizeText(text, 'X post text', TEXT_MAX); } catch (error) { return block(action, error.message, 'input-validation'); }
  let audience;
  try { audience = normalizeReplyAudience(opts.replyAudience); } catch (error) { return block(action, error.message, 'input-validation'); }
  const blocked = validateLive(session, action, opts, 'enablePost');
  if (blocked) return blocked;
  let media;
  try { media = await validatePostMedia(opts.media); } catch (error) { return block(action, error.message, 'media-validation'); }
  await session.navigateGuardedForWrite(xSelectors.compose.url, { waitUntil: 'domcontentloaded', timeout: 30_000 });
  ...
  const before = new Set(await session.page.locator('main [data-testid="toast"] [data-post-id]').evaluateAll((nodes) => nodes
    .map((node) => node.getAttribute('data-post-id'))
    .filter((id) => /^\d{1,32}$/.test(id))));

  return guarded(session, {
    action, rateKey: 'x-post', maxPerWindow: 5, target: xSelectors.compose.url,
    content: { text: expected, audience, mediaHashes: media.items.map((item) => item.fileHash) }, runId: opts.runId,
    dispatch: async () => {
    ...
    verify: () => waitFor(session.page, async () => (await newConfirmedPostId(session, before)) !== null),
...
export async function replyToPost(session, postRef, text, opts = {}) {
  ...
  try { post = assertPostRef(postRef); } catch (error) { return invalid(action, error, 'post-ref-validation'); }
  let expected;
  try { expected = normalizeText(text, 'X reply text', TEXT_MAX); } catch (error) { return block(action, error.message, 'input-validation'); }
  let audience;
  try { audience = normalizeReplyAudience(opts.replyAudience); } catch (error) { return block(action, error.message, 'input-validation'); }
  ...
  let media;
  try { media = await validatePostMedia(opts.media); } catch (error) { return block(action, error.message, 'media-validation'); }
  await session.navigateGuardedForWrite(post.url, { waitUntil: 'domcontentloaded', timeout: 30_000 });
  ...
  return guarded(session, {
    action, rateKey: 'x-reply', maxPerWindow: 5, target: post.url,
    content: { postId: post.postId, text: expected, audience, mediaHashes: media.items.map((item) => item.fileHash) }, runId: opts.runId,
    dispatch: async () => {
    ...
    verify: () => waitFor(session.page, async () => await exactTextCount(session.page, xSelectors.post.replyText, expected) > before),
    successText: 'reply created (state verified)'
  });
}

async function exactTextCount(page, selector, text) {
  return page.$$eval(selector, (nodes, expectedText) => nodes.filter((node) => (node.textContent || '').normalize('NFKC').replace(/\s+/gu, ' ').trim() === expectedText).length, text);
}
...
export async function quotePost(session, postRef, text, opts = {}) {
  ...
  try { post = assertPostRef(postRef); } catch (error) { return invalid(action, error, 'post-ref-validation'); }
  let expected;
  try { expected = normalizeText(text, 'X quote text', TEXT_MAX); } catch (error) { return block(action, error.message, 'input-validation'); }
  let audience;
  try { audience = normalizeReplyAudience(opts.replyAudience); } catch (error) { return block(action, error.message, 'input-validation'); }
  ...
  if (await trigger.count() !== 1) return block(action, 'owned X quote trigger is not unique', 'selector-ownership');
  ...
  return guarded(session, {
    action, rateKey: 'x-quote', maxPerWindow: 5, target: post.url,
    content: { quotedPostId: post.postId, text: expected, audience, mediaHashes: media.items.map((item) => item.fileHash) }, runId: opts.runId,
    dispatch: async () => {
...
async function messageRows(page) {
  return page.$$eval(xSelectors.dm.message, (nodes) => nodes.slice(-100).map((node) => ({
...
export async function replyToDM(session, threadRef, text, opts = {}) {
  ...
  let expected;
  try { expected = normalizeText(text, 'X DM', 10_000); } catch (error) { return block(action, error.message, 'input-validation'); }
  ...
  return guarded(session, {
    action, rateKey: 'x-dm', maxPerWindow: 5, target: thread.url,
...
export function blockedXAction(actionName) {
  ...
  if (!FORBIDDEN_ACTIONS.has(name)) throw new TypeError(`unsupported X structural blocker: ${name}`);
  return block(`x:${name}`, `${name} is structurally blocked for X automation`, 'structural-blocker');
}
