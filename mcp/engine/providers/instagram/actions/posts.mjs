...
function normalizePostsContent(value, { username, limit, includeReels }) {
  ...
  for (const entry of value.posts) {
    if (!entry || typeof entry !== 'object') throw new TypeError('Instagram post entries must be objects');
    let parsed;
    try {
...
export async function getUserPosts(session, username, opts = {}) {
  ...
  const dom = async () => {
    ...
    for (let pass = 0; pass < 6; pass += 1) {
      hrefs = await session.page.$$eval(
        ...
        (nodes) => nodes.slice(0, 400).map((node) => node.getAttribute('href')).filter(Boolean)
      );
  ...
  const { value } = await executeInstagramRead({
    ...
    dom,
    normalize: (content) => normalizePostsContent(content, {
      username: normalizedUsername,
      limit,
      includeReels
    })
