...
function serializableCause(error, depth = 0) {
  if (depth >= 3 || error == null || (typeof error !== 'object' && typeof error !== 'function')) {
    return error == null ? null : { name: 'NonErrorCause', message: String(error), code: null, cause: null };
  }
...
async function exactCommentCount(page, selector, expected) {
  return page.$$eval(selector, (nodes, needle) => nodes
    .filter((node) => (node.textContent || '').normalize('NFKC').replace(/\s+/gu, ' ').trim() === needle)
    .length, expected);
}
...
async function beginWrite(session, action, target, opts, rateKey) {
  ...
  try {
    ...
    if (!rate.allowed) {
      return { blocked: makeBlockedResult(action, rate.reason, { stage: 'rate-limit', rateLimit: rate }) };
    }
  } catch (error) {
    return {
...
async function finishWrite(session, action, observation, expected, verified, rateLimit) {
  ...
  return makeActionResult({
    ...
    performed: passed,
    changed: passed,
    failure: passed ? null : makeFailureDiagnostic({
...
export async function commentVideo(session, videoRef, text, opts = {}) {
  ...
  try {
    ...
    const verified = await waitForState(session.page, async () => {
      ...
      return threads
        ? (await exactCommentCount(session.page, threads, expected)) > before
        : false;
    });
    return finishWrite(
      session,
...
export async function subscribeChannel(session, channelRef, opts = {}) {
  ...
  if (opts.enableSubscribe !== true || opts.dryRun !== false) {
    return makeBlockedResult(action, 'YouTube subscribe is disabled by default; pass enableSubscribe:true and dryRun:false to act', {
  ...
  try {
    ...
    return finishWrite(
      session,
      action,
      { text: verified ? 'channel subscribed (state verified)' : 'subscription not verified', url: target },
...
export async function shareVideo(_session, videoRef, _opts = {}) {
  ...
  return makeBlockedResult(action, 'YouTube share is unsupported: recipient and destination require an explicit user decision', {
...
export async function saveToPlaylist(_session, videoRef, _opts = {}) {
  ...
  return makeBlockedResult(action, 'YouTube save to playlist is unsupported: playlist selection requires an explicit user decision', {
    stage: 'unsupported-native-action',
    requiresUserDecision: true
  });
}
