...
const CAFE_VISIBILITY_OPTION = Object.freeze({ public: 'all', neighbor: 'member', private: 'private' });
...
function validateCommentPayload(value) {
  ...
  if (value && typeof value === 'object' && !Array.isArray(value)) {
    ...
    if (keys.length !== 1 || keys[0] !== 'text') throw new TypeError('Naver comment payload must contain only a text field');
    text = value.text;
  }
...
function validatePostContent(content) {
  ...
  for (const key of Object.keys(content)) {
    if (!POST_CONTENT_KEYS.has(key)) throw new TypeError(`Naver post content has unsupported field "${key}"`);
  }
  ...
  if (!Array.isArray(media)) throw new TypeError('Naver post media must be an array of local file paths');
...
async function validateLocalImages(files) {
  if (!Array.isArray(files)) throw new TypeError('Naver media must be an array of local file paths');
  if (files.length > IMAGE_MAX_COUNT) throw new TypeError(`Naver media must contain at most ${IMAGE_MAX_COUNT} images`);
  if (files.length === 0) return [];
  if (!Number.isInteger(fsConstants.O_NOFOLLOW)) throw new TypeError('Naver media upload requires O_NOFOLLOW support');
...
async function requireCafeMembership(session, action, cafeUrl) {
  await session.navigateGuardedForWrite(cafeUrl, { waitUntil: 'domcontentloaded', timeout: 30_000 });
  const membership = session.page.locator(naverSelectors.cafe.membershipBadge);
  if (await membership.count() !== 1) return block(action, 'current Naver cafe membership could not be proven', 'membership-required');
  const writePermission = session.page.locator(naverSelectors.cafe.writePermission);
  if (await writePermission.count() !== 1) return block(action, 'Naver cafe write permission could not be proven', 'membership-required');
  return null;
}

async function desiredToggle(session, {
  action, target, desired, opts, enableKey, rateKey, maxPerWindow, offSelector, onSelector, content, membershipUrl
}) {
  ...
  if (await session.page.locator(selector).count() !== 1) return block(action, 'owned Naver transition control is not unique', 'selector-ownership');
...
export async function setBlogPostLiked(session, blogPostRef, liked, opts = {}) {
  ...
  let post;
  try { post = assertBlogPostRef(blogPostRef); } catch (error) { return invalid(action, error, 'blog-post-ref-validation'); }
  return desiredToggle(session, {
    action, target: post.url, desired: liked, opts, enableKey: 'enableBlogLike',
    rateKey: 'naver-reaction', maxPerWindow: 10,
    offSelector: naverSelectors.blog.like, onSelector: naverSelectors.blog.unlike,
...
export async function setCafePostLiked(session, cafePostRef, liked, opts = {}) {
  ...
  let post;
  try { post = assertCafePostRef(cafePostRef); } catch (error) { return invalid(action, error, 'cafe-post-ref-validation'); }
  return desiredToggle(session, {
    action, target: post.url, desired: liked, opts, enableKey: 'enableCafeLike',
    rateKey: 'naver-reaction', maxPerWindow: 10,
    offSelector: naverSelectors.cafe.like, onSelector: naverSelectors.cafe.unlike,
...
async function createComment(session, {
  action, target, parentId, expected, opts, enableKey, commentField, commentSubmit, replyControl, commentText, membershipUrl
}) {
  const blocked = validateLiveWrite(session, action, opts, enableKey);
  if (blocked) return blocked;
  if (membershipUrl) {
  ...
  if (parentId) {
    ...
    if (await reply.count() !== 1) return block(action, 'owned Naver reply control is not unique', 'selector-ownership');
    replyLocator = reply;
  }

  const before = await exactTextCount(session.page, commentText, expected);
  return guardedDispatch(session, {
    action, rateKey: 'naver-comment', maxPerWindow: 5, target,
    ...
    verify: () => waitFor(session.page, async () => await exactTextCount(session.page, commentText, expected) > before),
...
export async function commentBlogPost(session, blogPostRef, text, opts = {}) {
  ...
  try { post = assertBlogPostRef(blogPostRef); } catch (error) { return invalid(action, error, 'blog-post-ref-validation'); }
  let expected;
  try { expected = validateCommentPayload(text); } catch (error) { return block(action, error.message, 'input-validation'); }
  return createComment(session, {
    action, target: post.url, parentId: null, expected, opts, enableKey: 'enableBlogComment',
    commentField: naverSelectors.blog.commentField, commentSubmit: naverSelectors.blog.commentSubmit,
    replyControl: naverSelectors.blog.replyControl, commentText: naverSelectors.blog.commentText
  });
}
...
export async function replyToBlogComment(session, blogCommentRef, text, opts = {}) {
  ...
  let comment;
  try { comment = assertBlogCommentRef(blogCommentRef); } catch (error) { return invalid(action, error, 'blog-comment-ref-validation'); }
  let expected;
  try { expected = validateCommentPayload(text); } catch (error) { return block(action, error.message, 'input-validation'); }
  return createComment(session, {
    action, target: comment.url, parentId: comment.commentId, expected, opts, enableKey: 'enableBlogReply',
    commentField: naverSelectors.blog.commentField, commentSubmit: naverSelectors.blog.commentSubmit,
    replyControl: naverSelectors.blog.replyControl, commentText: naverSelectors.blog.commentText
  });
}
...
export async function commentCafePost(session, cafePostRef, text, opts = {}) {
  ...
  try { expected = validateCommentPayload(text); } catch (error) { return block(action, error.message, 'input-validation'); }
  return createComment(session, {
    action, target: post.url, parentId: null, expected, opts, enableKey: 'enableCafeComment',
    commentField: naverSelectors.cafe.commentField, commentSubmit: naverSelectors.cafe.commentSubmit,
    replyControl: naverSelectors.cafe.replyControl, commentText: naverSelectors.cafe.commentText,
    membershipUrl: `https://cafe.naver.com/${post.cafeId}`
  });
}
...
export async function replyToCafeComment(session, cafeCommentRef, text, opts = {}) {
  ...
  let comment;
  try { comment = assertCafeCommentRef(cafeCommentRef); } catch (error) { return invalid(action, error, 'cafe-comment-ref-validation'); }
  let expected;
  try { expected = validateCommentPayload(text); } catch (error) { return block(action, error.message, 'input-validation'); }
  return createComment(session, {
    action, target: comment.url, parentId: comment.commentId, expected, opts, enableKey: 'enableCafeReply',
    commentField: naverSelectors.cafe.commentField, commentSubmit: naverSelectors.cafe.commentSubmit,
    replyControl: naverSelectors.cafe.replyControl, commentText: naverSelectors.cafe.commentText,
...
export async function createBlogDraft(session, content, opts = {}) {
  ...
  let post;
  try { post = validatePostContent(content); } catch (error) { return block(action, error.message, 'input-validation'); }
  const blocked = validateLiveWrite(session, action, opts, 'enableBlogDraft');
  if (blocked) return blocked;
  let media;
  try { media = await validateLocalImages(post.media); } catch (error) { return block(action, error.message, 'media-validation'); }

  await session.navigateGuardedForWrite(BLOG_WRITE_URL, { waitUntil: 'domcontentloaded', timeout: 30_000 });
  ...
  if (media.length > 0 && await session.page.locator(naverSelectors.blog.write.fileInput).count() !== 1) {
    return block(action, 'owned Naver blog media upload control is not unique', 'selector-ownership');
  }

  const before = new Set(await session.page.locator(naverSelectors.blog.write.draftItem).evaluateAll((nodes) => nodes.map((node) => node.getAttribute('data-draft-id')).filter(Boolean)));
  return guardedDispatch(session, {
    action, rateKey: 'naver-blog-publish', maxPerWindow: 2, target: BLOG_WRITE_URL,
    content: { title: post.title, body: post.body, visibility: post.visibility, mediaHashes: media.map((file) => file.fileHash) },
    runId: opts.runId,
    dispatch: async () => {
      ...
      if (media.length > 0) {
        await session.page.locator(naverSelectors.blog.write.fileInput).setInputFiles(media.map((file) => ({
    ...
    verify: () => waitFor(session.page, async () => {
      const ids = await session.page.locator(naverSelectors.blog.write.draftItem).evaluateAll((nodes) => nodes
...
export async function publishBlogDraft(session, draftRef, opts = {}) {
  ...
  await session.navigateGuardedForWrite(BLOG_WRITE_URL, { waitUntil: 'domcontentloaded', timeout: 30_000 });
  const draftNode = session.page.locator(`${naverSelectors.blog.write.draftItem}[data-draft-id="${draft.draftId}"]`);
  if (await draftNode.count() !== 1) return block(action, 'existing owned Naver blog draft could not be proven', 'draft-state');
  ...
  return guardedDispatch(session, {
    ...
    dispatch: async () => {
      ...
      if (await publishControl.count() !== 1) throw new Error('owned Naver blog publish control is not unique');
      await session.humanClick(publishControl.first());
    },
    verify: () => waitFor(session.page, async () => {
      const logNos = await session.page.locator(publishedSelector).evaluateAll((nodes) => nodes
        .map((node) => node.getAttribute('data-log-no'))
...
export async function createCafePost(session, cafeRef, content, opts = {}) {
  ...
  let cafe;
  try { cafe = assertCafeRef(cafeRef); } catch (error) { return invalid(action, error, 'cafe-ref-validation'); }
  let post;
  try { post = validatePostContent(content); } catch (error) { return block(action, error.message, 'input-validation'); }
  ...
  await session.navigateGuardedForWrite(writeUrl, { waitUntil: 'domcontentloaded', timeout: 30_000 });
  ...
  if (media.length > 0 && await session.page.locator(naverSelectors.cafe.write.fileInput).count() !== 1) {
    return block(action, 'owned Naver cafe media upload control is not unique', 'selector-ownership');
  }

  const before = new Set(await session.page.locator(naverSelectors.cafe.write.articleItem).evaluateAll((nodes) => nodes.map((node) => node.getAttribute('data-article-id')).filter(Boolean)));
  return guardedDispatch(session, {
    action, rateKey: 'naver-cafe-post', maxPerWindow: 2, target: writeUrl,
    content: { cafeId: cafe.cafeId, title: post.title, body: post.body, visibility: post.visibility, mediaHashes: media.map((file) => file.fileHash) },
    runId: opts.runId,
    dispatch: async () => {
      await session.humanType(titleField.first(), post.title);
      await session.humanType(bodyField.first(), post.body);
      ...
      if (media.length > 0) {
        await session.page.locator(naverSelectors.cafe.write.fileInput).setInputFiles(media.map((file) => ({
          name: file.name, mimeType: file.mimeType, buffer: file.buffer
        })));
      }
    ...
    verify: () => waitFor(session.page, async () => {
      const ids = await session.page.locator(naverSelectors.cafe.write.articleItem).evaluateAll((nodes) => nodes
...
export function blockedNaverAction(actionName) {
  ...
  if (!FORBIDDEN_ACTIONS.has(name)) throw new TypeError(`unsupported Naver structural blocker: ${name}`);
  return block(`naver:${name}`, `${name} is structurally blocked for Naver automation`, 'structural-blocker');
}
