...
export async function runDiagnostics(input, { readFile = fsp.readFile } = {}) {
  ...
  for (const { relativePath, kind } of requested) {
    ...
    if (!isInside(stateRoot, file)) return diagnosticsEnvelope('failed', 'unknown', 'diagnostic path escaped stateDir', null, 'stop', { path: relativePath });
    try {
      const stat = await fsp.lstat(file);
      if (!stat.isFile() || stat.isSymbolicLink()) throw new Error('not a regular file');
      if (stat.size > MAX_DIAGNOSTIC_FILE_BYTES) throw new Error(`file exceeds ${MAX_DIAGNOSTIC_FILE_BYTES} byte diagnostic limit`);
...
function validateInput(input) {
  ...
  if (!input.lastAgentOutput || !['setup', 'browser-task'].includes(input.lastAgentOutput.agent) || !verifyAgentEnvelope(input.lastAgentOutput).ok) return 'lastAgentOutput must be a complete verified setup or browser-task envelope';
  if (!Array.isArray(input.logPaths) || !Array.isArray(input.screenshotPaths)) return 'logPaths and screenshotPaths must be arrays';
  for (const relativePath of [...input.logPaths, ...input.screenshotPaths]) if (typeof relativePath !== 'string' || !relativePath || path.isAbsolute(relativePath) || relativePath === '..' || relativePath.startsWith(`..${path.sep}`)) return 'diagnostic paths must be relative';
  if (!path.isAbsolute(input.stateDir || '')) return 'stateDir must be an absolute parent-authorized path';
  return null;
}
...
function diagnosticsEnvelope(status, layer, observedSignal, lastSafeActionValue, nextAuthorizedStep, reportJson, markdown = null) {
  const result = {
    ...
    nextAuthorizedStep, report: { json: reportJson, markdown: markdown || `# Diagnostics\n\n${String(observedSignal)}\n` }
  };
  return { schemaVersion: 1, agent: 'diagnostics', status, executionMode: 'parent', failure: status === 'succeeded' ? null : { code: 'diagnostics-input-invalid', phase: 'diagnostics-input', retryable: false, observedSignal: String(observedSignal) }, result };
}
function renderMarkdown(report) { return `# Hyper Cloaking Diagnostics\n\n- Layer: ${report.layer}\n- Signal: ${report.observedSignal}\n- Last safe action: ${report.lastSafeAction || 'none'}\n- Next authorized step: ${report.nextAuthorizedStep}\n`; }
...
async function secureRealDirectory(directory) {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) throw new Error('stateDir must be a non-symlink directory');
...
export async function runDiagnosticsCli(argv = process.argv.slice(2), io = {}) {
  ...
  if (argv.length !== 2 || argv[0] !== '--input-stdin' || argv[1] !== '--json') { stderr.write('Usage: diagnostics-agent --input-stdin --json\n'); return 2; }
  try {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) process.exitCode = await runDiagnosticsCli();
