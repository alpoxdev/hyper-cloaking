...
} from './instagram/network.mjs';
import {
  executeRedditRead,
  redditReadPromotions
} from './reddit/network.mjs';
import {
  executeYouTubeRead,
  youtubeReadPromotions
} from './youtube/network.mjs';
...
function apiFixture({ payload = { items: [{ id: 1 }] }, status = 200, finalUrl = 'https://api.example.test/data', headers = {}, body, getError, disposeError } = {}) {
...
test('read strategy remains DOM-default until all promotion evidence is present', async () => {
  ...
  assert.equal(chooseReadStrategy({ promotion: promoted, available: { direct: true, dom: true } }), 'direct');
  assert.throws(
  ...
  await assert.rejects(
    executeReadStrategy({
      promotion: promoted,
      handlers: {
        direct: async () => { throw new NetworkReadError('auth', 'auth failed', { dispatched: true }); },
...
test('normalized read strategies keep immutable DOM-default promotion and normalize every handler', async () => {
  ...
  const fallback = await executeNormalizedReadStrategy({
    ...
    handlers: { direct: async () => { directCalls += 1; return { rows: ['direct'] }; } },
    dom: async () => { domCalls += 1; return { rows: ['dom'] }; },
...
test('standard read envelope preserves the provider-neutral content contract', () => {
  assert.deepEqual(standardReadEnvelope({ url: 'https://example.test', kind: 'fixture-read', content: [1] }), {
    trusted: false,
...
test('isolated GET uses a module-owned context with exact cookies, headers, and disposal', async () => {
  ...
  assert.deepEqual(fixture.state.options.storageState.cookies.map((cookie) => cookie.name), ['exact', 'urlExact']);
...
test('isolated GET applies RFC cookie path boundaries', async () => {
  ...
  assert.deepEqual(fixture.state.options.storageState.cookies.map((cookie) => cookie.name), ['parent']);
  ...
  assert.deepEqual(trailingFixture.state.options.storageState.cookies.map((cookie) => cookie.name), ['root']);
});

test('isolated GET is independent of page fetch and service workers', async () => {
  const page = {
    evaluate() { throw new Error('page fetch must not run'); },
    context() { return { serviceWorkers: () => [{ active: true }] }; }
  };
...
test('isolated GET rejects origins, redirects, status, size, UTF-8, JSON and structure caps', async () => {
  await assert.rejects(
    isolatedJsonGet({ requestFactory: apiFixture().requestFactory, url: 'https://outside.test/data', allowedOrigins: ['https://api.example.test'] }),
  ...
  for (const [fixtureOptions, expected, options = {}] of [
    ...
    [{ payload: { rows: new Array(10).fill({ id: 1 }) } }, 'response-record-cap', { maxRecords: 5 }]
  ]) {
    await assert.rejects(
      isolatedJsonGet({
...
test('private capability validates exact path, bounded lease, query and headers', () => {
  ...
  assert.throws(() => validateObservedPrivateCapability(capability({ limits: { ttlMs: 60_001 } })), /at most 60000/);
  assert.throws(
    () => qualifyObservedPrivateRequest({ capability: validated, method: 'POST', url: qualified.url, headers: { authorization: 'x' } }),
    (error) => error.code === 'private-method-disallowed'
  );
  assert.throws(
    () => qualifyObservedPrivateRequest({ capability: validated, method: 'GET', url: `${qualified.url}&extra=1`, headers: { authorization: 'x' } }),
...
test('fixed private headers, cursor cardinality, and page-one cursor absence are enforced', () => {
  ...
  assert.throws(
    () => validateObservedPrivateCapability(capability({
...
test('observed-private lease binds identity, expiry, use count, and outer lifetime', async () => {
  ...
  await assert.rejects(
    withObservedPrivateReplay({
      binding: replayBinding(),
      capability: cap,
      template: { method: 'GET', url: 'https://api.example.test/data?fixed=yes', headers: { authorization: 'x' } },
      requestFactory: fixture.requestFactory,
      now: (() => { let call = 0; return () => call++ === 0 ? 0 : clock; })()
  ...
  await assert.rejects(
    withObservedPrivateReplay({
      ...
      capability: capability({ limits: { ttlMs: 1000, maxUses: 3, maxBytes: 1024, maxRecords: 20 } }),
      template: { method: 'GET', url: 'https://api.example.test/data?fixed=yes', headers: { authorization: 'x' } },
      requestFactory: fixture.requestFactory,
      now: () => partialClock
    }, async (client) => {
...
function eventPage() {
  ...
  return {
    on(name, handler) { listeners[name].add(handler); },
    off(name, handler) { listeners[name].delete(handler); },
    emit(name, value) { for (const handler of listeners[name]) handler(value); },
    counts() { return { request: listeners.request.size, response: listeners.response.size }; }
  };
}
...
test('capture waits for a delayed correlated response and hard-stops when it never arrives', async () => {
  ...
  await assert.rejects(
    ...
    (error) => error.code === 'private-response-timeout' && error.dispatched === true && !canUseDomFallback(error)
  );
...
test('capture distinguishes true no-match from post-dispatch ambiguity and failures', async () => {
  ...
  await assert.rejects(
    captureObservedPrivateResponse({
      page: ambiguousPage,
      session: { async navigateGuardedForRead() { exchange(ambiguousPage); exchange(ambiguousPage); } },
      capability: capability(),
      targetUrl: 'https://api.example.test/data'
    }),
    (error) => error.code === 'private-capture-ambiguous' && error.dispatched === true
  ...
  await assert.rejects(
    captureObservedPrivateResponse({
      page: failedPage,
      session: { async navigateGuardedForRead() { exchange(failedPage, { status: 429 }); } },
      capability: capability(),
      targetUrl: 'https://api.example.test/data'
    }),
