...
import { youtubeSelectors } from './selectors.mjs';
...
function writeSession({ liked = false, subscribed = false, comments = [], stateDir = path.join(TEST_STATE_ROOT, randomUUID()) } = {}) {
  ...
  const page = {
    ...
    $eval: async (selector, callback) => callback({
      getAttribute: (name) => {
        ...
        if (name === 'aria-label') return selector.includes('Subscribe')
          ? (subscribed ? 'Subscribed' : 'Subscribe')
          : (liked ? 'Liked' : 'Like this video');
        return null;
      }
    }),
    $$eval: async (_selector, callback, needle) => callback(
...
test('YouTube ID validators canonicalize valid references and reject invalid ones', () => {
  ...
  assert.equal(watchUrl('https://youtu.be/dQw4w9WgXcQ'), 'https://www.youtube.com/watch?v=dQw4w9WgXcQ');
...
test('enabled subscribe blocks for missing persistent rate state before navigation', async () => {
  ...
  const result = await subscribeChannel(session, '@RickAstley', { enableSubscribe: true, dryRun: false });
...
test('likeVideo verifies the same owned control after its label transitions to Unlike', async () => {
  ...
  const page = {
    ...
    evaluate: async () => ({ title: '', labels: [] }),
    locator: () => ({ count: async () => 1 }),
    $eval: async (_selector, callback) => callback({
      getAttribute: (name) => name === 'aria-pressed' ? 'false' : (liked ? 'Unlike this video' : 'Like this video')
    }),
    waitForTimeout: async () => {}
  };
  ...
  session.targetSafety = { disposition: 'ok', reason: 'public-https-fqdn', risks: [] };
  session.humanClick = async () => { liked = true; };
...
function commentTierSwitchSession(initialTier, replacementTier) {
  ...
  const page = {
    ...
    $$eval: async (selector, callback, needle) => {
      ...
      const texts = !clicked && selector === initialTier
        ? ['Switch comment']
        : (clicked && selector === replacementTier ? ['Switch comment', 'Switch comment'] : []);
      return callback(texts.map((text) => ({ textContent: text })), needle);
    },
    waitForTimeout: async () => {}
  };
  const session = buildYouTubeSession(page, {
  ...
  session.humanClick = async () => { clicked = true; };
...
test('commentVideo resists fallback-to-primary comment population switches', async () => {
  ...
  assert.equal(result.changed, false);
  assert.equal(result.rateLimit.count, 1);
  assert.deepEqual(evaluatedCommentTiers, [fallback, fallback, fallback, fallback, fallback, fallback]);
});
...
test('commentVideo binds a proven empty baseline to the first appearing tier for zero-to-one verification', async () => {
  ...
  session.targetSafety = { disposition: 'ok', reason: 'public-https-fqdn', risks: [] };
  session.humanType = async () => {};
  session.humanClick = async () => { phase = 'fallback-first'; };
...
test('commentVideo verifies a first comment from a proven zero-comment baseline', async () => {
  ...
  session.targetSafety = { disposition: 'ok', reason: 'public-https-fqdn', risks: [] };
  session.humanType = async () => {};
  session.humanClick = async () => { comments.push('First comment'); };
...
test('commentVideo verification failure still consumes and returns the rate attempt', async () => {
  ...
  try {
    ...
    const result = await commentVideo(session, 'dQw4w9WgXcQ', ' Exact   comment ', { dryRun: false });
...
test('subscribe opt-in preserves verified already-subscribed no-op semantics', async () => {
  ...
  const subscribed = await subscribeChannel(
    subscribeSession,
    '@RickAstley',
    { enableSubscribe: true, dryRun: false }
  );
...
test('registry metadata schema excludes action and automation internals', () => {
  ...
  for (const forbidden of ['actions', 'selectors', 'session', 'automationRecipe']) {
    assert.equal(Object.hasOwn(youtube, forbidden), false, `provider metadata must not carry "${forbidden}"`);
  }
});
