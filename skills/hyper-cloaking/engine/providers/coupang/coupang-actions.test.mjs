...
import { coupangReadPromotions } from './network.mjs';
...
function savedSession(stateDir, { saved = false, clickError = null } = {}) {
  ...
  return {
    ...
    page: { locator },
    async navigateGuardedForWrite() { navigations += 1; this.targetSafety = safety(); },
    async humanClick() {
...
test('Coupang IDs canonicalize owned refs and reject off-origin or incoherent handles', () => {
  ...
  assert.equal(normalizeProductRef({ productId: '1', url: 'https://www.coupang.com/vp/products/2' }), null);
  assert.deepEqual(normalizeCartLineRef({ cartLineId: 'line-1', productId: '123' }), {
    cartLineId: 'line-1',
    productId: '123',
    url: 'https://www.coupang.com/cartView.pang'
  });
  assert.equal(normalizeOrderItemRef({ orderItemId: 'order-1', productId: '123', url: 'https://evil.example/my-orders/1' }), null);
});
...
test('Coupang forced reads normalize whole bounded envelopes and explicit empties', async () => {
  ...
  const product = await getProduct({}, { productId: '123' }, {
    readStrategy: 'direct',
    readHandlers: {
      direct: async () => ({ title: 'Laptop', price: 1000, reviews: [], reviewsEmptyState: true, present: true })
    }
  });
...
test('Coupang writes block before navigation unless dry-run, enable, and state gates pass', async () => {
  const session = { async navigateGuardedForWrite() { assert.fail('must not navigate'); } };
...
function cartStateSession(stateDir, quantity) {
  const quantityLocator = {
    ...
    first() { return this; },
    async inputValue() { return String(quantity); }
  };
  const line = {
    async count() { return quantity == null ? 0 : 1; },
    first() { return this; },
    async getAttribute(name) { return name === 'data-cart-item-id' ? 'line-1' : null; },
    locator() { return quantityLocator; }
  };
  return {
    stateDir,
    targetSafety: safety(),
    ...
    async navigateGuardedForWrite() { this.targetSafety = safety(); }
  };
}
...
test('Coupang cart/review safety branches and structural blockers are exact', async () => {
  const noState = { async navigateGuardedForWrite() { assert.fail('must not navigate'); } };
