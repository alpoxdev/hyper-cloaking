...
import { tiktokReadPromotions } from './network.mjs';
...
function toggleSession(stateDir, { active = false, clickError = null } = {}) {
  ...
  const locator = (selector) => ({
    async count() { return selector.includes('aria-pressed="true"') ? (current ? 1 : 0) : 1; },
    first() { return this; },
...
test('TikTok metadata keeps short links navigation-only and registry-safe', () => {
  ...
  assert.deepEqual(tiktokProvider.domains.navigationOnlyAliases, ['vm.tiktok.com', 'vt.tiktok.com']);
...
test('TikTok refs canonicalize owned identities and reject cold, off-origin, or incoherent refs', () => {
  ...
  assert.equal(normalizeVideoRef({ handle: 'fixture', videoId: '1', url: 'https://www.tiktok.com/@fixture/video/2' }), null);
  assert.deepEqual(normalizeCommentRef({ handle: 'fixture', videoId: '123', commentId: 'comment-1' }), {
    handle: 'fixture',
    videoId: '123',
    url: 'https://www.tiktok.com/@fixture/video/123',
    commentId: 'comment-1'
  });
  assert.equal(normalizeThreadRef({ threadId: '1', accountId: 'acct', url: 'https://www.tiktok.com/messages/2' }, { accountId: 'acct' }), null);
...
test('TikTok forced reads preserve bounded canonical envelopes and DM repeats', async () => {
  ...
  const video = await getVideo({}, { handle: 'fixture', videoId: '789' }, {
    readStrategy: 'direct',
    readHandlers: {
      direct: async () => ({
        present: true,
  ...
  const threads = await listDMThreads({ accountId: 'acct' }, {
    readStrategy: 'direct',
    readHandlers: {
      direct: async () => ({ threads: [{ accountId: 'acct', threadId: 'thread-1', url: 'https://www.tiktok.com/messages/thread-1' }] })
    }
  });
...
test('TikTok analysis is bounded and truthful', () => {
  const analysis = analyzeVideos([
    { videoId: '1', viewCount: 100, likeCount: 10, commentCount: 2, hashtags: ['Build'], timestamp: 0 },
    { videoId: '2', viewCount: 300, likeCount: 30, commentCount: 4, hashtags: ['Build', 'Test'], timestamp: 86_400_000 }
  ]);
...
test('TikTok desired writes are dry-run default, require exact enable/state, and reserve atomically', async () => {
  ...
  const disabled = await setLiked(noNavigation, { handle: 'fixture', videoId: '123' }, true, { dryRun: false });
  ...
  try {
    ...
    const uncertain = await setLiked(uncertainSession, { handle: 'fixture', videoId: '456' }, true, {
      dryRun: false,
      enableLike: true,
...
test('TikTok DM, upload, publish, and forbidden actions fail closed before unsafe dispatch', async () => {
  const noNavigation = { accountId: 'acct', async navigateGuardedForWrite() { assert.fail('must not navigate'); } };
  ...
  try {
    ...
    const upload = await createUploadDraft({
      stateDir,
      async navigateGuardedForWrite() { navigated = true; }
    }, badFile, {
      dryRun: false,
