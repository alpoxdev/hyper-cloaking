...
import { getUser, normalizeUsername, profileUrl, InvalidUsernameError } from './actions/user.mjs';
...
test('threadRef rejects mismatched, off-origin, and decorated opaque handles', () => {
  assert.equal(normalizeThreadRef({ threadId: '1', url: 'https://www.instagram.com/direct/t/2/' }), null);
  assert.equal(normalizeThreadRef({ threadId: '1', url: 'https://evil.example/direct/t/1/' }), null);
...
test('replyToMany rejects duplicate targets and live runs without durable state', async () => {
  ...
  const duplicate = await replyToMany(session, [
  ...
  const noState = await replyToMany(session, [
    { threadRef: { threadId: '1' }, message: 'first' }
  ], { dryRun: false, confirmed: true, runId: 'state-test' });
...
test('Instagram reads stay DOM-default until promotion evidence is complete', async () => {
  ...
  let directCalls = 0;
  const session = {
    async navigateGuardedForRead() { navigations += 1; },
    page: {
      async evaluate() {
        return { displayName: 'Example', stats: ['1 post'], verified: true, private: false, present: true };
      }
    }
  };
...
test('Instagram strict navigation and explicit post empty states fail closed', async () => {
  await assert.rejects(
    getUser({
      async navigateGuardedForRead() { throw new Error('strict navigation failed'); }
    }, 'example'),
  ...
  const empty = await getUserPosts({
    ...
    async humanScroll() { throw new Error('must not scroll after explicit empty state'); }
  }, 'example');
...
test('Instagram DM normalization preserves latest order and repeated equal-text messages', async () => {
  const read = await readDMThread({}, { threadId: '42' }, {
    limit: 3,
    readStrategy: 'direct',
    readHandlers: {
      direct: async () => ({
  ...
  assert.deepEqual(threads.content.map((thread) => thread.threadId), ['2', '1']);
});
...
function dmWriteSession(messageSnapshots) {
  ...
  let clicks = 0;
  return {
    get writeNavigations() { return writeNavigations; },
    get clicks() { return clicks; },
    async navigateGuardedForWrite() { writeNavigations += 1; },
    async humanType() {},
    async humanClick() { clicks += 1; },
    page: {
      async $$eval() {
...
test('Instagram reaction writes preserve exact state-transition and verifier failures', async () => {
  ...
  const alreadyLiked = await likePost({
    ...
    async humanClick() { clicks += 1; },
    page: { async $() { return {}; } }
  }, 'https://www.instagram.com/p/ABC123/', { dryRun: false });
  ...
  await assert.rejects(
    commentPost({
      ...
      async humanClick() {},
      page: {
        async $$eval() { return matchCount; },
        async waitForFunction() {
          throw new Error('comment verification timeout');
        }
      }
    }, 'https://www.instagram.com/p/ABC123/', 'same', { dryRun: false }),
  ...
  const saved = await savePost({
    async navigateGuardedForWrite() {},
    async humanClick() { savedState = true; },
    page: {
      async $() { return savedState ? {} : null; },
