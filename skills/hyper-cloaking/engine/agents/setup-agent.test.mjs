...
function fakeRunCli({ validate = { ok: true }, config } = {}) {
  ...
  const runCli = async (argv, io) => {
    calls.push(argv);
    const value = argv[0] === 'validate' ? validate : config || {
      ok: true,
      client: argv[argv.indexOf('--client') + 1],
      home: argv[argv.indexOf('--home') + 1],
      executablePath: '/tmp/chromium',
      config: { command: 'npx', args: ['@playwright/mcp@latest', ...(argv.includes('--headless') ? ['--headless'] : []), '--sandbox', '--executable-path', '/tmp/chromium'] }
    };
...
const input = { schemaVersion: 1, client: 'direct', workspace: '/tmp/hyper', headless: true, sandbox: true };
...
test('rejects missing sandbox, no-sandbox and headless contradictions', async () => {
  for (const args of [
    ...
    ['@playwright/mcp@latest', '--headless', '--sandbox', '--no-sandbox', '--executable-path', '/tmp/chromium'],
    ['@playwright/mcp@latest', '--headless', '--sandbox', '--sandbox', '--executable-path', '/tmp/chromium'],
    ['@playwright/mcp@latest', '--sandbox', '--executable-path', '/tmp/chromium']
  ]) {
    const fake = fakeRunCli({ config: { ok: true, client: 'direct', home: '/tmp/hyper', executablePath: '/tmp/chromium', config: { command: 'npx', args } } });
    const result = await runSetup(input, { runCli: fake.runCli });
...
test('rejects malformed and extra stdout', async () => {
  const malformed = async (_argv, io) => { io.stdout.write('{bad'); return 0; };
  assert.equal((await runSetup(input, { runCli: malformed })).status, 'blocked');
  const extra = async (_argv, io) => { io.stdout.write('{}\n{}\n'); return 0; };
