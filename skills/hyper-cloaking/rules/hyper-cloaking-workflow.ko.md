...
`scripts/hyper-cloaking.mjs cookies inspect ... --json` 또는 `scripts/hyper-cloaking.mjs browser cookies ... --json`이 `needsAccount: true`를 보고하면 cookie를 load하기 전에 반환된 `availableAccounts` 중 하나를 사용자에게 선택하게 합니다.
...
- 모든 cookie import, normalization, inspection, redaction, Playwright injection은 `scripts/hyper-cloaking.mjs cookies`를 사용합니다.
...
- CloakBrowser JavaScript API로 browser를 직접 drive할 때 반복되는 human-like mouse movement, click, typing, scrolling, XPath lookup은 `scripts/hyper-cloaking.mjs browser` helper function을 사용합니다.
