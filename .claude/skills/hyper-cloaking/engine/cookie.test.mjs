...
test('cookie configuration rejects dangerous keys, malformed URLs, and unknown explicit sites', async () => {
  for (const key of ['__proto__', 'constructor', 'toString', 'valueOf', 'hasOwnProperty', 'apply', 'call', 'bind']) {
    assert.throws(
      () => parseCookieYaml(`sites:\n  ${key}:\n    accounts:\n      default:\n        cookies:\n          []\n`),
      /safe identifier/
    );
    assert.throws(
      () => parseCookieYaml(`sites:\n  safe:\n    accounts:\n      ${key}:\n        cookies:\n          []\n`),
  ...
  assert.throws(
    () => inferSiteForUrl({ sites: { default: {}, broken: { url: 'not a url' } } }, 'https://example.com'),
    /must be a valid URL/
  );
  assert.throws(
    () => selectCookieRecords({ sites: { default: { accounts: {} } } }, 'https://example.com', { site: 'typo' }),
    /Unknown cookie site/
  );
  assert.throws(
    () => selectCookieRecords({
  ...
  assert.throws(
    () => inferSiteForUrl({ sites: { default: {} } }, 'not a url'),
    /Target URL/
  );
