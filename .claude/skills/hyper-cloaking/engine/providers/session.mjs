...
export class StrictNavigationError extends Error {
  constructor(url, { code = 'strict-navigation-failed', phase = 'strict-navigation', status = null, cause } = {}) {
    super(`Strict provider navigation failed for "${url}"${status == null ? '' : ` (HTTP ${status})`}`, { cause });
...
export function buildProviderSession(page, { provider, allowedOrigins, targetSafety, stateDir, interactive } = {}) {
  ...
  const session = {
    ...
    requireOnStrictOrigin(url = session.currentUrl()) {
      if (strictOriginPolicy.invalidOverride || !isOriginApproved(url, strictOriginPolicy.origins)) {
...
function isMainNavigationRequest(request, page) {
  ...
  if (typeof request.frame !== 'function' || typeof page?.mainFrame !== 'function') return navigation;
  return request.frame() === page.mainFrame();
}

function strictStatusError(url, status, phase) {
  if (status === 401) return new StrictNavigationError(url, { code: 'network-auth', phase, status });
  if (status === 429) return new StrictNavigationError(url, { code: 'network-rate-limit', phase, status });
...
async function navigateStrict(page, session, {
  provider,
  targetSafety,
  ...
  if (typeof page?.route !== 'function' || typeof page?.unroute !== 'function' || typeof page?.goto !== 'function') {
    throw new StrictNavigationError(targetUrl, {
  ...
  try {
    ...
    const finalSafety = classifyTargetUrl(finalUrl);
    if (finalSafety.disposition !== 'ok') throw new TargetSafetyError(finalUrl, finalSafety, 'redirect');
  ...
  if (installed) {
    try {
      ...
      if (operationError) {
        throw new AggregateError([operationError, cleanupError], 'strict navigation and route cleanup failed');
      }
      throw new StrictNavigationError(targetUrl, {
