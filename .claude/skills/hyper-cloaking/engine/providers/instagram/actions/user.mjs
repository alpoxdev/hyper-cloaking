...
function boundedText(value, field, maxLength, { nullable = true } = {}) {
  ...
  if (text.length > maxLength) throw new TypeError(`Instagram ${field} exceeds ${maxLength} characters`);
  return text;
}
...
function normalizeProfileContent(value, { username, url }) {
  ...
  return {
    ...
    rawStats: value.rawStats.map((entry) => boundedText(entry, 'profile statistic', 1_000, { nullable: false }))
  };
}
...
export async function getUser(session, username, opts = {}) {
  ...
  const dom = async () => {
    ...
    const data = await session.page.evaluate((sel) => {
      ...
      const stats = header ? [...header.querySelectorAll('li')].map((item) => (item.textContent || '').trim()) : [];
      return {
  ...
  const { value } = await executeInstagramRead({
    ...
    dom,
    normalize: (content) => normalizeProfileContent(content, {
