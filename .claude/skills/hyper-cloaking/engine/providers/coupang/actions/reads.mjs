...
function normalizeProductEntry(entry) {
  ...
  if (!ref) throw new TypeError('Coupang product entry contains an invalid owned-origin product reference');
  return {
    ...ref,
...
function normalizeSearchContent(value, { query, limit }) {
  if (!value || typeof value !== 'object' || !Array.isArray(value.products) || value.products.length > MAX_RAW) {
...
function normalizeProductContent(value, { product, includeReviews, limit }) {
  if (!value || typeof value !== 'object' || !Array.isArray(value.reviews) || value.reviews.length > MAX_RAW) {
...
export async function searchProducts(session, query, opts = {}) {
  ...
  const dom = async () => {
    ...
    const rows = await session.page.$$eval(
      ...
      (nodes, selectors) => nodes.slice(0, 400).map((link) => {
        const card = link.closest(selectors.productCard) || link;
        return {
          url: link.getAttribute('href'),
          title: card.querySelector(selectors.title)?.textContent?.trim() || link.textContent?.trim() || null,
          price: card.querySelector(selectors.price)?.textContent?.trim() || null
        };
      }),
      coupangSelectors.search
    );
    if (rows.length === 0 && await session.page.locator(coupangSelectors.search.emptyState).count() === 0) {
...
export async function getProduct(session, productRef, opts = {}) {
  ...
  const dom = async () => {
    await session.navigateGuardedForRead(product.url, { waitUntil: 'domcontentloaded', timeout: 30_000 });
    ...
    if (includeReviews) {
      reviews = await session.page.$$eval(coupangSelectors.product.review, (nodes) => nodes.slice(0, 400).map((node) => ({
        reviewId: node.getAttribute('data-review-id'),
        author: node.querySelector('[data-author], .sdp-review__article__list__info__user__name')?.textContent?.trim() || null,
        text: node.querySelector('[data-review-text], .sdp-review__article__list__review__content')?.textContent?.trim() || node.textContent?.trim() || null,
        rating: node.getAttribute('data-rating'),
        timestamp: node.querySelector('time')?.getAttribute('datetime') || null
      })));
      reviewsEmptyState = reviews.length === 0
        && await session.page.locator(coupangSelectors.product.reviewEmptyState).count() > 0;
      if (reviews.length === 0 && !reviewsEmptyState) throw new Error('Coupang review state could not be proven');
    }
