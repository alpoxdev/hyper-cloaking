...
const SECRET_KEY_PATTERN = /(?:cookie|token|secret|credential|auth|authorization|session|sessionid|sid|key|password|passwd|api[_-]?key)/iu;
const PII_KEY_PATTERN = /(?:email|phone|address|name|user|username|person|pii)/iu;
const CONTENT_KEY_PATTERN = /(?:raw|text|html|screenshot|image|download|body|content|dom|cookie|headers)/iu;
...
function sanitizeValue(value, context) {
  ...
  if (Array.isArray(value)) return value.map((entry, index) => sanitizeValue(entry, { ...context, path: [...context.path, String(index)] }));
  ...
  if (typeof value === 'object') {
    ...
    for (const [key, child] of Object.entries(value)) {
      ...
      if (SECRET_KEY_PATTERN.test(normalizedKey) || PII_KEY_PATTERN.test(normalizedKey) || CONTENT_KEY_PATTERN.test(normalizedKey)) {
        output[key] = '[stripped]';
        continue;
      }
...
function shapePath(pathname) {
  return pathname
    .split('/')
    .filter(Boolean)
    .map((part) => (/^\d+$/u.test(part) || /^[0-9a-f-]{8,}$/iu.test(part) ? ':id' : part.slice(0, 32)))
    .join('/');
}
