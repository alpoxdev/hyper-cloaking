...
import { dispatchParent } from './parent-dispatcher.mjs';
...
const setupEnvelope = {
  ...
  executionMode: 'parent',
  failure: null,
  result: { agentType: 'setup', setupStatus: 'ready', mcpConfig: { type: 'direct', config: {} }, executablePath: '/tmp/chromium', blockers: [] }
};

test('parent_default invokes exactly one role and maps success', async () => {
  let calls = 0;
  const result = await dispatchParent(request, { runSetup: async () => { calls += 1; return setupEnvelope; } });
...
test('native_unavailable never falls back to parent', async () => {
  let calls = 0;
  const result = await dispatchParent({ ...request, executionMode: 'subagent' }, { runSetup: async () => { calls += 1; return setupEnvelope; } });
...
test('spawn_failed does not retry', async () => {
  let calls = 0;
  const result = await dispatchParent({ ...request, executionMode: 'subagent' }, { nativeAdapter: { spawn: async () => { calls += 1; throw new Error('transport'); } } });
...
test('role throws once and becomes contract failure', async () => {
  let calls = 0;
  const result = await dispatchParent(request, { runSetup: async () => { calls += 1; throw new Error('boom'); } });
  assert.equal(result.route, 'contract_failure');
  assert.equal(calls, 1);
});
...
test('browser cleanup failure prevents evidence publisher invocation', async () => {
  ...
  const browserEnvelope = {
    ...
    failure: { code: 'browser-cleanup-unverified', phase: 'browser-live', retryable: false, observedSignal: 'close timeout' },
    result: {
      agentType: 'browser-task', taskMode: 'verification-only', targetSafety: 'allowed', outcome: 'blocked', finalUrl: 'https://example.com/',
      contentBoundary: { allowedOrigins: ['https://example.com'], observedOrigin: 'https://example.com', redirects: [], violations: [] },
      humanizationProof: { enabled: true, telemetryAvailable: false, source: null, blocker: 'unavailable' },
      cleanup: { ok: false, closed: false, timedOut: true, blocker: 'close timeout' }, evidenceRefs: [], learning: { summary: '', limitations: [] }
    }
  };
  const result = await dispatchParent({ ...request, trigger: 'browser-task', evidence: { enabled: true, homeDir: '/tmp/home', publication: {} } }, { runBrowserTask: async () => browserEnvelope, persistEvidence: async () => { publishes += 1; } });
  assert.equal(result.exitCode, 1);
  assert.equal(publishes, 0);
