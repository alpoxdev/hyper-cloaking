...
export class CredentialStoreError extends Error {
  constructor(code, message, { operationId = null, beforeRevision = null, afterRevision = null, result = null, cause } = {}) {
    super(message, { cause });
...
function stringList(value, label) {
  ...
  const result = value.map((item, index) => requireString(item, `${label}[${index}]`, { maximum: 512 }));
...
class StrictJsonParser {
  constructor(text) {
    ...
    if (Buffer.byteLength(text) > MAX_STORE_BYTES) throw new TypeError('credential JSON exceeds size limit');
    this.text = text;
    this.index = 0;
    this.nodes = 0;
  }
  ...
  string() {
    ...
    while (this.index < this.text.length) {
      ...
      if (!escaped && character === '"') {
        ...
        let parsed;
        try { parsed = JSON.parse(raw); } catch { this.error('invalid string escape'); }
        if (parsed.length > MAX_STRING_LENGTH || parsed.includes('\0')) this.error('string exceeds policy');
        return parsed;
      }
...
function profileRequirements(provider, kind) {
  ...
  if (!kinds || !Object.hasOwn(kinds, kind)) throw new TypeError(`unsupported provider profile kind: ${provider}/${kind}`);
  return kinds[kind];
}

function validateCredentials(credentials, required, label) {
  if (!credentials || typeof credentials !== 'object' || Array.isArray(credentials)) throw new TypeError(`${label}.credentials must be an object`);
  const result = nullRecord();
  const allowed = new Set(required);
  for (const [key, value] of Object.entries(credentials)) {
    requireSafeKey(key, `${label}.credentials key`);
    if (!allowed.has(key)) throw new TypeError(`${label}.credentials.${key} is not allowed for this profile kind`);
...
function validateScopeEvidence(input, verifiedScopes, label) {
  ...
  const evidenceHash = requireString(input.evidenceHash, `${label}.scopeEvidence.evidenceHash`, { maximum: 64 });
  if (!/^[a-f0-9]{64}$/.test(evidenceHash)) throw new TypeError(`${label}.scopeEvidence.evidenceHash must be SHA-256 hex`);
  const verifiedAt = Number(input.verifiedAt);
  if (!Number.isFinite(verifiedAt) || verifiedAt <= 0) throw new TypeError(`${label}.scopeEvidence.verifiedAt must be positive and finite`);
  return { source, evidenceHash, verifiedAt };
}

function validateProfile(profileId, input) {
  requireSafeKey(profileId, 'profile id');
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw new TypeError(`profile ${profileId} must be an object`);
  ...
  const scopeEvidence = validateScopeEvidence(input.scopeEvidence, verifiedScopes, `profile ${profileId}`);
  ...
  if (expiresAt != null && !Number.isFinite(expiresAt)) throw new TypeError(`profile ${profileId}.expiresAt must be finite`);
  return {
    provider,
    kind,
    credentials,
...
export function validateCredentialStore(input) {
  ...
  if (!Number.isInteger(input.revision) || input.revision < 0) throw new TypeError('credential store revision must be a non-negative integer');
  if (!input.defaults || typeof input.defaults !== 'object' || Array.isArray(input.defaults)) throw new TypeError('credential defaults must be an object');
  if (!input.profiles || typeof input.profiles !== 'object' || Array.isArray(input.profiles)) throw new TypeError('credential profiles must be an object');
  const profiles = nullRecord();
  for (const [profileId, profile] of Object.entries(input.profiles)) profiles[profileId] = validateProfile(profileId, profile);
  ...
  for (const [provider, profileId] of Object.entries(input.defaults)) {
    ...
    if (!Object.hasOwn(profiles, profileId)) throw new TypeError(`default profile does not exist: ${provider}/${profileId}`);
    if (profiles[profileId].provider !== provider) throw new TypeError(`default profile provider mismatch: ${provider}/${profileId}`);
...
async function assertSecureDirectory(fsImpl, directory, { create = false } = {}) {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) throw new CredentialStoreError('unsafe-path', `Credential directory is unsafe: ${directory}`);
  if (typeof process.getuid !== 'function') throw new CredentialStoreError('posix-required', 'Credential storage requires POSIX ownership checks');
  if (stat.uid !== process.getuid() || (modeBits(stat) & 0o077) !== 0) throw new CredentialStoreError('unsafe-permissions', `Credential directory must be owner-only: ${directory}`);
}

async function assertSecureFile(fsImpl, file, { optional = false } = {}) {
  let stat;
  try { stat = await fsImpl.lstat(file); } catch (error) {
    if (optional && error?.code === 'ENOENT') return false;
    throw error;
  }
  if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1) throw new CredentialStoreError('unsafe-file', `Credential file must be a regular single-link file: ${file}`);
  if (stat.uid !== process.getuid() || (modeBits(stat) & 0o177) !== 0) throw new CredentialStoreError('unsafe-permissions', `Credential file must be mode 0600: ${file}`);
  return true;
}

async function readSecureDescriptorText(fsImpl, file, {
  optional = false,
  ...
  if (typeof process.getuid !== 'function') {
    throw new CredentialStoreError('posix-required', `${label} reads require POSIX ownership checks`);
  }
  ...
  try {
    ...
    throw new CredentialStoreError('unsafe-source', `${label} could not be opened securely`, { cause: error });
  }
  ...
  try {
    ...
    while (true) {
      ...
      total += bytesRead;
      if (total > maxBytes) {
        throw new CredentialStoreError('source-too-large', `${label} grew beyond the size limit while reading`);
      }
  ...
  try {
    ...
    if (primaryError) throw new AggregateError([primaryError, closeError], `${label} read and close failed`);
    throw new CredentialStoreError('source-close-uncertain', `${label} read succeeded but close failed`, { cause: closeError });
  }
...
async function acquireOwnedLock(lockPath, {
  fsImpl = fs,
  host = os.hostname(),
  ...
  while (true) {
    ...
    const owner = { version: 1, pid: process.pid, host, nonce, phase, createdAt: Date.now(), updatedAt: Date.now() };
    await writePrivateFile(fsImpl, path.join(candidate, 'owner.json'), stableJson(owner), { exclusive: true });
    try {
      ...
      try {
        ...
        current = parseCredentialJson(await fsImpl.readFile(path.join(lockPath, 'owner.json'), 'utf8'));
      } catch (inspectionError) {
        if (inspectionError?.code === 'ENOENT') continue;
        throw new CredentialStoreError('lock-corrupt', 'Credential lock is corrupt and cannot be reclaimed', { cause: inspectionError });
      }
      if (current.host !== host) throw new CredentialStoreError('lock-foreign-host', 'Credential lock belongs to another host');
      const status = processStatus(current.pid);
      if (status !== 'dead') {
        if (Date.now() >= deadline) throw new CredentialStoreError(status === 'permission-denied' ? 'lock-permission-denied' : 'lock-busy', 'Credential lock is active or unverifiable');
        await sleep(LOCK_RETRY_MS);
        continue;
      }
      const reap = path.join(lockPath, '.reap');
      try { await fsImpl.mkdir(reap); } catch (reapError) {
        if (reapError?.code === 'EEXIST' || reapError?.code === 'ENOENT') continue;
        throw reapError;
      }
      const verify = parseCredentialJson(await fsImpl.readFile(path.join(lockPath, 'owner.json'), 'utf8'));
      if (verify.host !== current.host || verify.pid !== current.pid || verify.nonce !== current.nonce || processStatus(verify.pid) !== 'dead') {
        throw new CredentialStoreError('lock-owner-changed', 'Credential lock owner changed before recovery');
      }
    ...
    const setPhase = async (nextPhase) => {
      if (!LOCK_PHASES.has(nextPhase)) throw new TypeError(`unsupported credential lock phase: ${nextPhase}`);
      const ownerPath = path.join(lockPath, 'owner.json');
      const current = parseCredentialJson(await fsImpl.readFile(ownerPath, 'utf8'));
      if (current.nonce !== nonce || current.pid !== process.pid || current.host !== host) throw new CredentialStoreError('lock-owner-changed', 'Credential lock owner changed');
    ...
    const release = async () => {
      ...
      await fsImpl.rename(lockPath, quarantine);
      const current = parseCredentialJson(await fsImpl.readFile(path.join(quarantine, 'owner.json'), 'utf8'));
      if (current.nonce !== nonce || current.pid !== process.pid || current.host !== host) throw new CredentialStoreError('lock-owner-changed', 'Refusing to release another credential lock');
      await fsImpl.rm(quarantine, { recursive: true });
    };
...
async function runWithOwnedLock(lock, operation) {
  ...
  try {
    ...
    if (primaryError) {
      throw new AggregateError([primaryError, releaseError], 'credential operation and lock release failed');
    }
    throw new CredentialStoreError('lock-release-uncertain', 'Credential operation completed but lock release failed', {
...
async function withProfileLock(home, profileId, options, operation) {
  ...
  try {
    ...
    if (primaryError) {
      throw new AggregateError([primaryError, releaseError], 'credential profile operation and lock release failed');
    }
    throw new CredentialStoreError('lock-release-uncertain', 'Credential profile operation completed but lock release failed', {
      result,
      cause: releaseError
    });
  }
...
async function digestFile(fsImpl, file) {
  try { return sha256(await fsImpl.readFile(file)); } catch (error) {
...
async function mutateStore(home, mutator, options = {}) {
  ...
  return runWithOwnedLock(lock, async () => {
    ...
    try {
      ...
      journal.updatedAt = Date.now();
      await writeJournal(fsImpl, journalPath, journal);
      return { store: after, operationId, beforeRevision: before.revision, afterRevision: after.revision };
    } catch (error) {
      let observed;
      try {
        ...
        throw new CredentialStoreError('operation-ambiguous', 'Credential operation could not be observed after failure', {
          operationId,
          beforeRevision: before.revision,
          afterRevision: after.revision,
          cause: new AggregateError(causes, 'credential mutation, observation, and bookkeeping failed')
        });
      }
      ...
      if (observed === afterDigest) {
        try {
          ...
          throw new CredentialStoreError('operation-ambiguous', 'Credential operation durability is ambiguous', {
            operationId,
            beforeRevision: before.revision,
            afterRevision: after.revision,
            cause: new AggregateError(causes, 'rename, durability, and bookkeeping acknowledgement failed')
          });
        }
      }
      ...
      throw new CredentialStoreError(
        ...
        `Credential operation ${journal.state}`,
        {
          ...
          beforeRevision: before.revision,
          afterRevision: after.revision,
          cause: causes.length === 1 ? causes[0] : new AggregateError(causes, 'credential mutation, bookkeeping, and cleanup failed')
        }
      );
    }
  });
...
export async function importCredentialProfile({ home, profileId, profile, ...options } = {}) {
  ...
  return withProfileLock(home, profileId, options, async () => {
    const result = await mutateStore(home, async (store) => {
      store.profiles[profileId] = validated;
      return store;
    }, options);
    return { operationId: result.operationId, revision: result.afterRevision, profile: redactedProfile(profileId, validated) };
  });
}
...
export async function removeCredentialProfile({ home, profileId, ...options } = {}) {
  ...
  return withProfileLock(home, profileId, options, async () => {
    const result = await mutateStore(home, async (store) => {
      if (!Object.hasOwn(store.profiles, profileId)) throw new CredentialStoreError('profile-not-found', 'Credential profile not found');
      delete store.profiles[profileId];
      for (const [provider, selected] of Object.entries(store.defaults)) if (selected === profileId) delete store.defaults[provider];
      return store;
    }, options);
    return { operationId: result.operationId, revision: result.afterRevision };
  });
...
export async function setDefaultCredentialProfile({ home, provider, profileId, ...options } = {}) {
  ...
  const result = await mutateStore(home, async (store) => {
    const profile = store.profiles[profileId];
    if (!profile || profile.provider !== provider) throw new CredentialStoreError('profile-provider-mismatch', 'Default profile is absent or belongs to another provider');
...
export async function recordVerifiedCredentialScopes({
  ...
  const scopeEvidence = validateScopeEvidence({ source, evidenceHash, verifiedAt }, verifiedScopes, `profile ${profileId}`);
  if (!scopeEvidence) throw new TypeError('at least one remotely verified scope is required');
  return withProfileLock(home, profileId, options, async () => {
    const result = await mutateStore(home, async (store) => {
      const profile = store.profiles[profileId];
      if (!profile) throw new CredentialStoreError('profile-not-found', 'Credential profile not found');
    ...
    return { operationId: result.operationId, revision: result.afterRevision, profileId, verifiedScopes };
  });
}
...
async function readMarker(paths, profileId, fsImpl = fs) {
  ...
  if (!PROFILE_MARKERS.has(marker.state) || marker.profileHash !== profileHash(profileId)) throw new CredentialStoreError('profile-marker-corrupt', 'Credential profile state marker is corrupt');
  return marker;
}

async function writeMarkerUnlocked(paths, profileId, state, evidenceHash, fsImpl = fs) {
  requireSafeKey(profileId, 'profile id');
  if (!PROFILE_MARKERS.has(state)) throw new TypeError('unsupported credential profile marker state');
  requireString(evidenceHash, 'evidenceHash', { maximum: 128 });
  if (!/^[a-f0-9]{64}$/.test(evidenceHash)) throw new TypeError('evidenceHash must be a SHA-256 hex digest');
  await prepareDirectories(paths, fsImpl);
  const file = path.join(paths.profileState, `${profileHash(profileId)}.json`);
  ...
  await writePrivateFile(
    fsImpl,
    temporary,
    stableJson({ version: 1, profileHash: profileHash(profileId), state, evidenceHash, updatedAt: Date.now() }),
    { exclusive: true }
  );
...
export async function markCredentialProfileState({ home, profileId, state, evidenceHash, ...options } = {}) {
  const paths = credentialPaths(home);
...
export async function resolveCredentialProfile({
  ...
  requireSafeKey(provider, 'provider');
  if (client && profileId) throw new CredentialStoreError('profile-client-conflict', 'Explicit client and stored profile cannot both be selected');
  ...
  if (client) {
    ...
    if (required.some((scope) => !available.has(scope))) {
      throw new CredentialStoreError('profile-under-scoped', 'External client lacks remotely verified scopes');
    }
  ...
  if (!profile || profile.provider !== provider) throw new CredentialStoreError('profile-invalid', 'Configured credential profile is invalid');
  const marker = await readMarker(paths, selectedId, fsImpl);
  if (marker) throw new CredentialStoreError(`profile-${marker.state}`, 'Credential profile requires recovery');
  if (profile.expiresAt != null && profile.expiresAt <= now) throw new CredentialStoreError('profile-expired', 'Credential profile is expired');
  const verified = new Set(profile.verifiedScopes);
  const missing = required.filter((scope) => !verified.has(scope));
  if (missing.length > 0) throw new CredentialStoreError('profile-under-scoped', 'Credential profile lacks remotely verified scopes');
...
export async function profileFromSecureSource({ file, fsImpl = fs } = {}) {
  ...
  return parseCredentialJson(await readSecureDescriptorText(fsImpl, source, {
...
function oauth1Authorization(credentials, { method, url, timestamp, nonce }) {
  ...
  for (const key of parsed.searchParams.keys()) {
    if (/^oauth_/i.test(key)) {
      throw new CredentialStoreError('oauth-query-conflict', 'OAuth protocol parameters must not be supplied in the request query');
    }
  }
  ...
  return Object.freeze({
    ...
    authorize({
      ...
      if (
        ...
        throw new CredentialStoreError('adapter-origin-rejected', 'Official credentials may only be sent to an approved provider origin');
      }
      const requestMethod = requireString(method, 'official request method', { maximum: 16 }).toUpperCase();
      ...
      if (validated.provider === 'youtube' && validated.kind === 'api-key') {
        ...
        const signature = crypto.createHmac('sha256', credentials.secretKey).update(message).digest('hex');
        ...
        if (!token) throw new CredentialStoreError('adapter-credential-missing', 'Official adapter has no bearer credential');
...
export async function reconcileCredentialOperation({ home, operationId, fsImpl = fs } = {}) {
  ...
  const state = observed === journal.afterDigest ? 'committed' : observed === journal.beforeDigest ? 'not-committed' : 'ambiguous';
  journal.state = state;
  journal.updatedAt = Date.now();
  await writeJournal(fsImpl, journalPath, journal);
  return { operationId, state, beforeRevision: journal.beforeRevision, afterRevision: journal.afterRevision };
}

export async function recoverCredentialProfile({ home, profileId, mode, correctedProfile, ...options } = {}) {
  const paths = credentialPaths(home);
  const fsImpl = options.fsImpl || fs;
  return withProfileLock(home, profileId, options, async () => {
    const marker = await readMarker(paths, profileId, fsImpl);
    if (!marker) throw new CredentialStoreError('profile-recovery-not-required', 'Credential profile has no recovery marker');
    const allowed = {
      'refresh-ambiguous': ['reimport'],
      'token-invalid': ['refresh', 'reimport'],
      'scope-unverified': ['introspect', 'refresh', 'reimport']
    };
    if (!allowed[marker.state].includes(mode)) throw new CredentialStoreError('profile-recovery-mode', 'Recovery mode is not allowed for this marker');
    const candidate = mode === 'reimport'
    ...
    try {
      ...
      try {
        ...
        throw new CredentialStoreError('profile-marker-changed', 'Credential profile marker replacement is invalid', {
          ...receiptFields,
          cause: replacementError
        });
      }
      if (quarantinedDigest !== markerDigest || replacement) {
        throw new CredentialStoreError('profile-marker-changed', 'Credential profile marker changed during recovery', receiptFields);
      }
      ...
      if (postConsumeMarker) {
        throw new CredentialStoreError('profile-marker-changed', 'Credential profile marker changed while recovery evidence was consumed', receiptFields);
      }
    } catch (error) {
      ...
      if (quarantined && !consumed) {
        ...
        if (canonicalBlocks) {
          throw new CredentialStoreError('profile-marker-changed', 'Credential profile marker changed while recovery restoration failed', {
            ...receiptFields,
            cause: new AggregateError([error, restoreError], 'recovery and marker restoration failed')
          });
        }
      } else if (consumed) {
        try {
      ...
      const retained = new CredentialStoreError(
        ...
        'Credential profile recovery is ambiguous; profile lock retained',
        {
          ...receiptFields,
          cause: restoreError
            ? new AggregateError([error, restoreError], 'recovery failed and marker could not be restored')
            : error
        }
      );
    ...
    return { operationId: result.operationId, revision: result.afterRevision, recoveredFrom: marker.state, mode };
  });
}
...
export async function withCredentialProfileOperation({
  ...
  if (typeof operation !== 'function') throw new TypeError('credential profile operation callback is required');
  if (phase !== 'reserved') throw new TypeError('credential profile operations must begin in the reserved phase');
  requireSafeKey(provider, 'provider');
  const paths = credentialPaths(home);
  const initialStore = await readStore(paths, fsImpl);
  const selectedId = profileId || initialStore.defaults[provider] || null;
  if (!selectedId) throw new CredentialStoreError('profile-required', 'Stored credential profile is required');
  requireSafeKey(selectedId, 'profile id');
  ...
  try {
    ...
    if (resolved.status !== 'selected') throw new CredentialStoreError('profile-required', 'Stored credential profile is required');
    operationStarted = true;
    result = await operation({ profileId: resolved.profileId, profile: resolved.profile, setPhase });
  } catch (error) {
    primaryError = error;
    if (operationStarted) {
      try {
  ...
  try {
    ...
    if (primaryError) {
      throw new AggregateError([primaryError, releaseError], 'credential operation and profile lock release failed');
    }
    throw new CredentialStoreError('lock-release-uncertain', 'Credential operation completed but profile lock release failed', {
      result,
      cause: releaseError
    });
  }
