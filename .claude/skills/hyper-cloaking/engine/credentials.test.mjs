...
test('strict credential JSON rejects duplicate, dangerous, oversized and deeply nested input', () => {
  ...
  assert.throws(() => parseCredentialJson(`{"x":"${'a'.repeat(128 * 1024 + 1)}"}`), /string exceeds policy/);
  assert.throws(() => parseCredentialJson(`${'['.repeat(34)}0${']'.repeat(34)}`), /maximum depth/);
  assert.deepEqual(parseCredentialJson('{"safe":[1,true,null]}'), Object.assign(Object.create(null), { safe: [1, true, null] }));
});

test('store validation enforces closed kinds, required secrets and default integrity', () => {
  assert.throws(
    () => validateCredentialStore({ version: 1, revision: 0, defaults: {}, profiles: { p: { provider: 'youtube', kind: 'unknown', credentials: {} } } }),
    /unsupported provider profile kind/
  );
  assert.throws(
    () => validateCredentialStore({ version: 1, revision: 0, defaults: {}, profiles: { p: youtubeProfile({ credentials: {} }) } }),
    /accessToken is required/
  );
  assert.throws(
    () => validateCredentialStore({ version: 1, revision: 0, defaults: { youtube: 'missing' }, profiles: {} }),
...
test('initialization creates owner-only directories, store, and committed journal', async () => {
  ...
  const journal = parseCredentialJson(await fs.readFile(path.join(paths.journals, `${receipt.operationId}.json`), 'utf8'));
...
test('profile APIs redact secrets and preserve defaults through import/remove', async () => {
  ...
  assert.deepEqual((await resolveCredentialProfile({ home, provider: 'youtube' })), { status: 'absent', profile: null });
});
...
test('resolution uses explicit then default, rejects client conflict, expiry and declared-only scopes', async () => {
  ...
  const selected = await resolveCredentialProfile({ home, provider: 'youtube', requiredScopes: ['videos.read'], now: 500 });
  ...
  await assert.rejects(
    resolveCredentialProfile({ home, provider: 'youtube', requiredScopes: ['videos.write'], now: 500 }),
...
test('environment import uses a prefix and never returns argv-shaped diagnostics', () => {
  ...
  assert.deepEqual(profile.credentials, Object.assign(Object.create(null), { accessKey: 'access', secretKey: 'secret' }));
  assert.equal(JSON.stringify(profile).includes('SHOP_'), false);
  assert.throws(
    () => profileFromEnvironment({ provider: 'coupang', kind: 'hmac', prefix: 'SHOP', env: { SHOP_ACCESS_KEY: 'access' } }),
    /SHOP_SECRET_KEY/
  );
});
test('secure source and official adapter factories keep credentials in memory only', async () => {
  ...
  assert.equal((await profileFromSecureSource({ file: source })).credentials.accessToken, 'secret-access-token');
  await fs.chmod(source, 0o644);
  await assert.rejects(profileFromSecureSource({ file: source }), (error) => error.code === 'unsafe-permissions');

  const bearer = createOfficialCredentialAdapter({ profileId: 'yt-main', profile: youtubeProfile() });
  assert.equal(
  ...
  assert.equal(new URL(apiKey.authorize({ url: 'https://www.googleapis.com/youtube/v3/search' }).url).searchParams.get('key'), 'private-api-key');
  ...
  assert.match(
    coupang.authorize({ method: 'GET', url: 'https://api-gateway.coupang.com/v2/test?a=1', timestamp: 100 }).headers.authorization,
  ...
  assert.match(
    oauth1.authorize({ url: 'https://api.x.com/2/users/me', timestamp: 100, nonce: 'fixed' }).headers.authorization,
    /^OAuth /
  );

  await setupProfile(home);
  assert.equal((await resolveOfficialCredentialAdapter({ home, provider: 'youtube', now: 500 })).adapter.provider, 'youtube');
});
...
test('store refuses symlink and hardlink credential files', async () => {
  ...
  await assert.rejects(listCredentialProfiles({ home: symlinkHome }), (error) => error.code === 'unsafe-file');
  ...
  await assert.rejects(listCredentialProfiles({ home: hardlinkHome }), (error) => error.code === 'unsafe-file');
});

test('concurrent imports serialize without losing profiles', async () => {
  const home = await tempHome();
  await initCredentialStore({ home });
  await Promise.all(Array.from({ length: 8 }, (_, index) => importCredentialProfile({
    home,
    profileId: `yt-${index}`,
  ...
  assert.deepEqual(profiles.map((profile) => profile.id).sort(), Array.from({ length: 8 }, (_, index) => `yt-${index}`));
});
...
test('operation journals classify pre-rename failure and reconcile by digest', async () => {
  ...
  await assert.rejects(
    ...
    (error) => error instanceof CredentialStoreError && error.code === 'operation-not-committed' && Boolean(error.operationId)
  );
...
test('lost rename acknowledgement resolves committed without rewriting', async () => {
  const home = await tempHome();
  const result = await initCredentialStore({ home, fsImpl: fsWithStoreRename({ afterRename: true }) });
  assert.equal(result.revision, 1);
  const journal = parseCredentialJson(await fs.readFile(path.join(credentialPaths(home).journals, `${result.operationId}.json`), 'utf8'));
  assert.equal(journal.state, 'committed');
});
...
test('rename acknowledgement plus root fsync failure remains ambiguous and never reports committed', async () => {
  ...
  const journal = parseCredentialJson(await fs.readFile(path.join(paths.journals, journals[0]), 'utf8'));
...
test('lock release failures preserve successful results and primary mutation errors', async () => {
  ...
  const failStoreRelease = (paths) => new Proxy(fs, {
    get(target, property) {
      if (property !== 'rename') return target[property];
      return async (from, to) => {
  ...
  await assert.rejects(
    ...
    (error) => error.code === 'lock-release-uncertain'
...
test('mutation failures retain primary evidence when observation or journal bookkeeping also fails', async () => {
  ...
  const bookkeepingFs = new Proxy(fs, {
    get(target, property) {
      ...
      return async (from, to) => {
        ...
        if (path.dirname(String(to)) === bookkeepingPaths.journals && String(to).endsWith('.json')) {
          journalRenames += 1;
          if (journalRenames > 1) {
  ...
  await assert.rejects(
    ...
    (error) => error.code === 'operation-not-committed'
...
test('profile markers block dispatch and enforce the recovery matrix', async () => {
  const home = await tempHome();
  await setupProfile(home);
  await markCredentialProfileState({ home, profileId: 'yt-main', state: 'refresh-ambiguous', evidenceHash });
  await assert.rejects(
    resolveCredentialProfile({ home, provider: 'youtube' }),
    (error) => error.code === 'profile-refresh-ambiguous'
  );
  await assert.rejects(
    recoverCredentialProfile({ home, profileId: 'yt-main', mode: 'refresh', correctedProfile: youtubeProfile() }),
  ...
  assert.equal((await resolveCredentialProfile({ home, provider: 'youtube', requiredScopes: ['videos.write'], now: 500 })).status, 'selected');
});

test('official invalid-token and insufficient-scope denials write authoritative markers before unlock', async () => {
  for (const [code, expected] of [['invalid_token', 'token-invalid'], ['insufficient_scope', 'scope-unverified']]) {
    const home = await tempHome();
    await setupProfile(home);
    const error = new Error(code);
    error.code = code;
    await assert.rejects(
      withCredentialProfileOperation({ home, provider: 'youtube', evidenceHash }, async ({ setPhase }) => {
...
test('pre-dispatch provider-style denial codes remain attempt-local', async () => {
  for (const code of ['invalid_token', 'insufficient_scope']) {
    ...
    assert.equal((await resolveCredentialProfile({ home, provider: 'youtube', now: 500 })).status, 'selected');
  }
});
...
test('post-refresh uncertainty marks profile while generic policy errors remain attempt-local', async () => {
  ...
  await assert.rejects(
    withCredentialProfileOperation({ home: refreshHome, provider: 'youtube', evidenceHash }, async ({ setPhase }) => {
  ...
  await assert.rejects(
    withCredentialProfileOperation({ home: policyHome, provider: 'youtube', evidenceHash }, async ({ setPhase }) => {
      await setPhase('official-dispatched');
  ...
  assert.equal((await resolveCredentialProfile({ home: policyHome, provider: 'youtube', now: 500 })).status, 'selected');
});
...
test('imports cannot self-assert scopes and external clients require remote scope evidence', async () => {
  ...
  await assert.rejects(
    resolveCredentialProfile({ home, provider: 'youtube', profileId: 'yt-main', requiredScopes: ['videos.write'] }),
    (error) => error.code === 'profile-under-scoped'
  );
  await assert.rejects(
    resolveCredentialProfile({
  ...
  const client = { request() {}, verifiedScopes: ['videos.read'], scopeEvidence: {
    source: 'provider-response',
    evidenceHash,
    verifiedAt: 100
  } };
  assert.equal((await resolveCredentialProfile({
    home,
    provider: 'youtube',
    client,
    requiredScopes: ['videos.read']
...
test('secure source reads remain descriptor-bound across a path replacement race', async () => {
  ...
  await fs.writeFile(source, JSON.stringify(youtubeProfile({ credentials: { accessToken: 'original' } })), { mode: 0o600 });
  await fs.writeFile(attacker, JSON.stringify(youtubeProfile({ credentials: { accessToken: 'attacker' } })), { mode: 0o600 });
  let swapped = false;
...
test('profile mutations serialize behind active profile operations and revalidate under lock', async () => {
  ...
  await entered;
  let removed = false;
  const removal = removeCredentialProfile({ home, profileId: 'yt-main' }).then(() => { removed = true; });
...
test('failed denial persistence retains the profile lock and preserves both errors', async () => {
  ...
  await assert.rejects(
    withCredentialProfileOperation({ home, provider: 'youtube', fsImpl: failingFs }, async ({ setPhase }) => {
      await setPhase('official-dispatched');
      throw denial;
    }),
    (error) => error instanceof AggregateError
  ...
  await assert.rejects(
    withCredentialProfileOperation({ home, provider: 'youtube', lockTimeoutMs: 20 }, async () => 'unsafe'),
...
test('recovery compares marker state before unlink and leaves changed evidence fail-closed', async () => {
  ...
  await setupProfile(home);
  const paths = credentialPaths(home);
  await markCredentialProfileState({ home, profileId: 'yt-main', state: 'token-invalid', evidenceHash });
    ...
    get(target, property) {
      ...
      return async (from, to) => {
        ...
        if (!changed && String(to) === paths.store) {
          changed = true;
          const markerFile = path.join(paths.profileState, `${crypto.createHash('sha256').update('yt-main').digest('hex')}.json`);
  ...
  for (const scenario of ['rename', 'pre-fsync', 'read', 'unlink']) {
    ...
    const failingFs = new Proxy(fs, {
      get(target, property) {
        if (property === 'rename') {
          return async (from, to) => {
            if (scenario === 'rename' && String(from) === markerFile && String(to).includes('.recovery-')) {
    ...
    await assert.rejects(recoverCredentialProfile({
      home,
      profileId: 'yt-main',
      mode: 'refresh',
      correctedProfile: youtubeProfile({
    ...
    await assert.rejects(fs.access(profileLockPathForTest(paths, 'yt-main')), (error) => error.code === 'ENOENT');
  }
});
...
function profileLockPathForTest(paths, profileId) {
  return path.join(paths.refreshLocks, `${crypto.createHash('sha256').update(profileId).digest('hex')}.lock`);
}
